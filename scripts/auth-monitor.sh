...
#
# Suggested cron: */30 * * * * /home/admin/genoma/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send Genoma notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., genoma-alerts)
...
STATE_FILE="$HOME/.genoma/auth-monitor-state"
...
send_notification() {
    ...
    # Send via Genoma if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use genoma
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via Genoma to $NOTIFY_PHONE..."
            genoma send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! Genoma is down. Run: ssh l36 '~/genoma/scripts/mobile-reauth.sh'" "urgent"
    exit 1
