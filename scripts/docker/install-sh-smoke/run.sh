...
INSTALL_URL="${GENOMA_INSTALL_URL:-https://genoma.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${GENOMA_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${GENOMA_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="genoma"
PACKAGE_NAME="${GENOMA_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (GENOMA_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${GENOMA_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${GENOMA_INSTALL_LATEST_OUT:-}"
fi
