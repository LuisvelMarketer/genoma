...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  set -euo pipefail
	  ...
	  if [ -f dist/index.mjs ]; then
	    GENOMA_ENTRY="dist/index.mjs"
	  elif [ -f dist/index.js ]; then
	    GENOMA_ENTRY="dist/index.js"
	  else
  ...
  export PATH="/tmp/genoma-bin:$PATH"
  mkdir -p /tmp/genoma-bin
  cat > /tmp/genoma-bin/trash <<'"'"'TRASH'"'"'
#!/usr/bin/env bash
set -euo pipefail
...
TRASH
  ...
  wait_for_log() {
    ...
    while true; do
	  ...
	  start_gateway() {
	    node "$GENOMA_ENTRY" gateway --port 18789 --bind loopback --allow-unconfigured > /tmp/gateway-e2e.log 2>&1 &
	    GATEWAY_PID="$!"
	  }
  ...
  run_wizard_cmd() {
    ...
    set_isolated_genoma_env "$home_dir"

    input_fifo="$(mktemp -u "/tmp/genoma-onboard-${case_name}.XXXXXX")"
    mkfifo "$input_fifo"
    local log_path="/tmp/genoma-onboard-${case_name}.log"
  ...
  run_wizard() {
    ...
    local validate_fn="${4:-}"
	    ...
	    run_wizard_cmd "$case_name" "$home_dir" "node \"$GENOMA_ENTRY\" onboard $ONBOARD_FLAGS" "$send_fn" true "$validate_fn"
	  }

  make_home() {
    mktemp -d "/tmp/genoma-e2e-$1.XXXXXX"
  }

  set_isolated_genoma_env() {
    local home_dir="$1"
    export HOME="$home_dir"
    export GENOMA_HOME="$home_dir"
    export GENOMA_STATE_DIR="$home_dir/.genoma"
    export GENOMA_CONFIG_PATH="$GENOMA_STATE_DIR/genoma.json"
    mkdir -p "$GENOMA_STATE_DIR"
  }
  ...
  run_case_local_basic() {
    ...
    workspace_dir="$GENOMA_STATE_DIR/workspace"
    config_path="$GENOMA_CONFIG_PATH"
    sessions_dir="$GENOMA_STATE_DIR/agents/main/sessions"
...
NODE
  ...
  run_case_remote_non_interactive() {
    ...
    set_isolated_genoma_env "$home_dir"
	    ...
	    node "$GENOMA_ENTRY" onboard --non-interactive --accept-risk \
	      --mode remote \
...
NODE
  ...
  run_case_channels() {
	    ...
	    run_wizard_cmd channels "$home_dir" "node \"$GENOMA_ENTRY\" configure --section channels" send_channels_flow
...
JSON
	    ...
	    run_wizard_cmd skills "$home_dir" "node \"$GENOMA_ENTRY\" configure --section skills" send_skills_flow
