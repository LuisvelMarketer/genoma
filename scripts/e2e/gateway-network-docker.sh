...
IMAGE_NAME="genoma-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="genoma-net-e2e-$$"
GW_NAME="genoma-gateway-e2e-$$"
...
docker run -d \
  --name "$GW_NAME" \
  --network "$NET_NAME" \
  -e "GENOMA_GATEWAY_TOKEN=$TOKEN" \
  -e "GENOMA_SKIP_CHANNELS=1" \
  -e "GENOMA_SKIP_GMAIL_WATCHER=1" \
  -e "GENOMA_SKIP_CRON=1" \
  -e "GENOMA_SKIP_CANVAS_HOST=1" \
  "$IMAGE_NAME" \
