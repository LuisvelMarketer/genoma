...
const shouldBuild = (deps) => {
  if (deps.env.GENOMA_FORCE_BUILD === "1") {
    return true;
  }
...
const logRunner = (message, deps) => {
  if (deps.env.GENOMA_RUNNER_LOG === "0") {
    return;
  }
  deps.stderr.write(`[genoma] ${message}\n`);
};

const runGenoma = async (deps) => {
  const nodeProcess = deps.spawn(deps.execPath, ["genoma.mjs", ...deps.args], {
    cwd: deps.cwd,
    env: deps.env,
    stdio: "inherit",
  });
