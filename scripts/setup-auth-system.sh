#!/bin/bash
# Setup Genoma Auth Management System
...
echo "  2. Subscribe to a topic (e.g., 'genoma-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  Genoma message: Send warning via Genoma itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/genoma-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/genoma-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/genoma-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now genoma-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status genoma-auth-monitor.timer"
echo ""
echo "Quick commands:"
