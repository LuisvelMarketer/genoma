 ...
 * `genoma update`. Use it to verify the completion installation flow
...
\`genoma update\`, \`genoma doctor\`, and \`genoma onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# Genoma Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`genoma completion --install\` later to enable.`));
    return;
  }
