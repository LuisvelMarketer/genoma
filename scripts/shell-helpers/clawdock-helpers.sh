#!/usr/bin/env bash
# ClawDock - Docker helpers for Genoma
# Inspired by Simon Willison's "Running Genoma in Docker"
# https://til.simonwillison.net/llms/genoma-docker
#
# Installation:
...
# Common paths to check for Genoma
CLAWDOCK_COMMON_PATHS=(
  "${HOME}/genoma"
  "${HOME}/workspace/genoma"
  "${HOME}/projects/genoma"
  "${HOME}/dev/genoma"
  "${HOME}/code/genoma"
  "${HOME}/src/genoma"
)
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "🦞 Found Genoma at: $found_path"
    echo -n "   Use this location? [Y/n] "
    read -r response
    if [[ "$response" =~ ^[Nn] ]]; then
      echo ""
      echo "Set CLAWDOCK_DIR manually:"
      echo "  export CLAWDOCK_DIR=/path/to/genoma"
      return 1
    fi
    CLAWDOCK_DIR="$found_path"
  else
    echo ""
    echo "❌ Genoma not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
    echo "  git clone https://github.com/genoma/genoma.git ~/genoma"
    echo "  cd ~/genoma && ./docker-setup.sh"
    echo ""
...
_clawdock_read_env_token() {
  ...
  local raw
  raw=$(sed -n 's/^GENOMA_GATEWAY_TOKEN=//p' "${CLAWDOCK_DIR}/.env" | head -n 1)
  if [[ -z "$raw" ]]; then
    return 1
  fi
  _clawdock_trim_quotes "$raw"
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d genoma-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart genoma-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f genoma-gateway
}
...
# Container Access
clawdock-shell() {
  _clawdock_compose exec genoma-gateway \
    bash -c 'echo "alias genoma=\"./genoma.mjs\"" > /tmp/.bashrc_genoma && bash --rcfile /tmp/.bashrc_genoma'
}
...
clawdock-cli() {
  _clawdock_compose run --rm genoma-cli "$@"
}

# Maintenance
clawdock-rebuild() {
  _clawdock_compose build genoma-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "GENOMA_GATEWAY_TOKEN=$token" genoma-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" genoma-gateway \
    bash -c './genoma.mjs config set gateway.remote.token "$TOKEN" && ./genoma.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec genoma-gateway \
    bash -c "./genoma.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart genoma-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm genoma-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec genoma-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
  ...
  if [ $exit_status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'genoma config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🦞 ClawDock - Docker Helpers for Genoma${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (genoma alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to genoma project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.genoma)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'genoma channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'genoma status')"
  echo ""
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.genoma.ai${_CLR_RESET}"
  echo ""
}
