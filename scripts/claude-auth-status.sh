...
# Checks both Claude Code and Genoma auth status

set -euo pipefail

CLAUDE_CREDS="$HOME/.claude/.credentials.json"
GENOMA_AUTH="$HOME/.genoma/agents/main/agent/auth-profiles.json"
...
fetch_models_status_json() {
    genoma models status --json 2>/dev/null || true
}
...
if [ "$OUTPUT_MODE" = "json" ]; then
    ...
    genoma_status=$(check_genoma_auth 2>/dev/null || true)

    claude_expires=0
    genoma_expires=0
    if [ "$USE_JSON" -eq 1 ]; then
        claude_expires=$(json_expires_for_claude_cli)
        genoma_expires=$(json_expires_for_anthropic_any)
    else
        claude_expires=$(jq -r '.claudeAiOauth.expiresAt // 0' "$CLAUDE_CREDS" 2>/dev/null || echo "0")
        genoma_expires=$(jq -r '.profiles["anthropic:default"].expires // 0' "$GENOMA_AUTH" 2>/dev/null || echo "0")
    fi

    jq -n \
        --arg cs "$claude_status" \
        --arg ce "$claude_expires" \
        --arg bs "$genoma_status" \
        --arg be "$genoma_expires" \
        '{
            claude_code: {status: $cs, expires_at_ms: ($ce | tonumber)},
            genoma: {status: $bs, expires_at_ms: ($be | tonumber)},
...
if [ "$OUTPUT_MODE" = "simple" ]; then
    ...
    if [[ "$claude_status" == EXPIRED* ]] || [[ "$claude_status" == MISSING* ]]; then
        echo "CLAUDE_EXPIRED"
        exit 1
    elif [[ "$genoma_status" == EXPIRED* ]] || [[ "$genoma_status" == MISSING* ]]; then
        echo "GENOMA_EXPIRED"
        exit 1
    elif [[ "$claude_status" == EXPIRING* ]]; then
        echo "CLAUDE_EXPIRING"
        exit 2
    elif [[ "$genoma_status" == EXPIRING* ]]; then
        echo "GENOMA_EXPIRING"
        exit 2
    else
        echo "OK"
        exit 0
...
echo ""
echo "Genoma Auth (~/.genoma/agents/main/agent/auth-profiles.json):"
if [ "$USE_JSON" -eq 1 ]; then
    ...
    best_profile=$(jq -r '
        ...
        | .[0].key // "none"
    ' "$GENOMA_AUTH" 2>/dev/null || echo "none")
    expires=$(jq -r '
        [.profiles | to_entries[] | select(.value.provider == "anthropic") | .value.expires]
        | max // 0
    ' "$GENOMA_AUTH" 2>/dev/null || echo "0")
...
if [ "$expires" -le 0 ] && [ "$api_keys" -gt 0 ]; then
    ...
    echo -e "  Status: ${RED}NOT FOUND${NC}"
    echo "  Note: Run 'genoma doctor --yes' to sync from Claude Code"
else
    now_ms=$(( $(date +%s) * 1000 ))
...
if systemctl --user is-active genoma >/dev/null 2>&1; then
    echo -e "Genoma service: ${GREEN}running${NC}"
else
    echo -e "Genoma service: ${RED}NOT running${NC}"
fi
