...
# Build and bundle Genoma into a minimal .app we can open.
# Outputs to dist/Genoma.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/Genoma.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="Genoma"
BUNDLE_ID="${BUNDLE_ID:-ai.genoma.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/genoma/genoma/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/Genoma/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :GenomaBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :GenomaGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/Genoma"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/Genoma"
fi
chmod +x "$APP_ROOT/Contents/MacOS/Genoma"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/Genoma" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/Genoma/Resources/Genoma.icns" "$APP_ROOT/Contents/Resources/Genoma.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/Genoma/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying GenomaKit resources"
GENOMAKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/GenomaKit_GenomaKit.bundle"
if [ -d "$GENOMAKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/GenomaKit_GenomaKit.bundle"
  cp -R "$GENOMAKIT_BUNDLE" "$APP_ROOT/Contents/Resources/GenomaKit_GenomaKit.bundle"
else
  echo "WARN: GenomaKit resource bundle not found at $GENOMAKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running Genoma"
killall -q Genoma 2>/dev/null || true
