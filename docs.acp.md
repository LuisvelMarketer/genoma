...
This document describes how the Genoma ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`genoma acp` exposes an ACP agent over stdio and forwards prompts to a running
Genoma Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
- ACP client spawns `genoma acp` and speaks ACP messages over stdio.
...
`genoma acp` resolves the Gateway URL and auth from CLI flags or config:
