...
describe("workspace path resolution", () => {
  it("resolves relative read/write/edit paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("genoma-ws-", async (workspaceDir) => {
      await withTempDir("genoma-cwd-", async (otherDir) => {
        const cwdSpy = vi.spyOn(process, "cwd").mockReturnValue(otherDir);
        try {
          const tools = createGenomaCodingTools({ workspaceDir });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("genoma-ws-", async (workspaceDir) => {
      await withTempDir("genoma-override-", async (overrideDir) => {
        const tools = createGenomaCodingTools({
          workspaceDir,
          exec: { host: "gateway", ask: "off", security: "full" },
        });
  ...
  it("rejects @-prefixed absolute paths outside workspace when workspaceOnly is enabled", async () => {
    await withTempDir("genoma-ws-", async (workspaceDir) => {
      const cfg: GenomaConfig = { tools: { fs: { workspaceOnly: true } } };
      const tools = createGenomaCodingTools({ workspaceDir, config: cfg });
      const { readTool } = expectReadWriteEditTools(tools);

      const outsideAbsolute = path.resolve(path.parse(workspaceDir).root, "outside-genoma.txt");
      await expect(
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("genoma-sandbox-", async (sandboxDir) => {
      await withTempDir("genoma-workspace-", async (workspaceDir) => {
        ...
        const tools = createGenomaCodingTools({ workspaceDir, sandbox });
        const { readTool, writeTool, editTool } = expectReadWriteEditTools(tools);
