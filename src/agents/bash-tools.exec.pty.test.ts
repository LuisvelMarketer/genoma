...
test("exec sets GENOMA_SHELL in pty mode", async () => {
  const tool = createExecTool({ allowBackground: false, security: "full", ask: "off" });
  const result = await tool.execute("toolcall-genoma-shell", {
    command: "node -e \"process.stdout.write(process.env.GENOMA_SHELL || '')\"",
    pty: true,
  });
