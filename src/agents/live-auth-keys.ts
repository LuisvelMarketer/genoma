...
const GOOGLE_LIVE_SINGLE_KEY = "GENOMA_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "GENOMA_LIVE_ANTHROPIC_KEY",
    listVar: "GENOMA_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "GENOMA_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `GENOMA_LIVE_${base}_KEY`;
