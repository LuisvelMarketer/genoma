...
type AgentEntry = NonNullable<NonNullable<GenomaConfig["agents"]>["list"]>[number];
...
export function listAgentEntries(cfg: GenomaConfig): AgentEntry[] {
...
export function listAgentIds(cfg: GenomaConfig): string[] {
...
export function resolveDefaultAgentId(cfg: GenomaConfig): string {
  const agents = listAgentEntries(cfg);
  if (agents.length === 0) {
    return DEFAULT_AGENT_ID;
  }
...
function resolveAgentEntry(cfg: GenomaConfig, agentId: string): AgentEntry | undefined {
...
export function resolveAgentModelPrimary(cfg: GenomaConfig, agentId: string): string | undefined {
...
export function resolveAgentWorkspaceDir(cfg: GenomaConfig, agentId: string) {
  const id = normalizeAgentId(agentId);
...
export function resolveAgentDir(cfg: GenomaConfig, agentId: string) {
  const id = normalizeAgentId(agentId);
