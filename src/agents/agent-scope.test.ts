...
describe("resolveAgentConfig", () => {
  ...
  it("resolves explicit and effective model primary separately", () => {
    ...
    const cfgWithObjectDefault: GenomaConfig = {
      agents: {
        defaults: {
          model: {
    ...
    const cfgNoDefaults: GenomaConfig = {
      agents: {
        list: [{ id: "main" }],
      },
    };
  ...
  it("supports per-agent model primary+fallbacks", () => {
    ...
    const cfgNoOverride: GenomaConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgInheritDefaults: GenomaConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-4.1"],
  ...
  it("computes whether any model fallbacks are configured via shared helper", () => {
    const cfgDefaultsOnly: GenomaConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-4.1"],
    ...
    const cfgAgentOverrideOnly: GenomaConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: [],
  ...
  it("should normalize agent id", () => {
    ...
    expect(result?.workspace).toBe("~/genoma");
  });

  it("uses GENOMA_HOME for default agent workspace", () => {
    const home = path.join(path.sep, "srv", "genoma-home");
    vi.stubEnv("GENOMA_HOME", home);

    const workspace = resolveAgentWorkspaceDir({} as GenomaConfig, "main");
    expect(workspace).toBe(path.join(path.resolve(home), ".genoma", "workspace"));
  });

  it("uses GENOMA_HOME for default agentDir", () => {
    const home = path.join(path.sep, "srv", "genoma-home");
    vi.stubEnv("GENOMA_HOME", home);
    // Clear state dir so it falls back to GENOMA_HOME
    vi.stubEnv("GENOMA_STATE_DIR", "");

    const agentDir = resolveAgentDir({} as GenomaConfig, "main");
    expect(agentDir).toBe(path.join(path.resolve(home), ".genoma", "agents", "main", "agent"));
  });
});
