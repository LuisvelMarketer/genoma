...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "genoma.sandbox=1");
  args.push("--label", `genoma.sessionKey=${params.scopeKey}`);
  args.push("--label", `genoma.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `genoma.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "genoma.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`genoma sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`genoma sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("genoma sandbox recreate --all");
}
