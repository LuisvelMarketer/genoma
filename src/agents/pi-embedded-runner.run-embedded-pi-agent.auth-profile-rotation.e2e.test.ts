...
vi.mock("./models-config.js", async (importOriginal) => {
  ...
  return {
    ...mod,
    ensureGenomaModelsJson: vi.fn(async () => ({ wrote: false })),
  };
});
...
const makeConfig = (opts?: { fallbacks?: string[]; apiKey?: string }): GenomaConfig =>
  ({
    agents: {
      defaults: {
        model: {
...
const makeAgentOverrideOnlyFallbackConfig = (agentId: string): GenomaConfig =>
  ({
    agents: {
      defaults: {
        model: {
...
const makeCopilotConfig = (): GenomaConfig =>
  ({
    models: {
      providers: {
        "github-copilot": {
...
async function withTimedAgentWorkspace<T>(
  ...
  vi.useFakeTimers();
  try {
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "genoma-agent-"));
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "genoma-workspace-"));
    const now = Date.now();
