...
import { resolvePreferredGenomaTmpDir } from "../infra/tmp-genoma-dir.js";
...
async function withOutsideHardlinkInGenomaTmp<T>(
  params: {
...
describe("resolveSandboxedMediaSource", () => {
  ...
  it.each([
    ...
    {
      name: "absolute paths under host tmp outside genoma tmp root",
      media: path.join(os.tmpdir(), "outside-genoma", "passwd"),
      expected: /sandbox/i,
    },
    {
  ...
  it("rejects symlinked Genoma tmp paths escaping tmp root", async () => {
  ...
  it("rejects hardlinked Genoma tmp paths to outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInGenomaTmp(
      {
        openClawTmpDir,
  ...
  it("rejects symlinked Genoma tmp paths to hardlinked outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInGenomaTmp(
