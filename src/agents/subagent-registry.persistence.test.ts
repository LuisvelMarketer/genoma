...
describe("subagent registry persistence", () => {
  const envSnapshot = captureEnv(["GENOMA_STATE_DIR"]);
  ...
  const writePersistedRegistry = async (
    ...
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "genoma-subagent-"));
  ...
  it("uses isolated temp state when GENOMA_STATE_DIR is unset in tests", async () => {
    ...
    expect(registryPath).toContain(path.join(os.tmpdir(), "genoma-test-state"));
  });
});
