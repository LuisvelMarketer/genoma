...
import {
  ...
  normalizeStringList,
  parseGenomaManifestInstallBase,
  parseFrontmatterBool,
  resolveGenomaManifestBlock,
  resolveGenomaManifestInstall,
  resolveGenomaManifestOs,
  resolveGenomaManifestRequires,
...
function parseInstallSpec(input: unknown): SkillInstallSpec | undefined {
  const parsed = parseGenomaManifestInstallBase(input, ["brew", "node", "go", "uv", "download"]);
  if (!parsed) {
    return undefined;
  }
  const { raw } = parsed;
...
export function resolveGenomaMetadata(
  frontmatter: ParsedSkillFrontmatter,
): GenomaSkillMetadata | undefined {
  const metadataObj = resolveGenomaManifestBlock({ frontmatter });
  if (!metadataObj) {
    return undefined;
  }
  const requires = resolveGenomaManifestRequires(metadataObj);
  const install = resolveGenomaManifestInstall(metadataObj, parseInstallSpec);
  const osRaw = resolveGenomaManifestOs(metadataObj);
  return {
