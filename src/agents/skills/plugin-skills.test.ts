    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.acpxRoot, "genoma.plugin.json"),
      },
      {
        id: "helper",
        name: "Helper",
        ...
        manifestPath: path.join(params.helperRoot, "genoma.plugin.json"),
      },
    ],
  };
}
    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.pluginRoot, "genoma.plugin.json"),
      },
    ],
  };
}
...
async function setupAcpxAndHelperRegistry() {
  const workspaceDir = await tempDirs.make("genoma-");
  const acpxRoot = await tempDirs.make("genoma-acpx-plugin-");
  const helperRoot = await tempDirs.make("genoma-helper-plugin-");
...
async function setupPluginOutsideSkills() {
  const workspaceDir = await tempDirs.make("genoma-");
  const pluginRoot = await tempDirs.make("genoma-plugin-");
  const outsideDir = await tempDirs.make("genoma-outside-");
