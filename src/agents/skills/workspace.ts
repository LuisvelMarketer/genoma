...
function resolveSkillsLimits(config?: GenomaConfig): ResolvedSkillsLimits {
...
function loadSkillEntries(
  ...
  const skillEntries: SkillEntry[] = Array.from(merged.values()).map((skill) => {
    ...
    return {
      skill,
      frontmatter,
      metadata: resolveGenomaMetadata(frontmatter),
...
function applySkillsPromptLimits(params: { skills: Skill[]; config?: GenomaConfig }): {
...
function resolveWorkspaceSkillPromptState(
  ...
  const truncationNote = truncated
    ? `⚠️ Skills truncated: included ${skillsForPrompt.length} of ${resolvedSkills.length}. Run \`genoma skills check\` to audit.`
    : "";
  const prompt = [
    remoteNote,
    truncationNote,
