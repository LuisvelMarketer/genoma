...
describe("Agent-specific tool filtering", () => {
  ...
  async function withApplyPatchEscapeCase(
    ...
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "genoma-pi-tools-"));
...
*** End Patch`;
  ...
  function createMainSessionTools(cfg: GenomaConfig) {
    return createGenomaCodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
  ...
  function createMainAgentConfig(params: {
    tools: NonNullable<GenomaConfig["tools"]>;
    agentTools?: NonNullable<NonNullable<GenomaConfig["agents"]>["list"]>[number]["tools"];
  }): GenomaConfig {
    return {
      tools: params.tools,
      agents: {
  ...
  it("should allow different tool policies for different agents", () => {
    ...
    const mainTools = createGenomaCodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
    ...
    const familyTools = createGenomaCodingTools({
      config: cfg,
  ...
  it("should apply group tool policy overrides (group-specific beats wildcard)", () => {
    ...
    const trustedTools = createGenomaCodingTools({
      config: cfg,
  ...
  it("should apply per-sender tool policies for group tools", () => {
    ...
    const aliceTools = createGenomaCodingTools({
      config: cfg,
    ...
    const bobTools = createGenomaCodingTools({
      config: cfg,
      sessionKey: "agent:main:whatsapp:group:family",
      senderId: "bob",
  ...
  it("should not let default sender policy override group tools", () => {
    ...
    const adminTools = createGenomaCodingTools({
      config: cfg,
  ...
  it("should apply agent-specific exec host defaults over global defaults", async () => {
    ...
    const helperTools = createGenomaCodingTools({
      config: cfg,
