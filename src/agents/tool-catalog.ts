...
function buildCoreToolGroupMap() {
  ...
  const genomaTools = CORE_TOOL_DEFINITIONS.filter((tool) => tool.includeInGenomaGroup).map(
    (tool) => tool.id,
  );
  return {
    "group:genoma": genomaTools,
