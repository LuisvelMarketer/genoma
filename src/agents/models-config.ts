...
type ModelsConfig = NonNullable<GenomaConfig["models"]>;
...
export async function ensureGenomaModelsJson(
  config?: GenomaConfig,
  agentDirOverride?: string,
): Promise<{ agentDir: string; wrote: boolean }> {
  const cfg = config ?? loadConfig();
  const agentDir = agentDirOverride?.trim() ? agentDirOverride.trim() : resolveGenomaAgentDir();
