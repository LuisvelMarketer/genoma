...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses GENOMA_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      GENOMA_HOME: "/srv/genoma-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/genoma-home"), ".genoma", "workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".genoma", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("genoma-workspace-");
...
describe("loadWorkspaceBootstrapFiles", () => {
  ...
  it("treats hardlinked bootstrap aliases as missing", async () => {
    ...
    const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "genoma-workspace-hardlink-"));
    try {
