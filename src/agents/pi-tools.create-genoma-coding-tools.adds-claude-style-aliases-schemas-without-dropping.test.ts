...
import { createGenomaTools } from "./genoma-tools.js";
import { __testing, createGenomaCodingTools } from "./pi-tools.js";
import { createGenomaReadTool, createSandboxedReadTool } from "./pi-tools.read.js";
...
describe("createGenomaCodingTools", () => {
  ...
  it("exposes raw for gateway config.apply tool calls", () => {
    const gateway = createGenomaCodingTools({ senderIsOwner: true }).find(
  ...
  it("uses stored spawnDepth to apply leaf tool policy for flat depth-2 session keys", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "genoma-depth-policy-"));
  ...
  it("removes unsupported JSON Schema keywords for Cloud Code Assist API compatibility", () => {
    ...
    const googleTools = createGenomaCodingTools({
  ...
  it("applies sandbox path guards to file_path alias", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "genoma-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "genoma-outside.txt");
  ...
  it("auto-pages read output across chunks when context window budget allows", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "genoma-read-autopage-"));
  ...
  it("adds capped continuation guidance when aggregated read output reaches budget", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "genoma-read-cap-"));
  ...
  it("strips truncation.content details from read results while preserving other fields", async () => {
    ...
    const wrapped = createGenomaReadTool(
      baseRead as unknown as Parameters<typeof createGenomaReadTool>[0],
    );
