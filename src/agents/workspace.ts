...
export function resolveDefaultAgentWorkspaceDir(
  ...
  const profile = env.GENOMA_PROFILE?.trim();
  if (profile && profile.toLowerCase() !== "default") {
    return path.join(home, ".genoma", `workspace-${profile}`);
  }
  return path.join(home, ".genoma", "workspace");
}
...
const WORKSPACE_STATE_DIRNAME = ".genoma";
