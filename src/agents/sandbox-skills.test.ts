...
vi.mock("./sandbox/docker.js", () => ({
  ensureSandboxContainer: vi.fn(async () => "genoma-sbx-test"),
}));
...
describe("sandbox skill mirroring", () => {
  ...
  const runContext = async (workspaceAccess: "none" | "ro") => {
    const bundledDir = await fs.mkdtemp(path.join(os.tmpdir(), "genoma-bundled-skills-"));
    await fs.mkdir(bundledDir, { recursive: true });

    process.env.GENOMA_BUNDLED_SKILLS_DIR = bundledDir;
