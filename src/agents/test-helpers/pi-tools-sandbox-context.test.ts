...
describe("createPiToolsSandboxContext", () => {
  it("provides stable defaults for pi-tools sandbox tests", () => {
    ...
    expect(sandbox.containerName).toBe("genoma-sbx-test");
    expect(sandbox.containerWorkdir).toBe("/workspace");
    expect(sandbox.docker.image).toBe("genoma-sandbox:bookworm-slim");
    expect(sandbox.docker.containerPrefix).toBe("genoma-sbx-");
