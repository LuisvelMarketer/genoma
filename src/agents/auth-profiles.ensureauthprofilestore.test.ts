...
describe("ensureAuthProfileStore", () => {
  it("migrates legacy auth.json and deletes it (PR #368)", () => {
    const agentDir = fs.mkdtempSync(path.join(os.tmpdir(), "genoma-auth-profiles-"));
    try {
  ...
  it("merges main auth profiles into agent store and keeps agent overrides", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "genoma-auth-merge-"));
    const previousAgentDir = process.env.GENOMA_AGENT_DIR;
    ...
    try {
      ...
      process.env.GENOMA_AGENT_DIR = mainDir;
      ...
      if (previousAgentDir === undefined) {
        delete process.env.GENOMA_AGENT_DIR;
      } else {
        process.env.GENOMA_AGENT_DIR = previousAgentDir;
      }
  ...
  it("normalizes auth-profiles credential aliases with canonical-field precedence", () => {
    ...
    for (const testCase of cases) {
      const agentDir = fs.mkdtempSync(path.join(os.tmpdir(), "genoma-auth-alias-"));
      try {
        const storeData = {
          version: AUTH_STORE_VERSION,
          profiles: {
  ...
  it("normalizes mode/apiKey aliases while migrating legacy auth.json", () => {
    const agentDir = fs.mkdtempSync(path.join(os.tmpdir(), "genoma-auth-legacy-alias-"));
    try {
      fs.writeFileSync(
  ...
  it("logs one warning with aggregated reasons for rejected auth-profiles entries", () => {
    const agentDir = fs.mkdtempSync(path.join(os.tmpdir(), "genoma-auth-invalid-"));
