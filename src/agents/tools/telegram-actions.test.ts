...
describe("handleTelegramAction", () => {
  ...
  function reactionConfig(reactionLevel: "minimal" | "extensive" | "off" | "ack"): GenomaConfig {
    return {
      channels: { telegram: { botToken: "tok", reactionLevel } },
    } as GenomaConfig;
  }

  function telegramConfig(overrides?: Record<string, unknown>): GenomaConfig {
    return {
      channels: {
        telegram: {
          botToken: "tok",
  ...
  it("rejects sticker actions when disabled by default", async () => {
    const cfg = { channels: { telegram: { botToken: "tok" } } } as GenomaConfig;
    await expect(
...
describe("handleTelegramAction per-account gating", () => {
  async function expectAccountStickerSend(cfg: GenomaConfig, accountId = "media") {
    await handleTelegramAction(
