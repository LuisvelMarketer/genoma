...
async function withTempAgentDir<T>(run: (agentDir: string) => Promise<T>): Promise<T> {
  const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "genoma-pdf-"));
  try {
...
function withDefaultModel(primary: string): GenomaConfig {
  return {
    agents: { defaults: { model: { primary } } },
  } as GenomaConfig;
}

function withPdfModel(primary: string): GenomaConfig {
  return {
...
async function stubPdfToolInfra(
  ...
  vi.spyOn(modelsConfig, "ensureGenomaModelsJson").mockResolvedValue({
    agentDir,
    wrote: false,
  });
...
describe("createPdfTool", () => {
  ...
  it("respects fsPolicy.workspaceOnly for non-sandbox pdf paths", async () => {
    await withTempAgentDir(async (agentDir) => {
      vi.stubEnv("ANTHROPIC_API_KEY", "anthropic-test");
      const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "genoma-pdf-ws-"));
      const outsideDir = await fs.mkdtemp(path.join(os.tmpdir(), "genoma-pdf-out-"));
      try {
        const cfg = withDefaultModel(ANTHROPIC_PDF_MODEL);
