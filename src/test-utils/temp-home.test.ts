...
describe("createTempHomeEnv", () => {
  it("sets home env vars and restores them on cleanup", async () => {
    ...
    const previousStateDir = process.env.GENOMA_STATE_DIR;

    const tempHome = await createTempHomeEnv("genoma-temp-home-");
    expect(process.env.HOME).toBe(tempHome.home);
    expect(process.env.USERPROFILE).toBe(tempHome.home);
    expect(process.env.GENOMA_STATE_DIR).toBe(path.join(tempHome.home, ".genoma"));
    ...
    expect(process.env.GENOMA_STATE_DIR).toBe(previousStateDir);
