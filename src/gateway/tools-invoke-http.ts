...
import { createGenomaTools } from "../agents/genoma-tools.js";
import {
...
export async function handleToolsInvokeHttpRequest(
  ...
  const messageChannel = normalizeMessageChannel(
    getHeader(req, "x-genoma-message-channel") ?? "",
  );
  const accountId = getHeader(req, "x-genoma-account-id")?.trim() || undefined;
  const agentTo = getHeader(req, "x-genoma-message-to")?.trim() || undefined;
  const agentThreadId = getHeader(req, "x-genoma-thread-id")?.trim() || undefined;
  ...
  const allTools = createGenomaTools({
