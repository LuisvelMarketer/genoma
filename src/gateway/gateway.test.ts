...
describe("gateway e2e", () => {
  ...
  it(
    ...
    async () => {
      ...
      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "genoma-gw-mock-home-"));
      ...
      const workspaceDir = path.join(tempHome, "genoma");
      await fs.mkdir(workspaceDir, { recursive: true });

      const nonceA = nextGatewayId("nonce-a");
      const nonceB = nextGatewayId("nonce-b");
      const toolProbePath = path.join(workspaceDir, `.genoma-tool-probe.${nonceA}.txt`);
      await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

      const configDir = path.join(tempHome, ".genoma");
  ...
  it(
    ...
    async () => {
      ...
      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "genoma-wizard-home-"));
      process.env.HOME = tempHome;
