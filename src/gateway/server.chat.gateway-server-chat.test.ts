...
describe("gateway server chat", () => {
  ...
  test("handles chat send and history flows", async () => {
    ...
    try {
      ...
      const sendPolicyDir = await fs.mkdtemp(path.join(os.tmpdir(), "genoma-gw-"));
      ...
      const agentBlockedDir = await fs.mkdtemp(path.join(os.tmpdir(), "genoma-gw-"));
      ...
      const historyDir = await fs.mkdtemp(path.join(os.tmpdir(), "genoma-gw-"));
  ...
  test("chat.history hides assistant NO_REPLY-only entries", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "genoma-gw-"));
    try {
