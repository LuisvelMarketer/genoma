...
import type { GenomaConfig, ConfigFileSnapshot, GatewayReloadMode } from "../config/config.js";
...
export function resolveGatewayReloadSettings(cfg: GenomaConfig): GatewayReloadSettings {
...
export function startGatewayConfigReloader(opts: {
  initialConfig: GenomaConfig;
  readSnapshot: () => Promise<ConfigFileSnapshot>;
  onHotReload: (plan: GatewayReloadPlan, nextConfig: GenomaConfig) => Promise<void>;
  onRestart: (plan: GatewayReloadPlan, nextConfig: GenomaConfig) => void | Promise<void>;
  log: {
  ...
  const queueRestart = (plan: GatewayReloadPlan, nextConfig: GenomaConfig) => {
    if (restartQueued) {
      return;
    }
    restartQueued = true;
  ...
  const applySnapshot = async (nextConfig: GenomaConfig) => {
