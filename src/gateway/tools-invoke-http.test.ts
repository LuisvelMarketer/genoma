...
let lastCreateGenomaToolsContext: Record<string, unknown> | undefined;
...
vi.mock("../agents/genoma-tools.js", () => {
  ...
  const tools = [
    ...
    },
    {
      ...
      execute: async () => ({
        ok: true,
        route: {
          agentTo: lastCreateGenomaToolsContext?.agentTo,
          agentThreadId: lastCreateGenomaToolsContext?.agentThreadId,
        },
      }),
    },
    {
  ...
  return {
    createGenomaTools: (ctx: Record<string, unknown>) => {
      lastCreateGenomaToolsContext = ctx;
      return tools;
    },
  };
});
...
beforeEach(() => {
  ...
  pluginHttpHandlers = [];
  cfg = {};
  lastCreateGenomaToolsContext = undefined;
});
...
describe("POST /tools/invoke", () => {
  ...
  it("propagates message target/thread headers into tools context for sessions_spawn", async () => {
    ...
    const res = await invokeTool({
      ...
      headers: {
        ...gatewayAuthHeaders(),
        "x-genoma-message-to": "channel:24514",
        "x-genoma-thread-id": "thread-24514",
      },
      tool: "sessions_spawn",
      sessionKey: "main",
    });
