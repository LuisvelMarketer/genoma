...
export function resolveHooksConfig(cfg: GenomaConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: GenomaConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-genoma-token"] === "string"
      ? req.headers["x-genoma-token"].trim()
      : "";
  if (headerToken) {
    return headerToken;
  }
