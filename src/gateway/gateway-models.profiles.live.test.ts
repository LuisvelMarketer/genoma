...
import { resolveGenomaAgentDir } from "../agents/agent-paths.js";
...
import { ensureGenomaModelsJson } from "../agents/models-config.js";
import { discoverAuthStorage, discoverModels } from "../agents/pi-model-discovery.js";
import { loadConfig } from "../config/config.js";
import type { ModelsConfig, GenomaConfig, ModelProviderConfig } from "../config/types.js";
...
const LIVE = isTruthyEnvValue(process.env.LIVE) || isTruthyEnvValue(process.env.GENOMA_LIVE_TEST);
const GATEWAY_LIVE = isTruthyEnvValue(process.env.GENOMA_LIVE_GATEWAY);
const ZAI_FALLBACK = isTruthyEnvValue(process.env.GENOMA_LIVE_GATEWAY_ZAI_FALLBACK);
const PROVIDERS = parseFilter(process.env.GENOMA_LIVE_GATEWAY_PROVIDERS);
...
function resolveGatewayLiveMaxModels(): number {
  const gatewayMax = toInt(process.env.GENOMA_LIVE_GATEWAY_MAX_MODELS, -1);
  if (gatewayMax >= 0) {
    return gatewayMax;
  }
  // Reuse shared live-model cap when gateway-specific cap is not provided.
  return Math.max(0, toInt(process.env.GENOMA_LIVE_MAX_MODELS, 0));
}
...
function sanitizeAuthConfig(params: {
  ...
  let profiles: NonNullable<GenomaConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  const previous = {
    configPath: process.env.GENOMA_CONFIG_PATH,
    token: process.env.GENOMA_GATEWAY_TOKEN,
    skipChannels: process.env.GENOMA_SKIP_CHANNELS,
    skipGmail: process.env.GENOMA_SKIP_GMAIL_WATCHER,
    skipCron: process.env.GENOMA_SKIP_CRON,
    skipCanvas: process.env.GENOMA_SKIP_CANVAS_HOST,
    agentDir: process.env.GENOMA_AGENT_DIR,
    piAgentDir: process.env.PI_CODING_AGENT_DIR,
    stateDir: process.env.GENOMA_STATE_DIR,
  };
  let tempAgentDir: string | undefined;
  let tempStateDir: string | undefined;

  process.env.GENOMA_SKIP_CHANNELS = "1";
  process.env.GENOMA_SKIP_GMAIL_WATCHER = "1";
  process.env.GENOMA_SKIP_CRON = "1";
  process.env.GENOMA_SKIP_CANVAS_HOST = "1";

  const token = `test-${randomUUID()}`;
  process.env.GENOMA_GATEWAY_TOKEN = token;
  const agentId = "dev";

  const hostAgentDir = resolveGenomaAgentDir();
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "genoma-live-state-"));
  process.env.GENOMA_STATE_DIR = tempStateDir;
  ...
  process.env.GENOMA_AGENT_DIR = tempAgentDir;
  ...
  const toolProbePath = path.join(workspaceDir, `.genoma-live-tool-probe.${nonceA}.txt`);
  await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

  const agentDir = resolveGenomaAgentDir();
  const sanitizedCfg: GenomaConfig = {
    ...params.cfg,
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "genoma-live-"));
  const tempConfigPath = path.join(tempDir, "genoma.json");
  await fs.writeFile(tempConfigPath, `${JSON.stringify(nextCfg, null, 2)}\n`);
  process.env.GENOMA_CONFIG_PATH = tempConfigPath;

  await ensureGenomaModelsJson(nextCfg);
  ...
  try {
          ...
          for (
            ...
            const toolProbe = await client.request<AgentFinalPayload>(
              "agent",
              {
                ...
                message: strictReply
                  ? "Genoma live tool probe (local, safe): " +
    ...
    process.env.GENOMA_CONFIG_PATH = previous.configPath;
    process.env.GENOMA_GATEWAY_TOKEN = previous.token;
    process.env.GENOMA_SKIP_CHANNELS = previous.skipChannels;
    process.env.GENOMA_SKIP_GMAIL_WATCHER = previous.skipGmail;
    process.env.GENOMA_SKIP_CRON = previous.skipCron;
    process.env.GENOMA_SKIP_CANVAS_HOST = previous.skipCanvas;
    process.env.GENOMA_AGENT_DIR = previous.agentDir;
    process.env.PI_CODING_AGENT_DIR = previous.piAgentDir;
    process.env.GENOMA_STATE_DIR = previous.stateDir;
  }
}
...
describeLive("gateway live (dev agent, profile keys)", () => {
  it(
    ...
    async () => {
      ...
      await ensureGenomaModelsJson(cfg);
      ...
      const rawModels = process.env.GENOMA_LIVE_GATEWAY_MODELS?.trim();
      ...
      if (selectedCandidates.length < candidates.length) {
        logProgress(
          `[all-models] capped to ${selectedCandidates.length}/${candidates.length} via GENOMA_LIVE_GATEWAY_MAX_MODELS=${maxModels}`,
        );
      }
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.genoma-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
