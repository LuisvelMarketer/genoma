...
function expectSecurityConnectError(
  ...
  expect(error.message).toContain("genoma doctor --fix");
...
describe("GatewayClient security checks", () => {
  const envSnapshot = captureEnv(["GENOMA_ALLOW_INSECURE_PRIVATE_WS"]);
  ...
  it("allows ws:// to private addresses only with GENOMA_ALLOW_INSECURE_PRIVATE_WS=1", () => {
