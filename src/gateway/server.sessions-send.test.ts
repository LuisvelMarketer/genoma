...
const { createGenomaTools } = await import("../agents/genoma-tools.js");
...
type SessionSendTool = ReturnType<typeof createGenomaTools>[number];
...
function getSessionsSendTool(): SessionSendTool {
  ...
  const tool = createGenomaTools().find((candidate) => candidate.name === "sessions_send");
  if (!tool) {
...
beforeAll(async () => {
  envSnapshot = captureEnv(["GENOMA_GATEWAY_PORT", "GENOMA_GATEWAY_TOKEN"]);
  gatewayPort = await getFreePort();
  testState.gatewayAuth = { mode: "token", token: gatewayToken };
  process.env.GENOMA_GATEWAY_PORT = String(gatewayPort);
  process.env.GENOMA_GATEWAY_TOKEN = gatewayToken;
...
describe("sessions_send label lookup", () => {
  it(
    ...
    async () => {
      ...
      if (!configPath) {
        throw new Error("GENOMA_CONFIG_PATH missing in gateway test environment");
      }
