...
export function resolveAgentIdFromHeader(req: IncomingMessage): string | undefined {
  const raw =
    getHeader(req, "x-genoma-agent-id")?.trim() ||
    getHeader(req, "x-genoma-agent")?.trim() ||
    "";
  if (!raw) {
    return undefined;
  }
...
export function resolveAgentIdFromModel(model: string | undefined): string | undefined {
  ...
  const m =
    raw.match(/^genoma[:/](?<agentId>[a-z0-9][a-z0-9_-]{0,63})$/i) ??
...
export function resolveSessionKey(params: {
  ...
  const explicit = getHeader(params.req, "x-genoma-session-key")?.trim();
  if (explicit) {
    return explicit;
  }
...
export function resolveGatewayRequestContext(params: {
  ...
  const messageChannel = params.useMessageChannelHeader
    ? (normalizeMessageChannel(getHeader(params.req, "x-genoma-message-channel")) ??
