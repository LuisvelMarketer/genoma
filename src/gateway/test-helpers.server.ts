...
const GATEWAY_TEST_ENV_KEYS = [
  ...
  "GENOMA_STATE_DIR",
  "GENOMA_CONFIG_PATH",
  "GENOMA_SKIP_BROWSER_CONTROL_SERVER",
  "GENOMA_SKIP_GMAIL_WATCHER",
  "GENOMA_SKIP_CANVAS_HOST",
  "GENOMA_BUNDLED_PLUGINS_DIR",
  "GENOMA_SKIP_CHANNELS",
  "GENOMA_SKIP_PROVIDERS",
  "GENOMA_SKIP_CRON",
  "GENOMA_TEST_MINIMAL_GATEWAY",
] as const;
...
async function setupGatewayTestHome() {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "genoma-gateway-home-"));
  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.GENOMA_STATE_DIR = path.join(tempHome, ".genoma");
  delete process.env.GENOMA_CONFIG_PATH;
}

function applyGatewaySkipEnv() {
  process.env.GENOMA_SKIP_BROWSER_CONTROL_SERVER = "1";
  process.env.GENOMA_SKIP_GMAIL_WATCHER = "1";
  process.env.GENOMA_SKIP_CANVAS_HOST = "1";
  process.env.GENOMA_SKIP_CHANNELS = "1";
  process.env.GENOMA_SKIP_PROVIDERS = "1";
  process.env.GENOMA_SKIP_CRON = "1";
  process.env.GENOMA_TEST_MINIMAL_GATEWAY = "1";
  process.env.GENOMA_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "genoma-test-no-bundled-extensions")
    : "genoma-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  if (options.uniqueConfigRoot) {
    const suiteRoot = path.join(tempHome, ".genoma-test-suite");
    ...
    tempConfigRoot = path.join(tempHome, ".genoma-test");
    await fs.rm(tempConfigRoot, { recursive: true, force: true });
    await fs.mkdir(tempConfigRoot, { recursive: true });
  }
...
const CONNECT_CHALLENGE_NONCE_KEY = "__genomaTestConnectChallengeNonce";
const CONNECT_CHALLENGE_TRACKED_KEY = "__genomaTestConnectChallengeTracked";
...
export async function startServerWithClient(
  ...
  const envSnapshot = captureEnv(["GENOMA_GATEWAY_TOKEN"]);
  const prev = process.env.GENOMA_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.GENOMA_GATEWAY_TOKEN = fallbackToken;
  }
...
function resolveDefaultTestDeviceIdentityPath(params: {
  ...
  const safe =
    `${params.clientId}-${params.clientMode}-${params.platform}-${params.deviceFamily ?? "none"}-${params.role}`
      ...
      .toLowerCase();
  const suiteRoot = process.env.GENOMA_STATE_DIR ?? process.env.HOME ?? os.tmpdir();
