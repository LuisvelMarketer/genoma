...
describe("resolveTaskScriptPath", () => {
  it.each([
    {
      name: "uses default path when GENOMA_PROFILE is unset",
      env: { USERPROFILE: "C:\\Users\\test" },
      expected: path.join("C:\\Users\\test", ".genoma", "gateway.cmd"),
    },
    {
      name: "uses profile-specific path when GENOMA_PROFILE is set to a custom value",
      env: { USERPROFILE: "C:\\Users\\test", GENOMA_PROFILE: "jbphoenix" },
      expected: path.join("C:\\Users\\test", ".genoma-jbphoenix", "gateway.cmd"),
    },
    {
      name: "prefers GENOMA_STATE_DIR over profile-derived defaults",
      env: {
        USERPROFILE: "C:\\Users\\test",
        GENOMA_PROFILE: "rescue",
        GENOMA_STATE_DIR: "C:\\State\\genoma",
      },
      expected: path.join("C:\\State\\genoma", "gateway.cmd"),
    },
    {
      name: "falls back to HOME when USERPROFILE is not set",
      env: { HOME: "/home/test", GENOMA_PROFILE: "default" },
      expected: path.join("/home/test", ".genoma", "gateway.cmd"),
    },
  ])("$name", ({ env, expected }) => {
...
describe("readScheduledTaskCommand", () => {
  async function withScheduledTaskScript(
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "genoma-schtasks-test-"));
    try {
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          "rem Genoma Gateway",
          "cd /d C:\\Projects\\genoma",
          "set NODE_ENV=production",
          "set GENOMA_PORT=18789",
      ...
      async (env) => {
        ...
        expect(result).toEqual({
          ...
          workingDirectory: "C:\\Projects\\genoma",
          environment: {
            NODE_ENV: "production",
            GENOMA_PORT: "18789",
          },
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"C:\\Program Files\\nodejs\\node.exe" C:\\Users\\test\\AppData\\Roaming\\npm\\node_modules\\genoma\\dist\\index.js gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "C:\\Program Files\\nodejs\\node.exe",
            "C:\\Users\\test\\AppData\\Roaming\\npm\\node_modules\\genoma\\dist\\index.js",
            "gateway",
            "--port",
            "18789",
          ],
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"\\\\fileserver\\Genoma Share\\node.exe" "\\\\fileserver\\Genoma Share\\dist\\index.js" gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "\\\\fileserver\\Genoma Share\\node.exe",
            "\\\\fileserver\\Genoma Share\\dist\\index.js",
            "gateway",
            "--port",
            "18789",
          ],
  ...
  it("reads script from GENOMA_STATE_DIR override", async () => {
    await withScheduledTaskScript(
      {
        env: (tmpDir) => ({ GENOMA_STATE_DIR: path.join(tmpDir, "custom-state") }),
