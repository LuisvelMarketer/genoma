...
describeLaunchdIntegration("launchd integration", () => {
  ...
  beforeAll(async () => {
    ...
    homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `genoma-launchd-int-${testId}-`));
    env = {
      HOME: homeDir,
      GENOMA_LAUNCHD_LABEL: `ai.genoma.launchd-int-${testId}`,
      GENOMA_LOG_PREFIX: `gateway-launchd-int-${testId}`,
    };
  });
