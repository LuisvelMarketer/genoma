...
function auditGatewayToken(
  ...
  const serviceToken = command?.environment?.GENOMA_GATEWAY_TOKEN?.trim();
  ...
  issues.push({
    ...
    message:
      "Gateway service GENOMA_GATEWAY_TOKEN does not match gateway.auth.token in genoma.json",
...
export function checkTokenDrift(params: {
  ...
  if (configToken && serviceToken !== configToken) {
    return {
      ...
      detail: "Run `genoma gateway install --force` to sync the token.",
      level: "recommended",
    };
  }
