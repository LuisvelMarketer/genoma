...
let ensureGenomaCliOnPath: typeof import("./path-env.js").ensureGenomaCliOnPath;

describe("ensureGenomaCliOnPath", () => {
  const envKeys = [
    "PATH",
    "GENOMA_PATH_BOOTSTRAPPED",
    "GENOMA_ALLOW_PROJECT_LOCAL_BIN",
    "MISE_DATA_DIR",
    "HOMEBREW_PREFIX",
    "HOMEBREW_BREW_FILE",
    "XDG_BIN_HOME",
  ...
  beforeAll(async () => {
    ({ ensureGenomaCliOnPath } = await import("./path-env.js"));
  });
  ...
  it("prepends the bundled app bin dir when a sibling genoma exists", () => {
    const tmp = abs("/tmp/genoma-path/case-bundled");
    const appBinDir = path.join(tmp, "AppBin");
    const cliPath = path.join(appBinDir, "genoma");
    setDir(tmp);
    setDir(appBinDir);
    setExe(cliPath);

    process.env.PATH = "/usr/bin";
    delete process.env.GENOMA_PATH_BOOTSTRAPPED;

    ensureGenomaCliOnPath({
      execPath: cliPath,
      cwd: tmp,
      homeDir: tmp,
      platform: "darwin",
  ...
  it("is idempotent", () => {
    ...
    process.env.GENOMA_PATH_BOOTSTRAPPED = "1";
    ensureGenomaCliOnPath({
  ...
  it("prepends mise shims when available", () => {
    const tmp = abs("/tmp/genoma-path/case-mise");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "genoma");
    setDir(tmp);
    setDir(appBinDir);
    setExe(appCli);
  ...
  it("only appends project-local node_modules/.bin when explicitly enabled", () => {
    const tmp = abs("/tmp/genoma-path/case-project-local");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "genoma");
    setDir(tmp);
    setDir(appBinDir);
    ...
    const localCli = path.join(localBinDir, "genoma");
  ...
  it("prepends Linuxbrew dirs when present", () => {
    const tmp = abs("/tmp/genoma-path/case-linuxbrew");
