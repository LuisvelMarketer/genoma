...
function getDiagnosticEventsState(): DiagnosticEventsGlobalState {
  const globalStore = globalThis as typeof globalThis & {
    __genomaDiagnosticEventsState?: DiagnosticEventsGlobalState;
  };
  if (!globalStore.__genomaDiagnosticEventsState) {
    globalStore.__genomaDiagnosticEventsState = {
      seq: 0,
      listeners: new Set<(evt: DiagnosticEventPayload) => void>(),
      dispatchDepth: 0,
    };
  }
  return globalStore.__genomaDiagnosticEventsState;
}

export function isDiagnosticsEnabled(config?: GenomaConfig): boolean {
