...
export function triggerGenomaRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.GENOMA_SYSTEMD_UNIT,
        process.env.GENOMA_PROFILE,
      );
  ...
  const label =
    process.env.GENOMA_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.GENOMA_PROFILE);
