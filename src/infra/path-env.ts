...
function candidateBinDirs(opts: EnsureGenomaPathOpts): { prepend: string[]; append: string[] } {
  ...
  // Bundled macOS app: `genoma` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "genoma");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.GENOMA_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "genoma"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `genoma` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureGenomaCliOnPath(opts: EnsureGenomaPathOpts = {}) {
  if (isTruthyEnvValue(process.env.GENOMA_PATH_BOOTSTRAPPED)) {
    return;
  }
