...
export async function resolveApnsAuthConfigFromEnv(
  ...
  const teamId = normalizeNonEmptyString(env.GENOMA_APNS_TEAM_ID);
  const keyId = normalizeNonEmptyString(env.GENOMA_APNS_KEY_ID);
  if (!teamId || !keyId) {
    return {
      ok: false,
      error: "APNs auth missing: set GENOMA_APNS_TEAM_ID and GENOMA_APNS_KEY_ID",
    };
  }

  const inlineKeyRaw =
    normalizeNonEmptyString(env.GENOMA_APNS_PRIVATE_KEY_P8) ??
    normalizeNonEmptyString(env.GENOMA_APNS_PRIVATE_KEY);
  if (inlineKeyRaw) {
    return {
      ok: true,
      value: {
  ...
  const keyPath = normalizeNonEmptyString(env.GENOMA_APNS_PRIVATE_KEY_PATH);
  if (!keyPath) {
    return {
      ok: false,
      error:
        "APNs private key missing: set GENOMA_APNS_PRIVATE_KEY_P8 or GENOMA_APNS_PRIVATE_KEY_PATH",
    };
  }
  try {
    ...
    return {
      ok: false,
      error: `failed reading GENOMA_APNS_PRIVATE_KEY_PATH (${keyPath}): ${message}`,
    };
  }
}
...
function createGenomaPushMetadata(params: {
...
export async function sendApnsAlert(params: {
  ...
  const payload = {
    ...
    genoma: createGenomaPushMetadata({
      kind: "push.test",
      nodeId: params.nodeId,
    }),
  };
