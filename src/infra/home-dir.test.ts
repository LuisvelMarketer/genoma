...
describe("resolveEffectiveHomeDir", () => {
  it("prefers GENOMA_HOME over HOME and USERPROFILE", () => {
    const env = {
  ...
  it("expands GENOMA_HOME when set to ~", () => {
    const env = {
      GENOMA_HOME: "~/svc",
      HOME: "/home/alice",
    } as NodeJS.ProcessEnv;
...
describe("resolveRequiredHomeDir", () => {
  ...
  it("returns a fully resolved path for GENOMA_HOME", () => {
    const result = resolveRequiredHomeDir(
      { GENOMA_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      () => "/fallback",
    );
    expect(result).toBe(path.resolve("/custom/home"));
  });

  it("returns cwd when GENOMA_HOME is tilde-only and no fallback home exists", () => {
    expect(
      resolveRequiredHomeDir({ GENOMA_HOME: "~" } as NodeJS.ProcessEnv, () => {
        throw new Error("no home");
      }),
    ).toBe(process.cwd());
  });
...
describe("expandHomePrefix", () => {
  it("expands tilde using effective home", () => {
    const value = expandHomePrefix("~/x", {
      env: { GENOMA_HOME: "/srv/genoma-home" } as NodeJS.ProcessEnv,
    });
    expect(value).toBe(`${path.resolve("/srv/genoma-home")}/x`);
  });
