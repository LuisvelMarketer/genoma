...
const TEMP_DIR_PREFIX = "genoma-install-source-utils-";
...
async function expectPackFallsBackToDetectedArchive(params: { stdout: string }) {
  const cwd = await createTempDir("genoma-install-source-utils-");
  const archivePath = path.join(cwd, "genoma-plugin-1.2.3.tgz");
...
describe("withTempDir", () => {
  it("creates a temp dir and always removes it after callback", async () => {
    ...
    const value = await withTempDir("genoma-install-source-utils-", async (tmpDir) => {
      observedDir = tmpDir;
...
describe("resolveArchiveSourcePath", () => {
  it("returns not found error for missing archive paths", async () => {
    const result = await resolveArchiveSourcePath("/tmp/does-not-exist-genoma-archive.tgz");
...
describe("packNpmSpecToArchive", () => {
  it("packs spec and returns archive path using JSON output metadata", async () => {
    ...
    mockPackCommandResult({
      stdout: JSON.stringify([
        {
          id: "genoma-plugin@1.2.3",
          name: "genoma-plugin",
          version: "1.2.3",
          filename: "genoma-plugin-1.2.3.tgz",
    ...
    const result = await runPack("genoma-plugin@1.2.3", cwd);
    ...
    expect(result).toEqual({
      ...
      metadata: {
        name: "genoma-plugin",
        version: "1.2.3",
        resolvedSpec: "genoma-plugin@1.2.3",
        integrity: "sha512-test-integrity",
        shasum: "abc123",
      },
    });
    expect(runCommandWithTimeoutMock).toHaveBeenCalledWith(
      ["npm", "pack", "genoma-plugin@1.2.3", "--ignore-scripts", "--json"],
  ...
  it("falls back to parsing final stdout line when npm json output is unavailable", async () => {
    const cwd = await createFixtureDir();
    const expectedArchivePath = path.join(cwd, "genoma-plugin-1.2.3.tgz");
    await fs.writeFile(expectedArchivePath, "", "utf-8");
    mockPackCommandResult({
      stdout: "npm notice created package\ngenoma-plugin-1.2.3.tgz\n",
    });
  ...
  it("returns friendly error for 404 (package not on npm)", async () => {
    ...
    mockPackCommandResult({
      stdout: "",
      stderr: "npm error code E404\nnpm error 404  '@genoma/whatsapp@*' is not in this registry.",
      code: 1,
    });

    const result = await runPack("@genoma/whatsapp", cwd);

    expect(result.ok).toBe(false);
    if (!result.ok) {
      expect(result.error).toContain("Package not found on npm");
      expect(result.error).toContain("@genoma/whatsapp");
      expect(result.error).toContain("docs.genoma.ai/tools/plugin");
    }
  });
  ...
  it("returns explicit error when npm pack produces no archive name", async () => {
    ...
    const result = await runPack("genoma-plugin@1.2.3", cwd, 5000);
  ...
  it("parses scoped metadata from id-only json output even with npm notice prefix", async () => {
    const cwd = await createFixtureDir();
    await fs.writeFile(path.join(cwd, "genoma-plugin-demo-2.0.0.tgz"), "", "utf-8");
    mockPackCommandResult({
      stdout:
        "npm notice creating package\n" +
        JSON.stringify([
          {
            id: "@genoma/plugin-demo@2.0.0",
            filename: "genoma-plugin-demo-2.0.0.tgz",
          },
        ]),
    });

    const result = await runPack("@genoma/plugin-demo@2.0.0", cwd);
    expect(result).toEqual({
      ok: true,
      archivePath: path.join(cwd, "genoma-plugin-demo-2.0.0.tgz"),
      metadata: {
        resolvedSpec: "@genoma/plugin-demo@2.0.0",
      },
    });
  });
