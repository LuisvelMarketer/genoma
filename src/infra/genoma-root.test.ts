...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__genoma_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "genoma-root");
...
describe("resolveGenomaPackageRoot", () => {
  let resolveGenomaPackageRoot: typeof import("./genoma-root.js").resolveGenomaPackageRoot;
  let resolveGenomaPackageRootSync: typeof import("./genoma-root.js").resolveGenomaPackageRootSync;

  beforeAll(async () => {
    ({ resolveGenomaPackageRoot, resolveGenomaPackageRootSync } =
      await import("./genoma-root.js"));
  });
  ...
  it("resolves package root from .bin argv1", async () => {
    ...
    const argv1 = path.join(project, "node_modules", ".bin", "genoma");
    const pkgRoot = path.join(project, "node_modules", "genoma");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "genoma" }));

    expect(resolveGenomaPackageRootSync({ argv1 })).toBe(pkgRoot);
  });

  it("resolves package root via symlinked argv1", async () => {
    const project = fx("symlink-scenario");
    const bin = path.join(project, "bin", "genoma");
    const realPkg = path.join(project, "real-pkg");
    state.realpaths.set(abs(bin), abs(path.join(realPkg, "genoma.mjs")));
    setFile(path.join(realPkg, "package.json"), JSON.stringify({ name: "genoma" }));

    expect(resolveGenomaPackageRootSync({ argv1: bin })).toBe(realPkg);
  });
  ...
  it("prefers moduleUrl candidates", async () => {
    ...
    expect(resolveGenomaPackageRootSync({ moduleUrl })).toBe(pkgRoot);
  });

  it("returns null for non-genoma package roots", async () => {
    const pkgRoot = fx("not-genoma");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "not-genoma" }));

    expect(resolveGenomaPackageRootSync({ cwd: pkgRoot })).toBeNull();
  });
  ...
  it("async resolver matches sync behavior", async () => {
    ...
    await expect(resolveGenomaPackageRoot({ cwd: pkgRoot })).resolves.toBe(pkgRoot);
  });

  it("async resolver returns null when no package roots exist", async () => {
    await expect(resolveGenomaPackageRoot({ cwd: fx("missing") })).resolves.toBeNull();
  });
});
