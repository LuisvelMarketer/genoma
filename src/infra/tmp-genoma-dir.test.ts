...
import { POSIX_GENOMA_TMP_DIR, resolvePreferredGenomaTmpDir } from "./tmp-genoma-dir.js";

type TmpDirOptions = NonNullable<Parameters<typeof resolvePreferredGenomaTmpDir>[0]>;

function fallbackTmp(uid = 501) {
  return path.join("/var/fallback", `genoma-${uid}`);
}
...
function resolveWithMocks(params: {
  ...
  const wrappedLstatSync = vi.fn((target: string) => {
    if (target === POSIX_GENOMA_TMP_DIR) {
  ...
  const resolved = resolvePreferredGenomaTmpDir({
    accessSync,
    chmodSync,
...
describe("resolvePreferredGenomaTmpDir", () => {
  it("prefers /tmp/genoma when it already exists and is writable", () => {
    ...
    expect(resolved).toBe(POSIX_GENOMA_TMP_DIR);
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("prefers /tmp/genoma when it does not exist but /tmp is writable", () => {
    ...
    expect(mkdirSync).toHaveBeenCalledWith(POSIX_GENOMA_TMP_DIR, expect.any(Object));
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("falls back to os.tmpdir()/genoma when /tmp/genoma is not a directory", () => {
  ...
  it("falls back to os.tmpdir()/genoma when /tmp is not writable", () => {
  ...
  it("falls back when /tmp/genoma is a symlink", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => true,
      uid: 501,
  ...
  it("falls back when /tmp/genoma is not owned by the current user", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => false,
      uid: 0,
  ...
  it("falls back when /tmp/genoma is group/other writable", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => false,
      uid: 501,
  ...
  it("throws when fallback path is a symlink", () => {
    ...
    ).toThrow(/Unsafe fallback Genoma temp dir/);
  });
