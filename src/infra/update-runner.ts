...
const DEFAULT_PACKAGE_NAME = "genoma";
...
function normalizeTag(tag?: string) {
  return normalizePackageTagInput(tag, ["genoma", DEFAULT_PACKAGE_NAME]) ?? "latest";
}
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "genoma-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "genoma.mjs");
    ...
    const doctorStep = await runStep(
      step("genoma doctor", doctorArgv, gitRoot, { GENOMA_UPDATE_IN_PROGRESS: "1" }),
    );
