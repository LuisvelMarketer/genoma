...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.GENOMA_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*genoma-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "genoma";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "genoma");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "genoma-gw");
  ...
  records.push(`_genoma-gw._tcp IN PTR ${instanceLabel}._genoma-gw._tcp`);
  records.push(`${instanceLabel}._genoma-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._genoma-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; genoma-content-hash: ${contentHash}\n${contentBody}`;
}
