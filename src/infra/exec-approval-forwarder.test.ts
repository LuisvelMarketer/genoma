...
function makeSessionCfg(options: { discordExecApprovalsEnabled?: boolean } = {}): GenomaConfig {
  return {
...
describe("exec approval forwarder", () => {
  ...
  it("prefers turn-source routing over stale session last route", async () => {
    vi.useFakeTimers();
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "genoma-exec-approval-forwarder-test-"));
    try {
