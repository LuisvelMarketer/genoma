...
vi.mock("./genoma-root.js", () => ({
  resolveGenomaPackageRoot: vi.fn(async () => null),
  resolveGenomaPackageRootSync: vi.fn(() => null),
}));
...
let genomaRoot: typeof import("./genoma-root.js");
...
describe("control UI assets helpers (fs-mocked)", () => {
  beforeAll(async () => {
    ...
    genomaRoot = await import("./genoma-root.js");
  });
  ...
  it("uses resolveGenomaPackageRoot when available", async () => {
    const pkgRoot = abs("fixtures/genoma");
    (
      genomaRoot.resolveGenomaPackageRoot as unknown as ReturnType<typeof vi.fn>
    ).mockResolvedValueOnce(pkgRoot);

    await expect(resolveControlUiDistIndexPath(abs("fixtures/bin/genoma"))).resolves.toBe(
  ...
  it("falls back to package.json name matching when root resolution fails", async () => {
    ...
    setFile(path.join(root, "package.json"), JSON.stringify({ name: "genoma" }));
    setFile(path.join(root, "dist", "control-ui", "index.html"), "<html></html>\n");

    await expect(resolveControlUiDistIndexPath(path.join(root, "genoma.mjs"))).resolves.toBe(
      path.join(root, "dist", "control-ui", "index.html"),
    );
  });

  it("returns null when fallback package name does not match", async () => {
    const root = abs("fixtures/not-genoma");
  ...
  it("resolves control-ui root for dist bundle argv1 and moduleUrl candidates", async () => {
    const pkgRoot = abs("fixtures/genoma-bundle");
    (
      genomaRoot.resolveGenomaPackageRootSync as unknown as ReturnType<typeof vi.fn>
