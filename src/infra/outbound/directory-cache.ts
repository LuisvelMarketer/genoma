...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: GenomaConfig | null = null;
  ...
  get(key: string, cfg: GenomaConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: GenomaConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  private resetIfConfigChanged(cfg: GenomaConfig): void {
