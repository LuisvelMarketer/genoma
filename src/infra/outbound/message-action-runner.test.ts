...
describe("runMessageAction sandboxed media validation", () => {
  ...
  it("allows media paths under preferred Genoma tmp root", async () => {
    const tmpRoot = resolvePreferredGenomaTmpDir();
    ...
    try {
      ...
      const hostTmpOutsideGenoma = path.join(os.tmpdir(), "outside-genoma", "test-media.png");
      await expect(
        runMessageAction({
          cfg: slackConfig,
          action: "send",
