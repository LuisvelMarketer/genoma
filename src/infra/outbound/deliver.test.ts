...
import { resolvePreferredGenomaTmpDir } from "../tmp-genoma-dir.js";
...
const telegramChunkConfig: GenomaConfig = {
  channels: { telegram: { botToken: "tok-1", textChunkLimit: 2 } },
};

const whatsappChunkConfig: GenomaConfig = {
...
describe("deliverOutboundPayloads", () => {
  ...
  it("includes Genoma tmp root in telegram mediaLocalRoots", async () => {
    const sendTelegram = vi.fn().mockResolvedValue({ messageId: "m1", chatId: "c1" });
    ...
    expect(sendTelegram).toHaveBeenCalledWith(
      ...
      "hi",
      expect.objectContaining({
        mediaLocalRoots: expect.arrayContaining([resolvePreferredGenomaTmpDir()]),
      }),
    );
  });

  it("includes Genoma tmp root in signal mediaLocalRoots", async () => {
  ...
  it("includes Genoma tmp root in whatsapp mediaLocalRoots", async () => {
  ...
  it("includes Genoma tmp root in imessage mediaLocalRoots", async () => {
  ...
  it("uses signal media maxBytes from config", async () => {
    const sendSignal = vi.fn().mockResolvedValue({ messageId: "s1", timestamp: 123 });
    const cfg: GenomaConfig = { channels: { signal: { mediaMaxMb: 2 } } };
