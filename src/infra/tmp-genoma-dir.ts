...
export const POSIX_GENOMA_TMP_DIR = "/tmp/genoma";
const TMP_DIR_ACCESS_MODE = fs.constants.W_OK | fs.constants.X_OK;

type ResolvePreferredGenomaTmpDirOptions = {
...
export function resolvePreferredGenomaTmpDir(
  options: ResolvePreferredGenomaTmpDirOptions = {},
): string {
  ...
  const fallback = (): string => {
    const base = tmpdir();
    const suffix = uid === undefined ? "genoma" : `genoma-${uid}`;
  ...
  const tryRepairWritableBits = (candidatePath: string): boolean => {
    try {
      ...
      warn(`[genoma] tightened permissions on temp dir: ${candidatePath}`);
  ...
  const ensureTrustedFallbackDir = (): string => {
    ...
    if (state === "invalid") {
      ...
      throw new Error(`Unsafe fallback Genoma temp dir: ${fallbackPath}`);
    }
    try {
      mkdirSync(fallbackPath, { recursive: true, mode: 0o700 });
      chmodSync(fallbackPath, 0o700);
    } catch {
      throw new Error(`Unable to create fallback Genoma temp dir: ${fallbackPath}`);
    }
  ...
  const existingPreferredState = resolveDirState(POSIX_GENOMA_TMP_DIR);
  if (existingPreferredState === "available") {
    return POSIX_GENOMA_TMP_DIR;
  }
  if (existingPreferredState === "invalid") {
    if (tryRepairWritableBits(POSIX_GENOMA_TMP_DIR)) {
      return POSIX_GENOMA_TMP_DIR;
    }
  ...
  try {
    ...
    mkdirSync(POSIX_GENOMA_TMP_DIR, { recursive: true, mode: 0o700 });
    chmodSync(POSIX_GENOMA_TMP_DIR, 0o700);
    if (
      resolveDirState(POSIX_GENOMA_TMP_DIR) !== "available" &&
      !tryRepairWritableBits(POSIX_GENOMA_TMP_DIR)
    ) {
      return ensureTrustedFallbackDir();
    }
    return POSIX_GENOMA_TMP_DIR;
