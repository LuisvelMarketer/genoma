...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: GenomaConfig) => void;
};

function hasExplicitHeartbeatAgents(cfg: GenomaConfig) {
  const list = cfg.agents?.list ?? [];
  return list.some((entry) => Boolean(entry?.heartbeat));
}

export function isHeartbeatEnabledForAgent(cfg: GenomaConfig, agentId?: string): boolean {
...
function resolveHeartbeatAgents(cfg: GenomaConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: GenomaConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: GenomaConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: GenomaConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
