...
describe("update-startup", () => {
  ...
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "genoma-update-check-suite-"));
  });
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv(["GENOMA_STATE_DIR", "NODE_ENV", "VITEST"]);
    process.env.GENOMA_STATE_DIR = tempDir;
    ...
    if (!loaded) {
      ({ resolveGenomaPackageRoot } = await import("./genoma-root.js"));
    ...
    vi.mocked(resolveGenomaPackageRoot).mockClear();
  ...
  function mockPackageUpdateStatus(tag = "latest", version = "2.0.0") {
    vi.mocked(resolveGenomaPackageRoot).mockResolvedValue("/opt/genoma");
  ...
  it("uses current runtime + entrypoint for default auto-update command execution", async () => {
    ...
    process.argv = [process.execPath, "/opt/genoma/dist/entry.js"];
    try {
      await runAutoUpdateCheckWithDefaults({
        cfg: createBetaAutoUpdateConfig(),
      });
