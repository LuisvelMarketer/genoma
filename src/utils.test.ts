...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    await withTempDirSync("genoma-test-", async (tmp) => {
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", () => {
    withTempDirSync("genoma-auth-", (authDir) => {
  ...
  it("falls back through lidMappingDirs in order", () => {
    withTempDirSync("genoma-lid-a-", (first) => {
      withTempDirSync("genoma-lid-b-", (second) => {
...
describe("resolveConfigDir", () => {
  it("prefers ~/.genoma when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "genoma-config-dir-"));
    try {
...
describe("resolveHomeDir", () => {
  it("prefers GENOMA_HOME over HOME", () => {
    vi.stubEnv("GENOMA_HOME", "/srv/genoma-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveHomeDir()).toBe(path.resolve("/srv/genoma-home"));
...
describe("shortenHomePath", () => {
  it("uses $GENOMA_HOME prefix when GENOMA_HOME is set", () => {
    vi.stubEnv("GENOMA_HOME", "/srv/genoma-home");
    vi.stubEnv("HOME", "/home/other");

    expect(shortenHomePath(`${path.resolve("/srv/genoma-home")}/.genoma/genoma.json`)).toBe(
      "$GENOMA_HOME/.genoma/genoma.json",
    );
...
describe("shortenHomeInString", () => {
  it("uses $GENOMA_HOME replacement when GENOMA_HOME is set", () => {
    vi.stubEnv("GENOMA_HOME", "/srv/genoma-home");
    vi.stubEnv("HOME", "/home/other");

    expect(
      shortenHomeInString(`config: ${path.resolve("/srv/genoma-home")}/.genoma/genoma.json`),
    ).toBe("config: $GENOMA_HOME/.genoma/genoma.json");
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/genoma")).toBe(path.resolve(os.homedir(), "genoma"));
  });
  ...
  it("prefers GENOMA_HOME for tilde expansion", () => {
    vi.stubEnv("GENOMA_HOME", "/srv/genoma-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveUserPath("~/genoma")).toBe(path.resolve("/srv/genoma-home", "genoma"));
