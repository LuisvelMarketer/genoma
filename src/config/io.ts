...
import type { GenomaConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
  validateConfigObjectRawWithPlugins,
  validateConfigObjectWithPlugins,
} from "./validation.js";
import { compareGenomaVersions } from "./version.js";
...
function formatConfigValidationFailure(pathLabel: string, issueMessage: string): string {
  ...
  return [
    ...
    "",
    "Fix with:",
    `  genoma config set ${allowPath} '["*"]'`,
    "",
    "Or switch policy:",
    `  genoma config set ${policyPath} "pairing"`,
  ].join("\n");
}
...
function unsetPathForWrite(
  root: GenomaConfig,
  pathSegments: string[],
): { changed: boolean; next: GenomaConfig } {
...
function coerceConfig(value: unknown): GenomaConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as GenomaConfig;
}
...
function stampConfigVersion(cfg: GenomaConfig): GenomaConfig {
...
function warnIfConfigFromFuture(cfg: GenomaConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) {
    return;
  }
  const cmp = compareGenomaVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as GenomaConfig, env);
  }
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): GenomaConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as GenomaConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: GenomaConfig, options: ConfigWriteOptions = {}) {
    clearConfigCache();
    ...
    const outputConfigBase =
      envRefMap && changedPaths
        ? (restoreEnvRefsFromMap(cfgToWrite, "", envRefMap, changedPaths) as GenomaConfig)
        : cfgToWrite;
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.GENOMA_TEST_CONFIG_OVERWRITE_LOG === "1";
    ...
    const logConfigWriteAnomalies = () => {
      ...
      const isVitest = deps.env.VITEST === "true";
      const shouldLogInVitest = deps.env.GENOMA_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
    ...
    const auditRecordBase = {
      ...
      watchMode: deps.env.GENOMA_WATCH_MODE === "1",
      watchSession:
        typeof deps.env.GENOMA_WATCH_SESSION === "string" &&
        deps.env.GENOMA_WATCH_SESSION.trim().length > 0
          ? deps.env.GENOMA_WATCH_SESSION.trim()
          : null,
      watchCommand:
        typeof deps.env.GENOMA_WATCH_COMMAND === "string" &&
        deps.env.GENOMA_WATCH_COMMAND.trim().length > 0
          ? deps.env.GENOMA_WATCH_COMMAND.trim()
          : null,
      existsBefore: snapshot.exists,
...
// module scope. `GENOMA_CONFIG_PATH` (and friends) are expected to work even
...
let runtimeConfigSnapshot: GenomaConfig | null = null;
let runtimeConfigSourceSnapshot: GenomaConfig | null = null;

function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.GENOMA_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.GENOMA_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function getRuntimeConfigSnapshot(): GenomaConfig | null {
  return runtimeConfigSnapshot;
}

export function loadConfig(): GenomaConfig {
