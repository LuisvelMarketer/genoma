...
import type { GenomaConfig } from "./config.js";
...
function isWhatsAppConfigured(cfg: GenomaConfig): boolean {
...
function isGenericChannelConfigured(cfg: GenomaConfig, channelId: string): boolean {
  const entry = resolveChannelConfig(cfg, channelId);
  return recordHasKeys(entry);
}
...
function collectModelRefs(cfg: GenomaConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: GenomaConfig, providerId: string): boolean {
...
function collectCandidateChannelIds(cfg: GenomaConfig): string[] {
...
function isPluginExplicitlyDisabled(cfg: GenomaConfig, pluginId: string): boolean {
...
function isPluginDenied(cfg: GenomaConfig, pluginId: string): boolean {
...
function registerPluginEntry(cfg: GenomaConfig, pluginId: string): GenomaConfig {
  const builtInChannelId = normalizeChatChannelId(pluginId);
  if (builtInChannelId) {
    const channels = cfg.channels as Record<string, unknown> | undefined;
