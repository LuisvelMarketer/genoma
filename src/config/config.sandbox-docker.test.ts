...
describe("sandbox browser binds config", () => {
  ...
  it("defaults browser network to dedicated sandbox network", () => {
    ...
    expect(resolved.network).toBe("genoma-sandbox-browser");
  });

  it("prefers agent browser network over global browser network", () => {
    const resolved = resolveSandboxBrowserConfig({
      scope: "agent",
      globalBrowser: { network: "genoma-sandbox-browser-global" },
      agentBrowser: { network: "genoma-sandbox-browser-agent" },
    });
    expect(resolved.network).toBe("genoma-sandbox-browser-agent");
  });
