...
import type { ConfigFileSnapshot } from "./types.genoma.js";
import { GenomaSchema } from "./zod-schema.js";

const { mapSensitivePaths } = __test__;
const mainSchemaHints = mapSensitivePaths(GenomaSchema, "", {});
...
function makeSnapshot<TConfig extends Record<string, unknown>>(
  ...
  return {
    path: "/home/user/.genoma/config.json5",
    exists: true,
...
describe("redactConfigSnapshot", () => {
  ...
  it("does not redact passwordFile path fields", () => {
    const snapshot = makeSnapshot({
      channels: {
        irc: {
          passwordFile: "/etc/genoma/irc-password.txt",
          nickserv: {
            passwordFile: "/etc/genoma/nickserv-password.txt",
    ...
    expect(irc.passwordFile).toBe("/etc/genoma/irc-password.txt");
    expect(nickserv.passwordFile).toBe("/etc/genoma/nickserv-password.txt");
...
describe("realredactConfigSnapshot_real", () => {
  it("main schema redact works (samples)", () => {
    const schema = GenomaSchema.toJSONSchema({
      target: "draft-07",
      unrepresentable: "any",
    });
    schema.title = "GenomaConfig";
    const hints = mainSchemaHints;
