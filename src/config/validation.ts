...
import type { GenomaConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: GenomaConfig): ConfigValidationIssue[] {
...
function validateGatewayTailscaleBind(config: GenomaConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
  raw: unknown,
): { ok: true; config: GenomaConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = GenomaSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as GenomaConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as GenomaConfig);
  if (avatarIssues.length > 0) {
    return { ok: false, issues: avatarIssues };
  }
  const gatewayTailscaleBindIssues = validateGatewayTailscaleBind(validated.data as GenomaConfig);
  if (gatewayTailscaleBindIssues.length > 0) {
    return { ok: false, issues: gatewayTailscaleBindIssues };
  }
  return {
    ok: true,
    config: validated.data as GenomaConfig,
  };
}
