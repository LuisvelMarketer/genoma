...
async function writePluginFixture(params: {
  ...
  await fs.writeFile(
    path.join(params.dir, "genoma.plugin.json"),
...
describe("config plugin validation", () => {
  ...
  const envSnapshot = {
    ...
    GENOMA_PLUGIN_MANIFEST_CACHE_MS: process.env.GENOMA_PLUGIN_MANIFEST_CACHE_MS,
  };

  const validateInSuite = (raw: unknown) => validateConfigObjectWithPlugins(raw);

  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "genoma-config-plugin-validation-"));
    ...
    process.env.GENOMA_STATE_DIR = path.join(suiteHome, ".genoma");
    process.env.GENOMA_PLUGIN_MANIFEST_CACHE_MS = "10000";
  ...
  afterAll(async () => {
    ...
    clearPluginManifestRegistryCache();
    if (envSnapshot.GENOMA_STATE_DIR === undefined) {
      delete process.env.GENOMA_STATE_DIR;
    } else {
      process.env.GENOMA_STATE_DIR = envSnapshot.GENOMA_STATE_DIR;
    }
    if (envSnapshot.GENOMA_PLUGIN_MANIFEST_CACHE_MS === undefined) {
      delete process.env.GENOMA_PLUGIN_MANIFEST_CACHE_MS;
    } else {
      process.env.GENOMA_PLUGIN_MANIFEST_CACHE_MS = envSnapshot.GENOMA_PLUGIN_MANIFEST_CACHE_MS;
    }
  });
