...
import { buildWebSearchProviderConfig, withTempHome, writeGenomaConfig } from "./test-helpers.js";
import { GenomaSchema } from "./zod-schema.js";

describe("$schema key in config (#14998)", () => {
  it("accepts config with $schema string", () => {
    const result = GenomaSchema.safeParse({
      $schema: "https://genoma.ai/config.json",
    });
    expect(result.success).toBe(true);
    if (result.success) {
      expect(result.data.$schema).toBe("https://genoma.ai/config.json");
    }
  });
  ...
  it("rejects non-string $schema", () => {
    const result = GenomaSchema.safeParse({ $schema: 123 });
...
describe("cron webhook schema", () => {
  it("accepts cron.webhookToken and legacy cron.webhook", () => {
    const res = GenomaSchema.safeParse({
      cron: {
        enabled: true,
...
describe("config strict validation", () => {
  ...
  it("does not mark resolved-only gateway.bind aliases as auto-migratable legacy", async () => {
    await withTempHome(async (home) => {
      await writeGenomaConfig(home, {
        gateway: { bind: "${GENOMA_BIND}" },
      });

      const prev = process.env.GENOMA_BIND;
      process.env.GENOMA_BIND = "0.0.0.0";
      try {
        const snap = await readConfigFileSnapshot();
        expect(snap.valid).toBe(false);
        ...
        if (prev === undefined) {
          delete process.env.GENOMA_BIND;
        } else {
          process.env.GENOMA_BIND = prev;
        }
      }
    });
  });
