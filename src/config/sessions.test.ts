...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "genoma-sessions-suite-"));
  });
  ...
  const withStateDir = <T>(stateDir: string, fn: () => T): T =>
    withEnv({ GENOMA_STATE_DIR: stateDir }, fn);
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-genoma",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-genoma#general");
  });
  ...
  it("derives session transcripts dir from GENOMA_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { GENOMA_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("resolves cross-agent paths when GENOMA_STATE_DIR differs from stored paths", () => {
