...
export async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "genoma-config-" });
}

export async function writeGenomaConfig(home: string, config: unknown): Promise<string> {
...
export async function withTempHomeConfig<T>(
  ...
  return withTempHome(async (home) => {
    const configPath = await writeGenomaConfig(home, config);
