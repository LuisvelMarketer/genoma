...
const { makeStorePath } = createCronStoreHarness({ prefix: "genoma-cron-store-" });
...
describe("resolveCronStorePath", () => {
  ...
  it("uses GENOMA_HOME for tilde expansion", () => {
    vi.stubEnv("GENOMA_HOME", "/srv/genoma-home");
    vi.stubEnv("HOME", "/home/other");

    const result = resolveCronStorePath("~/cron/jobs.json");
    expect(result).toBe(path.resolve("/srv/genoma-home", "cron", "jobs.json"));
  });
});
