...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "genoma-cron-submodel-" });
}

async function writeSessionStore(home: string) {
  const dir = path.join(home, ".genoma", "sessions");
...
function makeCfg(
  home: string,
  storePath: string,
  overrides: Partial<GenomaConfig> = {},
): GenomaConfig {
  const base: GenomaConfig = {
    agents: {
      defaults: {
...
async function runSubagentModelCase(params: {
  home: string;
  cfgOverrides?: Partial<GenomaConfig>;
  ...
  it.each([
    {
      ...
      } satisfies Partial<GenomaConfig>,
