...
export type RunCronAgentTurnResult = {
   ...
   * messages.  See: https://github.com/genoma/genoma/issues/15692
   */
  delivered?: boolean;
  /**
...
export async function runCronIsolatedAgentTurn(params: {
  ...
  const isFastTestEnv = process.env.GENOMA_TEST_FAST === "1";
  ...
  // correct per-agent paths (e.g. ~/.genoma/agents/<agentId>/agent/).
  ...
  const cfgWithAgentDefaults: GenomaConfig = {
    ...params.cfg,
    ...
    const fallbackResult = await runWithModelFallback({
      ...
      run: (providerOverride, modelOverride) => {
        ...
        if (isCliProvider(providerOverride, cfgWithAgentDefaults)) {
          ...
          // the configured timeoutSeconds. See: https://github.com/genoma/genoma/issues/29774
