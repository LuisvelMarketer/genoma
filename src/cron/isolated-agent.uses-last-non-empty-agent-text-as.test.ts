...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "genoma-cron-turn-suite-" });
}
...
describe("runCronIsolatedAgentTurn", () => {
  ...
  it("passes resolved agentDir to runEmbeddedPiAgent", async () => {
    await withTempHome(async (home) => {
      ...
      expect(call?.agentDir).toBe(path.join(home, ".genoma", "agents", "main", "agent"));
    });
  });
  ...
  it("uses agentId for workspace, session key, and store paths", async () => {
    await withTempHome(async (home) => {
      ...
      const cfg = makeCfg(
        home,
        path.join(home, ".genoma", "agents", "{agentId}", "sessions", "sessions.json"),
        {
          agents: {
