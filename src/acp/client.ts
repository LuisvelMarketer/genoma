...
export function resolveAcpClientSpawnEnv(
  ...
  return { ...baseEnv, GENOMA_SHELL: "acp-client" };
}
...
export async function createAcpClient(opts: AcpClientOptions = {}): Promise<AcpClientHandle> {
  ...
  const serverCommand = opts.serverCommand ?? (entryPath ? process.execPath : "genoma");
  ...
  await client.initialize({
    ...
    clientInfo: { name: "genoma-acp-client", version: "1.0.0" },
  });
