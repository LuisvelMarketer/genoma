...
const createTempDir = () => tempDirs.make("genoma-acp-client-test-");
...
describe("resolveAcpClientSpawnEnv", () => {
  it("sets GENOMA_SHELL marker and preserves existing env values", () => {
    const env = resolveAcpClientSpawnEnv({
      PATH: "/usr/bin",
      USER: "genoma",
    });

    expect(env.GENOMA_SHELL).toBe("acp-client");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.USER).toBe("genoma");
  });

  it("overrides pre-existing GENOMA_SHELL to acp-client", () => {
    const env = resolveAcpClientSpawnEnv({
      GENOMA_SHELL: "wrong",
    });
    expect(env.GENOMA_SHELL).toBe("acp-client");
...
describe("resolveAcpClientSpawnInvocation", () => {
  it("keeps non-windows invocation unchanged", () => {
    const resolved = resolveAcpClientSpawnInvocation(
      { serverCommand: "genoma", serverArgs: ["acp", "--verbose"] },
      {
        platform: "darwin",
        env: {},
  ...
  it("unwraps .cmd shim entrypoint on windows", async () => {
    ...
    const scriptPath = path.join(dir, "genoma", "dist", "entry.js");
    const shimPath = path.join(dir, "genoma.cmd");
    await mkdir(path.dirname(scriptPath), { recursive: true });
    await writeFile(scriptPath, "console.log('ok')\n", "utf8");
    await writeFile(shimPath, `@ECHO off\r\n"%~dp0\\genoma\\dist\\entry.js" %*\r\n`, "utf8");
...
describe("resolvePermissionRequest", () => {
  ...
  it("auto-approves read when rawInput path resolves inside cwd", async () => {
    ...
    const res = await resolvePermissionRequest(
      ...
      { prompt, log: () => {}, cwd: "/tmp/genoma-acp-cwd" },
    );
    expect(prompt).not.toHaveBeenCalled();
    expect(res).toEqual({ outcome: { outcome: "selected", optionId: "allow" } });
  });
    ...
    const res = await resolvePermissionRequest(
      makePermissionRequest({
        toolCall: {
          ...
          title: "read: ignored-by-raw-input",
          status: "pending",
          rawInput: { path: "file:///tmp/genoma-acp-cwd/docs/security.md" },
        },
      }),
      { prompt, log: () => {}, cwd: "/tmp/genoma-acp-cwd" },
    );
  ...
  it("prompts for read when rawInput path escapes cwd via traversal", async () => {
    ...
    const res = await resolvePermissionRequest(
      ...
      { prompt, log: () => {}, cwd: "/tmp/genoma-acp-cwd/workspace" },
    );
    expect(prompt).toHaveBeenCalledTimes(1);
