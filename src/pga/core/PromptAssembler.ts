...
export class PromptAssembler {
    ...
    /**
     * Select best genes from C1 using epsilon-greedy
     */
    private async selectBestFromC1(context?: SelectionContext): Promise<string[]> {
        ...
        for (const [category, genes] of byCategory) {
            const best = this.selectByEpsilonGreedy(genes, epsilon);
            selected.push(`## ${this.formatCategory(category)}\n\n${best.content}`);
        }
    ...
    private selectByEpsilonGreedy<T extends { fitness: { composite: number } }>(
        ...
        const sorted = [...candidates].sort(
            (a, b) => b.fitness.composite - a.fitness.composite
        );
    ...
    injectGenes(basePrompt: string, genes: OperativeGene[]): string {
        if (genes.length === 0) return basePrompt;
