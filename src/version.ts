...
declare const __GENOMA_VERSION__: string | undefined;
const CORE_PACKAGE_NAME = "genoma";
...
export function resolveRuntimeServiceVersion(
  ...
  return (
    firstNonEmpty(
      env["GENOMA_VERSION"],
      env["GENOMA_SERVICE_VERSION"],
      env["npm_package_version"],
    ) ?? fallback
  );
}

// Single source of truth for the current Genoma version.
// - Embedded/bundled builds: injected define or env var.
// - Dev/npm builds: package.json.
export const VERSION = resolveBinaryVersion({
  moduleUrl: import.meta.url,
  injectedVersion: typeof __GENOMA_VERSION__ === "string" ? __GENOMA_VERSION__ : undefined,
  bundledVersion: process.env.GENOMA_BUNDLED_VERSION,
});
