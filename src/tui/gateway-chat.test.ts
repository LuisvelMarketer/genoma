...
describe("resolveGatewayConnection", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["GENOMA_GATEWAY_TOKEN", "GENOMA_GATEWAY_PASSWORD"]);
  ...
  it("uses GENOMA_GATEWAY_TOKEN for local mode", () => {
    loadConfig.mockReturnValue({ gateway: { mode: "local" } });

    withEnv({ GENOMA_GATEWAY_TOKEN: "env-token" }, () => {
      const result = resolveGatewayConnection({});
  ...
  it("prefers GENOMA_GATEWAY_PASSWORD over remote password fallback", () => {
    ...
    withEnv({ GENOMA_GATEWAY_PASSWORD: "env-pass" }, () => {
      const result = resolveGatewayConnection({});
