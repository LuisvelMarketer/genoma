...
describe("resolveAgentRoute", () => {
  const resolveDiscordGuildRoute = (cfg: GenomaConfig) =>
    resolveAgentRoute({
      cfg,
      channel: "discord",
      accountId: "default",
  ...
  test("defaultAgentId is used when no binding matches", () => {
    const cfg: GenomaConfig = {
      agents: {
        list: [{ id: "home", default: true, workspace: "~/genoma-home" }],
      },
    };
    const route = resolveAgentRoute({
      cfg,
...
describe("role-based agent routing", () => {
  type DiscordBinding = NonNullable<GenomaConfig["bindings"]>[number];
