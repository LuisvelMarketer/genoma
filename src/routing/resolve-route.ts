...
function listAgents(cfg: GenomaConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

type AgentLookupCache = {
  agentsRef: GenomaConfig["agents"] | undefined;
  byNormalizedId: Map<string, string>;
  fallbackDefaultAgentId: string;
};

const agentLookupCacheByCfg = new WeakMap<GenomaConfig, AgentLookupCache>();

function resolveAgentLookupCache(cfg: GenomaConfig): AgentLookupCache {
...
function pickFirstExistingAgentId(cfg: GenomaConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: GenomaConfig["bindings"];
  byChannelAccount: Map<string, EvaluatedBinding[]>;
  byChannelAccountIndex: Map<string, EvaluatedBindingsIndex>;
};

const evaluatedBindingsCacheByCfg = new WeakMap<GenomaConfig, EvaluatedBindingsCache>();
const MAX_EVALUATED_BINDINGS_CACHE_KEYS = 2000;
const resolvedRouteCacheByCfg = new WeakMap<
  GenomaConfig,
  {
    bindingsRef: GenomaConfig["bindings"];
    agentsRef: GenomaConfig["agents"];
    sessionRef: GenomaConfig["session"];
...
function resolveRouteCacheForConfig(cfg: GenomaConfig): Map<string, ResolvedAgentRoute> {
