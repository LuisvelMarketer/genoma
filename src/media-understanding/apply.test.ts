...
const TEMP_MEDIA_PREFIX = "genoma-media-";
...
function createGroqAudioConfig(): GenomaConfig {
  return {
    tools: {
      media: {
        audio: {
...
function createMediaDisabledConfig(): GenomaConfig {
  return {
    tools: {
      media: {
        audio: { enabled: false },
...
function createMediaDisabledConfigWithAllowedMimes(allowedMimes: string[]): GenomaConfig {
  return {
...
async function setupAudioAutoDetectCase(stdout: string): Promise<{
  ...
  const cfg: GenomaConfig = { tools: { media: { audio: {} } } };
...
describe("applyMediaUnderstanding", () => {
  ...
  beforeAll(async () => {
    const baseDir = resolvePreferredGenomaTmpDir();
  ...
  it("skips audio auto-detect when no supported binaries or provider keys are available", async () => {
    ...
    await withMediaAutoDetectEnv(
      {
        PATH: emptyBinDir,
        GENOMA_AGENT_DIR: isolatedAgentDir,
