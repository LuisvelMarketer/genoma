...
const ENTRY_WRAPPER_PAIRS = [
  { wrapperBasename: "genoma.mjs", entryBasename: "entry.js" },
  { wrapperBasename: "genoma.js", entryBasename: "entry.js" },
] as const;
...
if (
  ...
  if (shouldForceReadOnlyAuthStore(process.argv)) {
    process.env.GENOMA_AUTH_STORE_READONLY = "1";
  }
  ...
  function ensureExperimentalWarningSuppressed(): boolean {
    ...
    if (isTruthyEnvValue(process.env.GENOMA_NO_RESPAWN)) {
      return false;
    }
    if (isTruthyEnvValue(process.env.GENOMA_NODE_OPTIONS_READY)) {
      return false;
    }
    ...
    process.env.GENOMA_NODE_OPTIONS_READY = "1";
  ...
  function tryHandleRootVersionFastPath(argv: string[]): boolean {
    ...
    import("./version.js")
      ...
      .catch((error) => {
        console.error(
          "[genoma] Failed to resolve version:",
          error instanceof Error ? (error.stack ?? error.message) : error,
        );
        process.exitCode = 1;
      });
  ...
  if (!ensureExperimentalWarningSuppressed()) {
    ...
    if (!parsed.ok) {
      ...
      console.error(`[genoma] ${parsed.error}`);
      process.exit(2);
    }
