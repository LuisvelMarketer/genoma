...
async function createDockerSetupSandbox(): Promise<DockerSetupSandbox> {
  const rootDir = await mkdtemp(join(tmpdir(), "genoma-docker-setup-"));
...
function createEnv(
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    GENOMA_GATEWAY_TOKEN: "test-token",
    GENOMA_CONFIG_DIR: join(sandbox.rootDir, "config"),
    GENOMA_WORKSPACE_DIR: join(sandbox.rootDir, "genoma"),
  };
...
describe("docker-setup.sh", () => {
  ...
  it("handles env defaults, home-volume mounts, and apt build args", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      GENOMA_DOCKER_APT_PACKAGES: "ffmpeg build-essential",
      GENOMA_EXTRA_MOUNTS: undefined,
      GENOMA_HOME_VOLUME: "genoma-home",
    });
    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("GENOMA_DOCKER_APT_PACKAGES=ffmpeg build-essential");
    expect(envFile).toContain("GENOMA_EXTRA_MOUNTS=");
    expect(envFile).toContain("GENOMA_HOME_VOLUME=genoma-home");
    const extraCompose = await readFile(
      join(activeSandbox.rootDir, "docker-compose.extra.yml"),
      "utf8",
    );
    expect(extraCompose).toContain("genoma-home:/home/node");
    expect(extraCompose).toContain("volumes:");
    expect(extraCompose).toContain("genoma-home:");
    const log = await readFile(activeSandbox.logPath, "utf8");
    expect(log).toContain("--build-arg GENOMA_DOCKER_APT_PACKAGES=ffmpeg build-essential");
    expect(log).toContain("run --rm genoma-cli onboard --mode local --no-install-daemon");
    expect(log).toContain("run --rm genoma-cli config set gateway.mode local");
    expect(log).toContain("run --rm genoma-cli config set gateway.bind lan");
  });
  ...
  it("precreates config identity dir for CLI device auth writes", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      GENOMA_CONFIG_DIR: configDir,
      GENOMA_WORKSPACE_DIR: workspaceDir,
    });
  ...
  it("reuses existing config token when GENOMA_GATEWAY_TOKEN is unset", async () => {
    const activeSandbox = requireSandbox(sandbox);
    ...
    const result = runDockerSetup(activeSandbox, {
      GENOMA_GATEWAY_TOKEN: undefined,
      GENOMA_CONFIG_DIR: configDir,
      GENOMA_WORKSPACE_DIR: workspaceDir,
    });
    ...
    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("GENOMA_GATEWAY_TOKEN=config-token-123");
  });

  it("treats GENOMA_SANDBOX=0 as disabled", async () => {
    const activeSandbox = requireSandbox(sandbox);
    ...
    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("GENOMA_SANDBOX=");

    const log = await readFile(activeSandbox.logPath, "utf8");
    expect(log).toContain("--build-arg GENOMA_INSTALL_DOCKER_CLI=");
    expect(log).not.toContain("--build-arg GENOMA_INSTALL_DOCKER_CLI=1");
  ...
  it("resets stale sandbox mode and overlay when sandbox is not active", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      GENOMA_SANDBOX: "1",
      DOCKER_STUB_FAIL_MATCH: "--entrypoint docker genoma-gateway --version",
    });
  ...
  it("skips sandbox gateway restart when sandbox config writes fail", async () => {
    ...
    await withUnixSocket(socketPath, async () => {
      const result = runDockerSetup(activeSandbox, {
        GENOMA_SANDBOX: "1",
        GENOMA_DOCKER_SOCKET: socketPath,
      ...
      const gatewayStarts = log
        ...
        .filter(
          (line) =>
            ...
            line.includes(" up -d") &&
            line.includes("genoma-gateway"),
        );
      expect(gatewayStarts).toHaveLength(2);
      expect(log).toContain(
        "run --rm --no-deps genoma-cli config set agents.defaults.sandbox.mode non-main",
      );
      expect(log).toContain("config set agents.defaults.sandbox.mode off");
      const forceRecreateLine = log
        .split("\n")
        .find((line) => line.includes("up -d --force-recreate genoma-gateway"));
  ...
  it("rejects injected multiline GENOMA_EXTRA_MOUNTS values", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      GENOMA_EXTRA_MOUNTS: "/tmp:/tmp\n  evil-service:\n    image: alpine",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("GENOMA_EXTRA_MOUNTS cannot contain control characters");
  });

  it("rejects invalid GENOMA_EXTRA_MOUNTS mount format", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      GENOMA_EXTRA_MOUNTS: "bad mount spec",
    });
  ...
  it("rejects invalid GENOMA_HOME_VOLUME names", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      GENOMA_HOME_VOLUME: "bad name",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("GENOMA_HOME_VOLUME must match");
  });
  ...
  it("keeps docker-compose CLI network namespace settings in sync", async () => {
    const compose = await readFile(join(repoRoot, "docker-compose.yml"), "utf8");
    expect(compose).toContain('network_mode: "service:genoma-gateway"');
