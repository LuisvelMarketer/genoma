...
import { resolveStateDir, type GenomaConfig } from "../config/config.js";
...
export async function runSecretsAudit(
  ...
  const previousAuthStoreReadOnly = process.env.GENOMA_AUTH_STORE_READONLY;
  ...
  try {
    ...
    const config = snapshot.valid ? snapshot.config : ({} as GenomaConfig);
    ...
    if (previousAuthStoreReadOnly === undefined) {
      delete process.env.GENOMA_AUTH_STORE_READONLY;
    } else {
      process.env.GENOMA_AUTH_STORE_READONLY = previousAuthStoreReadOnly;
    }
  }
}
