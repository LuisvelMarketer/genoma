...
function getSecretProviders(config: GenomaConfig): Record<string, SecretProviderConfig> {
...
function removeSecretProvider(config: GenomaConfig, providerAlias: string): boolean {
  if (!isRecord(config.secrets?.providers)) {
    return false;
  }
...
function buildCandidates(config: GenomaConfig): ConfigureCandidate[] {
...
function toSourceChoices(config: GenomaConfig): Array<{ value: SecretRefSource; label: string }> {
...
async function configureProvidersInteractive(config: GenomaConfig): Promise<void> {
  while (true) {
...
function collectProviderPlanChanges(params: { original: GenomaConfig; next: GenomaConfig }): {
...
export async function runSecretsConfigureInteractive(
  ...
  if (!params.providersOnly) {
    ...
    if (candidates.length === 0) {
      throw new Error("No configurable secret-bearing fields found in genoma.json.");
    }
  ...
  const plan: SecretsApplyPlan = {
    ...
    generatedBy: "genoma secrets configure",
