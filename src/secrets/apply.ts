...
function setByPathSegments(root: GenomaConfig, segments: string[], value: unknown): boolean {
  if (segments.length === 0) {
...
function deleteByPathSegments(root: GenomaConfig, segments: string[]): boolean {
  if (segments.length === 0) {
    return false;
  }
  let cursor: Record<string, unknown> = root as unknown as Record<string, unknown>;
