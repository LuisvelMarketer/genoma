...
function buildFixturePaths(rootDir: string) {
  const stateDir = path.join(rootDir, ".genoma");
  return {
    rootDir,
    stateDir,
...
async function createApplyFixture(): Promise<ApplyFixture> {
  const paths = buildFixturePaths(
    await fs.mkdtemp(path.join(os.tmpdir(), "genoma-secrets-apply-")),
  );
  ...
  return {
    ...paths,
    env: {
      GENOMA_STATE_DIR: paths.stateDir,
      GENOMA_CONFIG_PATH: paths.configPath,
