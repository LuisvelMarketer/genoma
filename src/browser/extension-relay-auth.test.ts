...
async function probeRelay(baseUrl: string, relayAuthToken: string): Promise<boolean> {
  return await probeAuthenticatedGenomaRelay({
    baseUrl,
    relayAuthHeader: "x-genoma-relay-token",
    relayAuthToken,
  });
}
...
describe("extension-relay-auth", () => {
  ...
  it("accepts authenticated genoma relay probe responses", async () => {
