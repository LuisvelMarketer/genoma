import type { BrowserConfig, BrowserProfileConfig, GenomaConfig } from "../config/config.js";
...
import {
  ...
  DEFAULT_GENOMA_BROWSER_ENABLED,
...
function normalizeHexColor(raw: string | undefined) {
  ...
  if (!value) {
    return DEFAULT_GENOMA_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "genoma" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_GENOMA_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_GENOMA_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an Genoma browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for genoma/genoma CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: GenomaConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_GENOMA_BROWSER_ENABLED;
  ...
  const defaultProfile =
    ...
    (profiles[DEFAULT_BROWSER_DEFAULT_PROFILE_NAME]
      ? DEFAULT_BROWSER_DEFAULT_PROFILE_NAME
      : profiles[DEFAULT_GENOMA_BROWSER_PROFILE_NAME]
        ? DEFAULT_GENOMA_BROWSER_PROFILE_NAME
        : "chrome");
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "genoma";
