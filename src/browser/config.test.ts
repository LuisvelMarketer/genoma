...
describe("browser config", () => {
  it("defaults to enabled with loopback defaults and lobster-orange color", () => {
    ...
    expect(profile?.name).toBe("genoma");
    expect(profile?.driver).toBe("genoma");
    expect(profile?.cdpPort).toBe(18800);
    expect(profile?.cdpUrl).toBe("http://127.0.0.1:18800");

    const genoma = resolveProfile(resolved, "genoma");
    expect(genoma?.driver).toBe("genoma");
    expect(genoma?.cdpPort).toBe(18800);
    expect(genoma?.cdpUrl).toBe("http://127.0.0.1:18800");
  ...
  it("derives default ports from GENOMA_GATEWAY_PORT when unset", () => {
    withEnv({ GENOMA_GATEWAY_PORT: "19001" }, () => {
      const resolved = resolveBrowserConfig(undefined);
      ...
      const genoma = resolveProfile(resolved, "genoma");
      expect(genoma?.cdpPort).toBe(19012);
      expect(genoma?.cdpUrl).toBe("http://127.0.0.1:19012");
    });
  });

  it("derives default ports from gateway.port when env is unset", () => {
    withEnv({ GENOMA_GATEWAY_PORT: undefined }, () => {
      ...
      const genoma = resolveProfile(resolved, "genoma");
      expect(genoma?.cdpPort).toBe(19022);
      expect(genoma?.cdpUrl).toBe("http://127.0.0.1:19022");
    });
  });
  ...
  it("supports overriding the local CDP auto-allocation range start", () => {
    ...
    expect(genoma?.cdpPort).toBe(19000);
    expect(genoma?.cdpUrl).toBe("http://127.0.0.1:19000");
  });
  ...
  it("treats non-loopback cdpUrl as remote", () => {
    ...
    const profile = resolveProfile(resolved, "genoma");
  ...
  it("does not add the built-in chrome extension profile if the derived relay port is already used", () => {
    const resolved = resolveBrowserConfig({
      profiles: {
        genoma: { cdpPort: 18792, color: "#FF4500" },
      },
    });
    expect(resolveProfile(resolved, "chrome")).toBe(null);
    expect(resolved.defaultProfile).toBe("genoma");
  });
  ...
  describe("default profile preference", () => {
    it("defaults to genoma profile when defaultProfile is not configured", () => {
      const resolved = resolveBrowserConfig({
        headless: false,
        noSandbox: false,
      });
    ...
    it("keeps genoma default when headless=true", () => {
      const resolved = resolveBrowserConfig({
        headless: true,
      });
      expect(resolved.defaultProfile).toBe("genoma");
    ...
    it("keeps genoma default when noSandbox=true", () => {
      const resolved = resolveBrowserConfig({
        noSandbox: true,
      });
      expect(resolved.defaultProfile).toBe("genoma");
    ...
    it("keeps genoma default when both headless and noSandbox are true", () => {
      const resolved = resolveBrowserConfig({
        headless: true,
        noSandbox: true,
      });
