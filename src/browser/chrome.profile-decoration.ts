...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".genoma-profile-decorated");
}
 ...
 */
export function decorateGenomaProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_GENOMA_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_GENOMA_BROWSER_COLOR).toUpperCase();
