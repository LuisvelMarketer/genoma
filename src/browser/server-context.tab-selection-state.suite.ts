...
describe("browser server-context tab selection state", () => {
  it("updates lastTargetId when openTab is created via CDP", async () => {
    ...
    const state = makeState("genoma");
    const ctx = createBrowserRouteContext({ getState: () => state });
    const genoma = ctx.forProfile("genoma");

    const opened = await genoma.openTab("http://127.0.0.1:8080");
    expect(opened.targetId).toBe("CREATED");
    expect(state.profiles.get("genoma")?.lastTargetId).toBe("CREATED");
  ...
  it("closes excess managed tabs after opening a new tab", async () => {
    ...
    const opened = await genoma.openTab("http://127.0.0.1:3009");
  ...
  it("does not block openTab on slow best-effort cleanup closes", async () => {
    ...
    const opened = await Promise.race([
      genoma.openTab("http://127.0.0.1:3009"),
  ...
  it("blocks unsupported non-network URLs before any HTTP tab-open fallback", async () => {
    ...
    await expect(genoma.openTab("file:///etc/passwd")).rejects.toBeInstanceOf(
