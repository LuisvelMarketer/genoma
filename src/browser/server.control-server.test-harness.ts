...
export function restoreGatewayPortEnv(prevGatewayPort: string | undefined): void {
  ...
  process.env.GENOMA_GATEWAY_PORT = prevGatewayPort;
}
...
const chromeUserDataDir = vi.hoisted(() => ({ dir: "/tmp/genoma" }));
  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        headless: true,
        defaultProfile: "genoma",
        profiles: {
          genoma: { cdpPort: state.testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
vi.mock("./chrome.js", () => ({
  ...
  launchGenomaChrome: vi.fn(async (_resolved: unknown, profile: { cdpPort: number }) => {
...
export async function resetBrowserControlServerTestContext(): Promise<void> {
  ...
  state.prevGatewayPort = process.env.GENOMA_GATEWAY_PORT;
  process.env.GENOMA_GATEWAY_PORT = String(state.testPort - 2);
  ...
  state.prevGatewayToken = process.env.GENOMA_GATEWAY_TOKEN;
  state.prevGatewayPassword = process.env.GENOMA_GATEWAY_PASSWORD;
...
export function restoreGatewayAuthEnv(
  ...
  if (prevGatewayToken === undefined) {
    ...
    process.env.GENOMA_GATEWAY_TOKEN = prevGatewayToken;
  }
  if (prevGatewayPassword === undefined) {
    delete process.env.GENOMA_GATEWAY_PASSWORD;
  } else {
    process.env.GENOMA_GATEWAY_PASSWORD = prevGatewayPassword;
  }
}
