...
vi.mock("./chrome.js", () => ({
  resolveGenomaUserDataDir: vi.fn(() => "/tmp/genoma-test/genoma/user-data"),
}));

import { loadConfig, writeConfigFile } from "../config/config.js";
import { resolveGenomaUserDataDir } from "./chrome.js";
...
describe("BrowserProfilesService", () => {
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "genoma-profile-"));
    const userDataDir = path.join(tempDir, "work", "user-data");
    fs.mkdirSync(path.dirname(userDataDir), { recursive: true });
    vi.mocked(resolveGenomaUserDataDir).mockReturnValue(userDataDir);
