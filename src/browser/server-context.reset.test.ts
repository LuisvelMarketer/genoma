...
describe("createProfileResetOps", () => {
  it("stops extension relay for extension profiles", async () => {
    const ops = createProfileResetOps({
      ...
      resolveGenomaUserDataDir: (name: string) => `/tmp/${name}`,
    });
  ...
  it("stops local browser, closes playwright connection, and trashes profile dir", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "genoma-reset-"));
    const profileDir = path.join(tempRoot, "genoma");
    ...
    const ops = createProfileResetOps({
      ...
      stopRunningBrowser,
      isHttpReachable,
      resolveGenomaUserDataDir: () => profileDir,
    });
  ...
  it("forces playwright disconnect when loopback cdp is occupied by non-owned process", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "genoma-reset-no-own-"));
    const profileDir = path.join(tempRoot, "genoma");
    fs.mkdirSync(profileDir, { recursive: true });
