...
export function resolveGenomaUserDataDir(profileName = DEFAULT_GENOMA_BROWSER_PROFILE_NAME) {
...
export async function launchGenomaChrome(
  ...
  const userDataDir = resolveGenomaUserDataDir(profile.name);
  fs.mkdirSync(userDataDir, { recursive: true });

  const needsDecorate = !isProfileDecorated(
    userDataDir,
    profile.name,
    (profile.color ?? DEFAULT_GENOMA_BROWSER_COLOR).toUpperCase(),
  );
  ...
  if (needsDecorate) {
    try {
      decorateGenomaProfile(userDataDir, {
        name: profile.name,
        color: profile.color,
      });
  ...
  try {
    ...
    log.warn(`genoma browser clean-exit prefs failed: ${String(err)}`);
  }
