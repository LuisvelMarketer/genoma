...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredGenomaTmpDir: vi.fn(() => "/tmp/genoma"),
}));
vi.mock("../infra/tmp-genoma-dir.js", () => tmpDirMocks);
...
describe("pw-tools-core", () => {
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredGenomaTmpDir.mockReturnValue("/tmp/genoma");
  });

  async function withTempDir<T>(run: (tempDir: string) => Promise<T>): Promise<T> {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "genoma-browser-download-test-"));
    try {
  ...
  async function expectAtomicDownloadSave(params: {
    ...
    expect(path.basename(String(savedPath))).toContain(".genoma-output-");
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredGenomaTmpDir.mockReturnValue("/tmp/genoma-preferred");
    ...
    const expectedRootedDownloadsDir = path.resolve(
      path.join(path.sep, "tmp", "genoma-preferred", "downloads"),
    );
    const expectedDownloadsTail = `${path.join("tmp", "genoma-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredGenomaTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(String(outPath))).toBe(
      path.resolve(path.join(path.sep, "tmp", "genoma-preferred", "downloads")),
    );
    expect(path.basename(String(outPath))).toMatch(/-passwd$/);
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "genoma-preferred", "downloads")}${path.sep}`),
    );
  });
