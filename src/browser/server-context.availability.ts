...
export function createProfileAvailability({
  ...
  const waitForCdpReadyAfterLaunch = async (): Promise<void> => {
    // launchGenomaChrome() can return before Chrome is fully ready to serve /json/version + CDP WS.
  ...
  const ensureBrowserAvailable = async (): Promise<void> => {
    ...
    if (!httpReachable) {
      ...
      const launched = await launchGenomaChrome(current.resolved, profile);
      attachRunning(launched);
      try {
        await waitForCdpReadyAfterLaunch();
      } catch (err) {
        await stopGenomaChrome(launched).catch(() => {});
        setProfileRunning(null);
        throw err;
      }
      return;
    ...
    if (!profileState.running) {
      throw new Error(
        `Port ${profile.cdpPort} is in use for profile "${profile.name}" but not by genoma. ` +
          `Run action=reset-profile profile=${profile.name} to kill the process.`,
      );
    }

    await stopGenomaChrome(profileState.running);
    setProfileRunning(null);

    const relaunched = await launchGenomaChrome(current.resolved, profile);
