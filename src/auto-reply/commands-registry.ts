...
export function isCommandEnabled(cfg: GenomaConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: GenomaConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as GenomaConfig),
...
export function getCommandDetection(_cfg?: GenomaConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: GenomaConfig) {
