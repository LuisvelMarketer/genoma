...
describe("abort detection", () => {
  ...
  async function createAbortConfig(params?: {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "genoma-abort-"));
  ...
  it("isAbortRequestText aligns abort command semantics", () => {
    ...
    expect(isAbortRequestText("stop genoma!!!")).toBe(true);
    ...
    expect(isAbortRequestText("/stop@genoma_bot", { botUsername: "genoma_bot" })).toBe(true);
    expect(isAbortRequestText("/Stop@genoma_bot", { botUsername: "genoma_bot" })).toBe(true);
