...
function createDiscordParams(commandBody: string, cfg: GenomaConfig = baseCfg) {
...
function createThreadParams(commandBody: string, cfg: GenomaConfig = baseCfg) {
  const params = createDiscordParams(commandBody, cfg);
  params.ctx.MessageThreadId = defaultThreadId;
  return params;
}

async function runDiscordAcpCommand(commandBody: string, cfg: GenomaConfig = baseCfg) {
  return handleAcpCommand(createDiscordParams(commandBody, cfg), true);
}

async function runThreadAcpCommand(commandBody: string, cfg: GenomaConfig = baseCfg) {
