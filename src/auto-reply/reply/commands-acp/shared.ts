...
export function resolveConfiguredAcpBackendId(cfg: GenomaConfig): string {
  return cfg.acp?.backend?.trim() || "acpx";
}

export function resolveAcpInstallCommandHint(cfg: GenomaConfig): string {
  ...
  if (backendId === "acpx") {
    ...
    if (existsSync(localPath)) {
      return `genoma plugins install ${localPath}`;
    }
    return "genoma plugins install @genoma/acpx";
  }
