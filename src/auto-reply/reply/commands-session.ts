...
import { scheduleGatewaySigusr1Restart, triggerGenomaRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting Genoma in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerGenomaRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting Genoma via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
