...
async function requireRiskAcknowledgement(params: {
  ...
  await params.prompter.note(
    [
      ...
      "",
      "Must read: https://docs.genoma.ai/gateway/security",
    ].join("\n"),
    "Security",
  );
...
export async function runOnboardingWizard(
  ...
  let baseConfig: GenomaConfig = snapshot.valid ? snapshot.config : {};
  ...
  if (snapshot.exists && !snapshot.valid) {
    ...
    await prompter.outro(
      `Config invalid. Run \`${formatCliCommand("genoma doctor")}\` to repair it, then re-run onboarding.`,
    );
    runtime.exit(1);
    return;
  }

  const quickstartHint = `Configure details later via ${formatCliCommand("genoma configure")}.`;
  ...
  let nextConfig: GenomaConfig = applyOnboardingLocalWorkspaceConfig(baseConfig, workspaceDir);
