...
function telegramCfg(): GenomaConfig {
  return { channels: { telegram: { botToken: "tok" } } } as GenomaConfig;
}
...
async function runTelegramAction(
  ...
  options?: { cfg?: GenomaConfig; accountId?: string },
) {
...
async function runSignalAction(
  ...
  const cfg =
    options?.cfg ?? ({ channels: { signal: { account: "+15550001111" } } } as GenomaConfig);
...
function slackHarness() {
  const cfg = { channels: { slack: { botToken: "tok" } } } as GenomaConfig;
...
function createSignalAccountOverrideCfg(): GenomaConfig {
  return {
    channels: {
      signal: {
...
describe("discord message actions", () => {
  it("lists channel and upload actions by default", async () => {
    const cfg = { channels: { discord: { token: "d0" } } } as GenomaConfig;
  ...
  it("lists moderation when at least one account enables it", () => {
    ...
    for (const channelConfig of cases) {
      const cfg = channelConfig as unknown as GenomaConfig;
      const actions = discordMessageActions.listActions?.({ cfg }) ?? [];
  ...
  it("maps reaction targets into signal sendReaction calls", async () => {
    const cases = [
      ...
      },
      {
        ...
        cfg: { channels: { signal: { account: "+15550001111" } } } as GenomaConfig,
        accountId: undefined,
        params: {
