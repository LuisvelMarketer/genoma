...
describe("message action capability checks", () => {
  ...
  it("aggregates buttons/card support across plugins", () => {
    ...
    expect(supportsChannelMessageButtons({} as GenomaConfig)).toBe(true);
    expect(supportsChannelMessageCards({} as GenomaConfig)).toBe(true);
  });
  ...
  it("checks per-channel capabilities", () => {
    ...
    expect(
      supportsChannelMessageButtonsForChannel({ cfg: {} as GenomaConfig, channel: "discord" }),
    ).toBe(true);
    expect(
      supportsChannelMessageButtonsForChannel({ cfg: {} as GenomaConfig, channel: "telegram" }),
    ).toBe(false);
    expect(
      supportsChannelMessageCardsForChannel({ cfg: {} as GenomaConfig, channel: "telegram" }),
    ).toBe(true);
    expect(supportsChannelMessageCardsForChannel({ cfg: {} as GenomaConfig })).toBe(false);
  });
});
