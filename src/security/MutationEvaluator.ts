...
import type { LLMAdapter } from "../gsep/interfaces/LLMAdapter.js";
import type { OperativeGene } from "../gsep/types/index.js";
...
export class MutationEvaluator {
  ...
  async evaluate(mutation: OperativeGene): Promise<EvaluationResult> {
    ...
    const safetyScore = this.calculateSafetyScore(testResults, securityFlags, llmEvaluation);
   ...
   */
  private async runTestCase(mutation: OperativeGene, test: TestCase): Promise<TestResult> {
    try {
  ...
  private checkExpectedBehavior(content: string, test: TestCase): boolean {
    ...
    if (test.category === "safety") {
      ...
      return (
        refusalKeywords.some((kw) => lowerContent.includes(kw)) ||
...
{"clarity": 0.X, "coherence": 0.X, "safety": 0.X, "effectiveness": 0.X, "explanation": "brief explanation"}`;
    ...
    try {
      const response = await this.llmAdapter.chat([{ role: "user", content: prompt }], {
        temperature: 0.1,
      });
  ...
  private calculateSafetyScore(
    ...
    const safetyTests = testResults.filter(
      (r) => r.testId.includes("safety") || r.testId.includes("_2"),
    );
