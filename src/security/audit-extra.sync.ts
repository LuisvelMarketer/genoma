...
function summarizeGroupPolicy(cfg: GenomaConfig): {
  open: number;
  allowlist: number;
  other: number;
} {
...
function isGatewayRemotelyExposed(cfg: GenomaConfig): boolean {
...
function collectModels(cfg: GenomaConfig): ModelRef[] {
  const out: ModelRef[] = [];
  addModel(
    out,
...
function listKnownNodeCommands(cfg: GenomaConfig): Set<string> {
  const baseCfg: GenomaConfig = {
    ...cfg,
    gateway: {
      ...cfg.gateway,
      nodes: {
...
function hasWebSearchKey(cfg: GenomaConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebSearchEnabled(cfg: GenomaConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebFetchEnabled(cfg: GenomaConfig): boolean {
...
function isBrowserEnabled(cfg: GenomaConfig): boolean {
  try {
...
function listGroupPolicyOpen(cfg: GenomaConfig): string[] {
  const out: string[] = [];
  const channels = cfg.channels as Record<string, unknown> | undefined;
  if (!channels || typeof channels !== "object") {
    return out;
...
function listPotentialMultiUserSignals(cfg: GenomaConfig): string[] {
  const out = new Set<string>();
  const channels = cfg.channels as Record<string, unknown> | undefined;
  if (!channels || typeof channels !== "object") {
    return [];
...
function collectRiskyToolExposureContexts(cfg: GenomaConfig): {
...
export function collectAttackSurfaceSummaryFindings(cfg: GenomaConfig): SecurityAuditFinding[] {
...
export function collectSyncedFolderFindings(params: {
  ...
  if (isProbablySyncedPath(params.stateDir) || isProbablySyncedPath(params.configPath)) {
    findings.push({
      ...
      remediation: `Keep GENOMA_STATE_DIR on a local-only volume and re-run "${formatCliCommand("genoma security audit --fix")}".`,
    });
  }
  return findings;
}

export function collectSecretsInConfigFindings(cfg: GenomaConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const password =
  ...
  if (password && !looksLikeEnvRef(password)) {
    findings.push({
      ...
      remediation:
        "Prefer GENOMA_GATEWAY_PASSWORD (env) and remove gateway.auth.password from disk.",
    });
  }
...
export function collectHooksHardeningFindings(
  ...
  const genomaGatewayToken =
    typeof env.GENOMA_GATEWAY_TOKEN === "string" && env.GENOMA_GATEWAY_TOKEN.trim()
...
export function collectGatewayHttpSessionKeyOverrideFindings(
  ...
  findings.push({
    ...
    detail:
      `${enabledEndpoints.join(", ")} accept x-genoma-session-key for per-request session routing. ` +
...
export function collectSandboxDockerNoopFindings(cfg: GenomaConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectSandboxDangerousConfigFindings(cfg: GenomaConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const agents = Array.isArray(cfg.agents?.list) ? cfg.agents.list : [];
  ...
  if (browserExposurePaths.length > 0) {
    findings.push({
      ...
      remediation:
        "Set sandbox.browser.network to a dedicated bridge network (recommended default: genoma-sandbox-browser), " +
...
export function collectNodeDenyCommandPatternFindings(cfg: GenomaConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectMinimalProfileOverrideFindings(cfg: GenomaConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectModelHygieneFindings(cfg: GenomaConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectExposureMatrixFindings(cfg: GenomaConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectLikelyMultiUserSetupFindings(cfg: GenomaConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  ...
  findings.push({
    ...
    detail:
      ...
      `\n${impactLine}\n${riskyContextsDetail}\n` +
      "Genoma's default security model is personal-assistant (one trusted operator boundary), not hostile multi-tenant isolation on one shared gateway.",
    remediation:
