...
describe("memory index", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "genoma-mem-fixtures-"));
  ...
  beforeEach(async () => {
    ...
    vi.stubEnv("GENOMA_TEST_MEMORY_UNSAFE_REINDEX", "1");
    embedBatchCalls = 0;
  ...
  it("reindexes sessions when source config adds sessions to an existing index", async () => {
    ...
    try {
      ...
      if (previousStateDir === undefined) {
        ...
        process.env.GENOMA_STATE_DIR = previousStateDir;
      }
