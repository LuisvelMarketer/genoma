...
const MISSING_EXTENSIONS_ERROR =
  'package.json missing genoma.extensions; update the plugin package to include genoma.extensions (for example ["./dist/index.js"]). See https://docs.genoma.ai/help/troubleshooting#plugin-install-fails-with-missing-genoma-extensions';

export const PLUGIN_INSTALL_ERROR_CODE = {
  INVALID_NPM_SPEC: "invalid_npm_spec",
  MISSING_GENOMA_EXTENSIONS: "missing_genoma_extensions",
  EMPTY_GENOMA_EXTENSIONS: "empty_genoma_extensions",
...
function ensureGenomaExtensions(params: { manifest: PackageManifest }):
  | {
      ok: true;
      entries: string[];
    }
  ...
  if (resolved.status === "missing") {
    return {
      ...
      code: PLUGIN_INSTALL_ERROR_CODE.MISSING_GENOMA_EXTENSIONS,
    };
  }
  if (resolved.status === "empty") {
    return {
      ok: false,
      error: "package.json genoma.extensions is empty",
      code: PLUGIN_INSTALL_ERROR_CODE.EMPTY_GENOMA_EXTENSIONS,
    };
  }
  return {
    ok: true,
...
async function installPluginFromPackageDir(params: {
  ...
  const extensionsResult = ensureGenomaExtensions({
    manifest,
  });
  ...
  // Prefer the canonical `id` from genoma.plugin.json over the npm package name.
  // This avoids a latent key-mismatch bug: if the manifest id (e.g. "memory-cognee")
  // differs from the npm package name (e.g. "cognee-genoma"), the plugin registry
  ...
  try {
    ...
    if (scanSummary.critical > 0) {
      ...
      logger.warn?.(
        `Plugin "${pluginId}" has ${scanSummary.warn} suspicious code pattern(s). Run "genoma security audit --deep" for details.`,
      );
    }
  } catch (err) {
    logger.warn?.(
      `Plugin "${pluginId}" code safety scan failed (${String(err)}). Installation continues; run "genoma security audit --deep" after install.`,
    );
  }
