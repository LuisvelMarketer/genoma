...
function ensureSuiteTempRoot() {
  ...
  suiteTempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "genoma-plugin-install-"));
  return suiteTempRoot;
}
...
async function expectArchiveInstallReservedSegmentRejection(params: {
  ...
  const result = await installArchivePackageAndReturnResult({
    packageJson: {
      ...
      version: "0.0.1",
      genoma: { extensions: ["./dist/index.js"] },
    },
    outName: params.outName,
    withDistIndex: true,
  });
...
beforeAll(async () => {
  ...
  fs.writeFileSync(
    path.join(manifestInstallTemplateDir, "genoma.plugin.json"),
    JSON.stringify({
...
describe("installPluginFromArchive", () => {
  it("installs into ~/.genoma/extensions and uses unscoped id", async () => {
  ...
  it("rejects packages without genoma.extensions", async () => {
    const result = await installArchivePackageAndReturnResult({
      packageJson: { name: "@genoma/nope", version: "0.0.1" },
      outName: "bad.tgz",
    });
    expect(result.ok).toBe(false);
    if (result.ok) {
    ...
    expect(result.error).toContain("genoma.extensions");
    expect(result.code).toBe(PLUGIN_INSTALL_ERROR_CODE.MISSING_GENOMA_EXTENSIONS);
  });

  it("rejects legacy plugin package shape when genoma.extensions is missing", async () => {
    const { pluginDir, extensionsDir } = setupPluginInstallDirs();
    fs.writeFileSync(
      path.join(pluginDir, "package.json"),
      JSON.stringify({
        name: "@genoma/legacy-entry-fallback",
        version: "0.0.1",
      }),
      "utf-8",
    );
    ...
    expect(result.ok).toBe(false);
    if (!result.ok) {
      expect(result.error).toContain("package.json missing genoma.extensions");
      expect(result.error).toContain("update the plugin package");
      expect(result.code).toBe(PLUGIN_INSTALL_ERROR_CODE.MISSING_GENOMA_EXTENSIONS);
      return;
    }
    expect.unreachable("expected install to fail without genoma.extensions");
  });
...
describe("installPluginFromDir", () => {
  ...
  it("strips workspace devDependencies before npm install", async () => {
    ...
    expect(manifest.devDependencies?.genoma).toBeUndefined();
    expect(manifest.devDependencies?.vitest).toBe("^3.0.0");
  });

  it("uses genoma.plugin.json id as install key when it differs from package name", async () => {
    ...
    expect(
      infoMessages.some((msg) =>
        msg.includes(
          'Plugin manifest id "memory-cognee" differs from npm package name "cognee-genoma"',
        ),
      ),
    ).toBe(true);
  });
...
describe("installPluginFromNpmSpec", () => {
  it("uses --ignore-scripts for npm pack and cleans up temp dir", async () => {
      ...
      if (argv[0] === "npm" && argv[1] === "pack") {
        ...
        return {
          ...
          stdout: JSON.stringify([
            {
              id: "@genoma/voice-call@0.0.1",
              name: "@genoma/voice-call",
              version: "0.0.1",
              filename: packedName,
    ...
    expect(result.npmResolution?.resolvedSpec).toBe("@genoma/voice-call@0.0.1");
    expect(result.npmResolution?.integrity).toBe("sha512-plugin-test");

    expectSingleNpmPackIgnoreScriptsCall({
      calls: run.mock.calls,
      expectedSpec: "@genoma/voice-call@0.0.1",
    });
