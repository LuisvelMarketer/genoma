...
function makeTempDir() {
  const dir = path.join(os.tmpdir(), `genoma-manifest-registry-${randomUUID()}`);
...
function writeManifest(dir: string, manifest: Record<string, unknown>) {
  fs.writeFileSync(path.join(dir, "genoma.plugin.json"), JSON.stringify(manifest), "utf-8");
}
...
describe("loadPluginManifestRegistry", () => {
  ...
  it("rejects manifest paths that escape plugin root via symlink", () => {
    ...
    const outsideManifest = path.join(outsideDir, "genoma.plugin.json");
    const linkedManifest = path.join(rootDir, "genoma.plugin.json");
