...
function createSinglePluginWithEmptySlotsConfig(): GenomaConfig {
  return {
    plugins: {
...
function createSingleNpmInstallConfig(installPath: string): GenomaConfig {
  return {
    plugins: {
      entries: createSinglePluginEntries(),
      installs: {
...
describe("resolveUninstallDirectoryTarget", () => {
  ...
  it("falls back to default path when configured installPath is untrusted", () => {
    const extensionsDir = path.join(os.tmpdir(), "genoma-uninstall-safe");
    const target = resolveUninstallDirectoryTarget({
      ...
      installRecord: {
        source: "npm",
        spec: "my-plugin@1.0.0",
        installPath: "/tmp/not-genoma-extensions/my-plugin",
      },
      extensionsDir,
    });
