...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: GenomaPluginDefinition;
  register?: GenomaPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as GenomaPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as GenomaPluginDefinition;
...
export function loadGenomaPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverGenomaPlugins({
    workspaceDir: options.workspaceDir,
  ...
  const getJiti = () => {
    ...
    jitiLoader = createJiti(import.meta.url, {
      ...
      ...(pluginSdkAlias || pluginSdkAccountIdAlias
        ? {
            alias: {
              ...(pluginSdkAlias ? { "genoma/plugin-sdk": pluginSdkAlias } : {}),
              ...(pluginSdkAccountIdAlias
                ? { "genoma/plugin-sdk/account-id": pluginSdkAccountIdAlias }
                : {}),
            },
          }
        : {}),
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: GenomaPluginModule | null = null;
    try {
      mod = getJiti()(safeSource) as GenomaPluginModule;
    } catch (err) {
      recordPluginError({
        logger,
        registry,
