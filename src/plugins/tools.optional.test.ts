...
const loadGenomaPluginsMock = vi.fn();

vi.mock("./loader.js", () => ({
  loadGenomaPlugins: (params: unknown) => loadGenomaPluginsMock(params),
}));
...
function setRegistry(entries: MockRegistryToolEntry[]) {
  ...
  loadGenomaPluginsMock.mockReturnValue(registry);
  return registry;
}
...
describe("resolvePluginTools optional tools", () => {
  beforeEach(() => {
    loadGenomaPluginsMock.mockClear();
  });
