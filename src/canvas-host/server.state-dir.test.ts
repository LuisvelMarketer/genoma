...
describe("canvas host state dir defaults", () => {
  it("uses GENOMA_STATE_DIR for the default canvas root", async () => {
    await withStateDirEnv("genoma-canvas-state-", async ({ stateDir }) => {
      ...
      try {
        ...
        expect(indexContents).toContain("Genoma Canvas");
      } finally {
        await handler.close();
      }
    });
