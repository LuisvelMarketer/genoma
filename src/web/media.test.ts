...
beforeAll(async () => {
  fixtureRoot = await fs.mkdtemp(
    path.join(resolvePreferredGenomaTmpDir(), "genoma-media-test-"),
  );
...
describe("web media loading", () => {
  beforeAll(() => {
    // Ensure state dir is stable and not influenced by other tests that stub GENOMA_STATE_DIR.
    // Also keep it outside the Genoma temp root so default localRoots doesn't accidentally make all state readable.
    stateDirSnapshot = captureEnv(["GENOMA_STATE_DIR"]);
    process.env.GENOMA_STATE_DIR = path.join(
      path.parse(os.tmpdir()).root,
      "var",
      "lib",
      "genoma-media-state-test",
    );
  });
...
describe("local media root guard", () => {
  ...
  it("allows local paths under an explicit root", async () => {
    const result = await loadWebMedia(tinyPngFile, 1024 * 1024, {
      localRoots: [resolvePreferredGenomaTmpDir()],
    });
    expect(result.kind).toBe("image");
  });
  ...
  it("allows default Genoma state workspace and sandbox roots", async () => {
  ...
  it("rejects default Genoma state per-agent workspace-* roots without explicit local roots", async () => {
    const stateDir = resolveStateDir();
    const readFile = vi.fn(async () => Buffer.from("generated-media"));
