...
describe("runCommandWithTimeout", () => {
  ...
  it("merges custom env with base env and drops undefined values", async () => {
    const resolved = resolveCommandEnv({
      argv: ["node", "script.js"],
      baseEnv: {
        GENOMA_BASE_ENV: "base",
        GENOMA_TO_REMOVE: undefined,
      },
      env: {
        GENOMA_TEST_ENV: "ok",
      },
    ...
    expect(resolved.GENOMA_BASE_ENV).toBe("base");
    expect(resolved.GENOMA_TEST_ENV).toBe("ok");
    expect(resolved.GENOMA_TO_REMOVE).toBeUndefined();
  });
