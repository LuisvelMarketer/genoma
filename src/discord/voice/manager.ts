...
function resolveVoiceTtsConfig(params: { cfg: GenomaConfig; override?: TtsConfig }): {
  cfg: GenomaConfig;
...
async function writeWavFile(pcm: Buffer): Promise<{ path: string; durationSeconds: number }> {
  const tempDir = await fs.mkdtemp(path.join(resolvePreferredGenomaTmpDir(), "discord-voice-"));
