  ...
  return {
    agents: {
      defaults: {
        ...
        workspace: "/tmp/genoma",
      },
    },
    session: { store: "/tmp/genoma-sessions.json" },
...
describe("discord tool result dispatch", () => {
  it(
    ...
    async () => {
      const cfg = createMentionRequiredGuildConfig({
        messages: {
          responsePrefix: "PFX",
          groupChat: { mentionPatterns: ["\\bgenoma\\b"] },
        },
      });
      ...
      await handler(
        createGuildMessageEvent({ messageId: "m2", content: "genoma: hello" }),
        client,
      );
      ...
      createGuildMessageEvent({
        ...
        messagePatch: {
          referencedMessage: {
            ...
            mentionedUsers: [],
            mentionedRoles: [],
            author: { id: "bot-id", bot: true, username: "Genoma" },
          },
        },
        eventPatch: {
  ...
  it("treats forum threads as distinct sessions without channel payloads", async () => {
    ...
    const cfg = {
      agent: { model: "anthropic/claude-opus-4-5", workspace: "/tmp/genoma" },
      session: { store: "/tmp/genoma-sessions.json" },
      channels: {
        discord: {
          dm: { enabled: true, policy: "open" },
