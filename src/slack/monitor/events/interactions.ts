...
// Prefix for Genoma-generated action IDs to scope our handler
const GENOMA_ACTION_PREFIX = "genoma:";
...
export function registerSlackInteractionEvents(params: { ctx: SlackMonitorContext }) {
  ...
  const modalMatcher = new RegExp(`^${GENOMA_ACTION_PREFIX}`);

  // Handle Genoma modal submissions with callback_ids scoped by our prefix.
  registerModalLifecycleHandler({
