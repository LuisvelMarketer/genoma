...
export const DEFAULT_LOG_DIR = resolvePreferredGenomaTmpDir();
export const DEFAULT_LOG_FILE = path.join(DEFAULT_LOG_DIR, "genoma.log"); // legacy single-file path

const LOG_PREFIX = "genoma";
...
function canUseSilentVitestFileLogFastPath(envLevel: LogLevel | undefined): boolean {
  return (
    ...
    process.env.GENOMA_TEST_FILE_LOG !== "1" &&
    !envLevel &&
...
function resolveSettings(): ResolvedSettings {
  ...
  let cfg: GenomaConfig["logging"] | undefined =
    (loggingState.overrideSettings as LoggerSettings | null) ?? readLoggingConfig();
  if (!cfg) {
    try {
      const loaded = requireConfig?.("../config/config.js") as
        | {
            loadConfig?: () => GenomaConfig;
          }
        | undefined;
  ...
  const defaultLevel =
    process.env.VITEST === "true" && process.env.GENOMA_TEST_FILE_LOG !== "1" ? "silent" : "info";
      ...
      if (nextBytes > settings.maxFileBytes) {
        if (!warnedAboutSizeCap) {
          ...
          process.stderr.write(
            `[genoma] log file size cap reached; suppressing writes file=${settings.file} maxFileBytes=${settings.maxFileBytes}\n`,
          );
        }
        return;
      }
