...
let originalGenomaLogLevel: string | undefined;
...
beforeEach(() => {
  originalTestFileLog = process.env.GENOMA_TEST_FILE_LOG;
  originalGenomaLogLevel = process.env.GENOMA_LOG_LEVEL;
  delete process.env.GENOMA_TEST_FILE_LOG;
  delete process.env.GENOMA_LOG_LEVEL;
...
afterEach(() => {
  if (originalTestFileLog === undefined) {
    ...
    process.env.GENOMA_TEST_FILE_LOG = originalTestFileLog;
  }
  if (originalGenomaLogLevel === undefined) {
    delete process.env.GENOMA_LOG_LEVEL;
  } else {
    process.env.GENOMA_LOG_LEVEL = originalGenomaLogLevel;
  }
  logging.resetLogger();
  logging.setLoggerOverride(null);
  vi.restoreAllMocks();
