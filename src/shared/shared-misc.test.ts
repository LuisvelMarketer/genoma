...
describe("shared/frontmatter", () => {
  ...
  test("resolveGenomaManifestBlock parses JSON5 metadata and picks genoma block", () => {
    const frontmatter = {
      metadata: "{ genoma: { foo: 1, bar: 'baz' } }",
    };
    expect(resolveGenomaManifestBlock({ frontmatter })).toEqual({ foo: 1, bar: "baz" });
  });

  test("resolveGenomaManifestBlock returns undefined for invalid input", () => {
    expect(resolveGenomaManifestBlock({ frontmatter: {} })).toBeUndefined();
    expect(
      resolveGenomaManifestBlock({ frontmatter: { metadata: "not-json5" } }),
    ).toBeUndefined();
    expect(
      resolveGenomaManifestBlock({ frontmatter: { metadata: "{ nope: { a: 1 } }" } }),
    ).toBeUndefined();
  });
});
