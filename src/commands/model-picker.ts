...
type PromptDefaultModelResult = { model?: string; config?: GenomaConfig };
...
function resolveConfiguredModelRaw(cfg: GenomaConfig): string {
  return resolveAgentModelPrimaryValue(cfg.agents?.defaults?.model) ?? "";
}

function resolveConfiguredModelKeys(cfg: GenomaConfig): string[] {
...
export function applyPrimaryModel(cfg: GenomaConfig, model: string): GenomaConfig {
...
export function applyModelAllowlist(cfg: GenomaConfig, models: string[]): GenomaConfig {
  const defaults = cfg.agents?.defaults;
