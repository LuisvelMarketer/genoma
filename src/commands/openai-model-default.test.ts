...
function expectPrimaryModelChanged(
  applied: { changed: boolean; next: GenomaConfig },
  primary: string,
) {
...
type SharedDefaultModelCase = {
  apply: (cfg: GenomaConfig) => { changed: boolean; next: GenomaConfig };
  defaultModel: string;
  overrideConfig: GenomaConfig;
  alreadyDefaultConfig: GenomaConfig;
};
...
describe("applyDefaultModelChoice", () => {
  it("ensures allowlist entry exists when returning an agent override", async () => {
    ...
    const applied = await applyDefaultModelChoice({
      ...
      applyProviderConfig: (config: GenomaConfig) => config,
      applyDefaultConfig: (config: GenomaConfig) => config,
      noteAgentModel,
...
describe("shared default model behavior", () => {
  it("sets defaults when model is unset", () => {
    for (const testCase of SHARED_DEFAULT_MODEL_CASES) {
      const cfg: GenomaConfig = { agents: { defaults: {} } };
