...
describe("onboard (non-interactive): gateway and remote auth", () => {
  ...
  beforeAll(async () => {
    ...
    tempHome = await makeTempWorkspace("genoma-onboard-");
  ...
  it("writes gateway token auth into config", async () => {
    await withStateDir("state-noninteractive-", async (stateDir) => {
      ...
      const workspace = path.join(stateDir, "genoma");
  ...
  it("uses GENOMA_GATEWAY_TOKEN when --gateway-token is omitted", async () => {
    await withStateDir("state-env-token-", async (stateDir) => {
      ...
      process.env.GENOMA_GATEWAY_TOKEN = envToken;
  ...
  it("auto-generates token auth when binding LAN and persists the token", async () => {
    ...
    await withStateDir("state-lan-", async (stateDir) => {
      process.env.GENOMA_STATE_DIR = stateDir;
      process.env.GENOMA_CONFIG_PATH = path.join(stateDir, "genoma.json");
