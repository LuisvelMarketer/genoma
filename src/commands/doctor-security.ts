...
import type { GenomaConfig, GatewayBindMode } from "../config/config.js";
...
export async function noteSecurityWarnings(cfg: GenomaConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("genoma security audit --deep")}`;

  if (cfg.approvals?.exec?.enabled === false) {
    warnings.push(
      "- Note: approvals.exec.enabled=false disables approval forwarding only.",
      "  Host exec gating still comes from ~/.genoma/exec-approvals.json.",
      `  Check local policy with: ${formatCliCommand("genoma approvals get --gateway")}`,
    );
  }
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("genoma configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("genoma config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("genoma doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("genoma config set gateway.bind loopback")}`,
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('genoma config set session.dmScope "per-channel-peer"') +
