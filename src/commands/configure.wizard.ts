...
async function runGatewayHealthCheck(params: {
  ...
  const token = params.cfg.gateway?.auth?.token ?? process.env.GENOMA_GATEWAY_TOKEN;
  const password = params.cfg.gateway?.auth?.password ?? process.env.GENOMA_GATEWAY_PASSWORD;
  ...
  try {
    ...
    note(
      [
        "Docs:",
        "https://docs.genoma.ai/gateway/health",
        "https://docs.genoma.ai/gateway/troubleshooting",
      ].join("\n"),
      "Health check help",
    );
  }
    ...
    await select({
      ...
      options: [
        ...
        },
        {
          ...
          hint: "Delete channel tokens/settings from genoma.json",
        },
      ],
...
async function promptWebToolsConfig(
  ...
  note(
    [
      ...
      "Docs: https://docs.genoma.ai/tools/web",
    ].join("\n"),
    "Web search",
  );
...
export async function runConfigureWizard(
  ...
  try {
    ...
    intro(opts.command === "update" ? "Genoma update wizard" : "Genoma configure");
    const prompter = createClackPrompter();

    const snapshot = await readConfigFileSnapshot();
    const baseConfig: GenomaConfig = snapshot.valid ? snapshot.config : {};
    ...
    if (snapshot.exists) {
      ...
      if (!snapshot.valid && snapshot.issues.length > 0) {
        note(
          [
            ...
            "",
            "Docs: https://docs.genoma.ai/gateway/configuration",
          ].join("\n"),
          "Config issues",
        );
      }
      if (!snapshot.valid) {
        outro(
          `Config invalid. Run \`${formatCliCommand("genoma doctor")}\` to repair it, then re-run configure.`,
        );
        runtime.exit(1);
        return;
      }
    ...
    const localProbe = await probeGatewayReachable({
      url: localUrl,
      token: baseConfig.gateway?.auth?.token ?? process.env.GENOMA_GATEWAY_TOKEN,
      password: baseConfig.gateway?.auth?.password ?? process.env.GENOMA_GATEWAY_PASSWORD,
    });
    ...
    const newPassword = nextConfig.gateway?.auth?.password ?? process.env.GENOMA_GATEWAY_PASSWORD;
    const oldPassword = baseConfig.gateway?.auth?.password ?? process.env.GENOMA_GATEWAY_PASSWORD;
    const token = nextConfig.gateway?.auth?.token ?? process.env.GENOMA_GATEWAY_TOKEN;
    ...
    note(
      [
        ...
        gatewayStatusLine,
        "Docs: https://docs.genoma.ai/web/control-ui",
      ].join("\n"),
      "Control UI",
    );
