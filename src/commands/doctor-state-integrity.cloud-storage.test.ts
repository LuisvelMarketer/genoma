...
describe("detectMacCloudSyncedStateDir", () => {
  ...
  it("detects state dir under Library/CloudStorage", () => {
    const stateDir = path.join(home, "Library", "CloudStorage", "Dropbox", "Genoma", ".genoma");
  ...
  it("detects cloud-synced target when state dir resolves via symlink", () => {
    const symlinkPath = "/tmp/genoma-state";
  ...
  it("ignores cloud-synced symlink prefix when resolved target is local", () => {
    ...
    const resolvedLocalPath = path.join(home, ".genoma");
  ...
  it("anchors cloud detection to OS homedir when GENOMA_HOME is overridden", () => {
    const stateDir = path.join(home, "Library", "CloudStorage", "iCloud Drive", ".genoma");
    const originalGenomaHome = process.env.GENOMA_HOME;
    process.env.GENOMA_HOME = "/tmp/genoma-home-override";
    ...
    try {
      ...
      if (originalGenomaHome === undefined) {
        delete process.env.GENOMA_HOME;
      } else {
        process.env.GENOMA_HOME = originalGenomaHome;
      }
    }
  });
