...
describe("normalizeCompatibilityConfigValues", () => {
  ...
  beforeEach(() => {
    previousOauthDir = process.env.GENOMA_OAUTH_DIR;
    tempOauthDir = fs.mkdtempSync(path.join(os.tmpdir(), "genoma-oauth-"));
    process.env.GENOMA_OAUTH_DIR = tempOauthDir;
  });
  ...
  afterEach(() => {
    if (previousOauthDir === undefined) {
      ...
      process.env.GENOMA_OAUTH_DIR = previousOauthDir;
    }
    if (tempOauthDir) {
  ...
  it("copies legacy ack reaction when authDir override exists", () => {
    const customDir = fs.mkdtempSync(path.join(os.tmpdir(), "genoma-wa-auth-"));
    try {
