...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveGenomaAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("genoma models auth setup-token")}\` or \`${formatCliCommand("genoma configure")}\`.`
          : `Run \`${formatCliCommand("genoma configure")}\` or set an API key env var.`;
