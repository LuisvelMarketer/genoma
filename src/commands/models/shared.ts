...
export async function loadValidConfigOrThrow(): Promise<GenomaConfig> {
...
export async function updateConfig(
  mutator: (cfg: GenomaConfig) => GenomaConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: GenomaConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: GenomaConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("genoma agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
