...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveGenomaAgentDir: vi.fn().mockReturnValue("/tmp/genoma-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/genoma-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/genoma-agent/auth-profiles.json"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveGenomaAgentDir: mocks.resolveGenomaAgentDir,
}));
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/genoma-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveGenomaAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.auth.storePath).toBe("/tmp/genoma-agent/auth-profiles.json");
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/genoma-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/genoma-agent-custom");
