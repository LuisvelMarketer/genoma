...
beforeAll(() => {
  envSnapshot = captureEnv(["GENOMA_PROFILE"]);
  process.env.GENOMA_PROFILE = "isolated";
});
...
vi.mock("../infra/genoma-root.js", () => ({
  resolveGenomaPackageRoot: vi.fn().mockResolvedValue("/tmp/genoma"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/genoma/pnpm-lock.yaml",
      markerPath: "/tmp/genoma/node_modules/.modules.yaml",
    },
...
vi.mock("../daemon/service.js", () => ({
  resolveGatewayService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.genoma.gateway.plist",
    }),
  }),
}));
vi.mock("../daemon/node-service.js", () => ({
  resolveNodeService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.genoma.node.plist",
    }),
  }),
}));
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    expect(
      logs.some(
        (line) =>
          line.includes("genoma status --all") ||
          line.includes("genoma --profile isolated status --all"),
      ),
    ).toBe(true);
  });

  it("shows gateway auth when reachable", async () => {
    await withEnvVar("GENOMA_GATEWAY_TOKEN", "abcd1234", async () => {
