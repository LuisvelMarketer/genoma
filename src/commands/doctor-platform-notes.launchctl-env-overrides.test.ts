...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "GENOMA_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("GENOMA_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv GENOMA_GATEWAY_TOKEN");
    expect(message).not.toContain("GENOMA_GATEWAY_PASSWORD");
  });
