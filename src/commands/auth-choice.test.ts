...
describe("applyAuthChoice", () => {
  ...
  async function setupTempState() {
    ...
    const env = await setupAuthTestEnv("genoma-auth-");
      ...
      config: {
        secrets: {
          providers: {
            filemain: {
              source: "file",
              path: "/tmp/genoma-missing-secrets.json",
              mode: "json",
            },
          },
        },
  ...
  it("ignores legacy LiteLLM oauth profiles when selecting litellm-api-key", async () => {
    ...
    const authProfilePath = authProfilePathForAgent(requireGenomaAgentDir());
    await fs.writeFile(
      authProfilePath,
      JSON.stringify(
        {
