  ...
  while (true) {
    ...
    if (source === "env") {
      ...
      await params.prompter.note(
        `Validated environment variable ${envVar}. Genoma will store a reference, not the key value.`,
        "Reference validated",
      );
    ...
    try {
      ...
      await params.prompter.note(
        `Validated ${providerEntry.source} reference ${selectedProvider}:${id}. Genoma will store a reference, not the key value.`,
        "Reference validated",
      );
      return { ref, resolvedValue };
    } catch (error) {
    ...
    options: [
      {
        ...
        hint: "Stores the key directly in Genoma config",
      },
      {
        value: "ref",
