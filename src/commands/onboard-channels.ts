...
async function noteChannelPrimer(
  ...
  await prompter.note(
    [
      ...
      `Approve with: ${formatCliCommand("genoma pairing approve <channel> <code>")}`,
  ...
  const selectPolicy = async (policy: ChannelOnboardingDmPolicy) => {
    await prompter.note(
      [
        ...
        `Approve: ${formatCliCommand(`genoma pairing approve ${policy.channel} <code>`)}`,
      ...
      if (adapter) {
        await prompter.note(
          `${channel} plugin not available (continuing with onboarding). If the channel still doesn't work after setup, run \`${formatCliCommand(
            "genoma plugins list",
          )}\` and \`${formatCliCommand("genoma plugins enable " + channel)}\`, then restart the gateway.`,
          "Channel setup",
        );
    ...
    const choice = (await prompter.select({
      ...
      options: [
        ...buildSelectionOptions(entries),
        {
          value: "__skip__",
          label: "Skip for now",
          hint: `You can add channels later via \`${formatCliCommand("genoma channels add")}\``,
        },
      ],
