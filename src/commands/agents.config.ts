...
function resolveAgentName(cfg: GenomaConfig, agentId: string) {
...
function resolveAgentModel(cfg: GenomaConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: GenomaConfig): AgentSummary[] {
