...
export function normalizeCompatibilityConfigValues(cfg: GenomaConfig): {
  config: GenomaConfig;
  changes: string[];
} {
  const changes: string[] = [];
  let next: GenomaConfig = cfg;
  ...
  const seedMissingDefaultAccountsFromSingleAccountBase = () => {
    ...
    next = {
      ...next,
      channels: nextChannels as GenomaConfig["channels"],
    };
  };
  ...
  const normalizeBrowserSsrFPolicyAlias = () => {
    ...
    next = {
      ...next,
      browser: migratedBrowser as GenomaConfig["browser"],
    };
    changes.push(
