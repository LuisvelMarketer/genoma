...
type EnvSnapshot = {
  HOME?: string;
  GENOMA_HOME?: string;
  GENOMA_STATE_DIR?: string;
  GENOMA_OAUTH_DIR?: string;
};

function captureEnv(): EnvSnapshot {
  return {
    HOME: process.env.HOME,
    GENOMA_HOME: process.env.GENOMA_HOME,
    GENOMA_STATE_DIR: process.env.GENOMA_STATE_DIR,
    GENOMA_OAUTH_DIR: process.env.GENOMA_OAUTH_DIR,
  };
}
...
function setupSessionState(cfg: GenomaConfig, env: NodeJS.ProcessEnv, homeDir: string) {
  const agentId = "main";
...
async function runStateIntegrity(cfg: GenomaConfig) {
...
async function runStateIntegrityText(cfg: GenomaConfig): Promise<string> {
...
describe("doctor state integrity oauth dir checks", () => {
  ...
  beforeEach(() => {
    ...
    tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "genoma-doctor-state-integrity-"));
    process.env.HOME = tempHome;
    process.env.GENOMA_HOME = tempHome;
    process.env.GENOMA_STATE_DIR = path.join(tempHome, ".genoma");
    delete process.env.GENOMA_OAUTH_DIR;
    fs.mkdirSync(process.env.GENOMA_STATE_DIR, { recursive: true, mode: 0o700 });
  ...
  it("prompts for oauth dir when GENOMA_OAUTH_DIR is explicitly configured", async () => {
    process.env.GENOMA_OAUTH_DIR = path.join(tempHome, ".oauth");
    const cfg: GenomaConfig = {};
    const confirmSkipInNonInteractive = await runStateIntegrity(cfg);
    expect(confirmSkipInNonInteractive).toHaveBeenCalledWith(OAUTH_PROMPT_MATCHER);
    expect(stateIntegrityText()).toContain("CRITICAL: OAuth dir missing");
  ...
  it("prints genoma-only verification hints when recent sessions are missing transcripts", async () => {
    const cfg: GenomaConfig = {};
    ...
    expect(text).toMatch(/genoma sessions --store ".*sessions\.json"/);
    expect(text).toMatch(/genoma sessions cleanup --store ".*sessions\.json" --dry-run/);
    expect(text).toMatch(
      /genoma sessions cleanup --store ".*sessions\.json" --enforce --fix-missing/,
    );
