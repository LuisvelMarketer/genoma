...
const SEARCH_TOOL = "https://docs.genoma.ai/mcp.SearchGenoma";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.genoma.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('genoma docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.genoma.ai/");
      runtime.log(`Search: ${formatCliCommand('genoma docs "your query"')}`);
    }
    return;
  }
