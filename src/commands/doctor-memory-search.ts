 ...
 * Runs as part of `genoma doctor` — config-only, no network calls.
 */
  ...
  if (resolved.provider !== "auto") {
    if (resolved.provider === "local") {
        ...
        if (opts?.gatewayMemoryProbe?.checked && !opts.gatewayMemoryProbe.ready) {
          ...
          note(
            [
              ...
              "",
              `Verify: ${formatCliCommand("genoma memory status --deep")}`,
            ]
              .filter(Boolean)
              .join("\n"),
            "Memory search",
      ...
      note(
        [
          ...
          `- Switch to a remote provider: ${formatCliCommand("genoma config set agents.defaults.memorySearch.provider openai")}`,
          "",
          `Verify: ${formatCliCommand("genoma memory status --deep")}`,
        ].join("\n"),
        "Memory search",
    ...
    note(
      [
        ...
        `- Configure credentials: ${formatCliCommand("genoma configure --section model")}`,
        `- To disable: ${formatCliCommand("genoma config set agents.defaults.memorySearch.enabled false")}`,
        "",
        `Verify: ${formatCliCommand("genoma memory status --deep")}`,
      ].join("\n"),
      "Memory search",
