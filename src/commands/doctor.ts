...
function resolveMode(cfg: GenomaConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  const root = await resolveGenomaPackageRoot({
  ...
  let cfg: GenomaConfig = configResult.cfg;
  ...
  if (!cfg.gateway?.mode) {
    const lines = [
      ...
      `Fix: run ${formatCliCommand("genoma configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("genoma config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("genoma setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("genoma doctor --fix")}" to apply changes.`);
  }
