...
export const resolveGenomaPackageRoot = vi.fn().mockResolvedValue(null) as unknown as MockFn;
...
vi.mock("../plugins/loader.js", () => ({
  loadGenomaPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
beforeEach(() => {
  ...
  resolveGenomaPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.GENOMA_STATE_DIR;
  originalUpdateInProgress = process.env.GENOMA_UPDATE_IN_PROGRESS;
  process.env.GENOMA_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "genoma-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.GENOMA_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.GENOMA_UPDATE_IN_PROGRESS;
  } else {
    process.env.GENOMA_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
