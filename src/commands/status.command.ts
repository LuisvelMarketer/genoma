...
export async function statusCommand(
  ...
  runtime.log(theme.heading("Genoma status"));
  runtime.log("");
  ...
  if (pairingRecovery) {
    ...
    if (pairingRecovery.requestId) {
      runtime.log(
        theme.muted(
          `Recovery: ${formatCliCommand(`genoma devices approve ${pairingRecovery.requestId}`)}`,
        ),
      );
    }
    runtime.log(theme.muted(`Fallback: ${formatCliCommand("genoma devices approve --latest")}`));
    runtime.log(theme.muted(`Inspect: ${formatCliCommand("genoma devices list")}`));
  }
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("genoma security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("genoma security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.genoma.ai/faq");
  runtime.log("Troubleshooting: https://docs.genoma.ai/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("genoma status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("genoma logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("genoma status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("genoma gateway probe")}`);
  }
}
