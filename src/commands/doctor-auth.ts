...
function pruneAuthProfiles(
  cfg: GenomaConfig,
  profileIds: Set<string>,
): { next: GenomaConfig; changed: boolean } {
...
export async function maybeRemoveDeprecatedCliAuthProfiles(
  ...
  if (deprecated.has(CLAUDE_CLI_PROFILE_ID)) {
    lines.push(
      `- ${CLAUDE_CLI_PROFILE_ID} (Anthropic): use setup-token → ${formatCliCommand("genoma models auth setup-token")}`,
    );
  }
...
function formatAuthIssueHint(issue: AuthIssue): string | null {
  if (issue.provider === "anthropic" && issue.profileId === CLAUDE_CLI_PROFILE_ID) {
    return `Deprecated profile. Use ${formatCliCommand("genoma models auth setup-token")} or ${formatCliCommand(
      "genoma configure",
    )}.`;
  }
  if (issue.provider === "openai-codex" && issue.profileId === CODEX_CLI_PROFILE_ID) {
    return `Deprecated profile. Use ${formatCliCommand(
      "genoma models auth login --provider openai-codex",
    )} or ${formatCliCommand("genoma configure")}.`;
  }
  return `Re-auth via \`${formatCliCommand("genoma configure")}\` or \`${formatCliCommand("genoma onboard")}\`.`;
}
