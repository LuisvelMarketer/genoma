...
describe("writeOAuthCredentials", () => {
  ...
  it("writes auth-profiles.json under GENOMA_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("genoma-oauth-");
  ...
  it("writes OAuth credentials to all sibling agent dirs when syncSiblingAgents=true", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "genoma-oauth-sync-"));
    ...
    process.env.GENOMA_AGENT_DIR = kidAgentDir;
  ...
  it("writes OAuth credentials only to target dir by default", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "genoma-oauth-nosync-"));
    process.env.GENOMA_STATE_DIR = tempStateDir;
  ...
  it("syncs siblings from explicit agentDir outside GENOMA_STATE_DIR", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "genoma-oauth-external-"));
    process.env.GENOMA_STATE_DIR = tempStateDir;

    // Create standard-layout agents tree *outside* GENOMA_STATE_DIR
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to GENOMA_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("genoma-minimax-", { agentSubdir: "custom-agent" });
    lifecycle.setStateDir(env.stateDir);
