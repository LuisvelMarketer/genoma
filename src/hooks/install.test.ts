...
const fixtureRoot = path.join(os.tmpdir(), `genoma-hook-install-${randomUUID()}`);
let tempDirIndex = 0;
...
describe("installHooksFromPath", () => {
  it("uses --ignore-scripts for dependency install", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "@genoma/test-hooks",
        version: "0.0.1",
        genoma: { hooks: ["./hooks/one-hook"] },
    ...
    fs.writeFileSync(
      ...
      [
        ...
        'metadata: {"genoma":{"events":["command:new"]}}',
        "---",
        "",
        "# One Hook",
      ].join("\n"),
  ...
  it("rejects hook pack entries that traverse outside package directory", async () => {
    ...
    expect(result.error).toContain("genoma.hooks entry escapes package directory");
  });
  ...
  it("rejects hook pack entries that escape via symlink", async () => {
    ...
    expect(result.error).toContain("genoma.hooks entry resolves outside package directory");
  });
});
...
describe("installHooksFromNpmSpec", () => {
  it("uses --ignore-scripts for npm pack and cleans up temp dir", async () => {
    ...
    expect(result.npmResolution?.resolvedSpec).toBe("@genoma/test-hooks@0.0.1");
    ...
    expectSingleNpmPackIgnoreScriptsCall({
      ...
      expectedSpec: "@genoma/test-hooks@0.0.1",
    });
