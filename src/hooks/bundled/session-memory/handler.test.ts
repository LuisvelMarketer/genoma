...
beforeAll(async () => {
  ...
  suiteWorkspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "genoma-session-memory-"));
});
...
async function runNewWithPreviousSession(params: {
  ...
  cfg?: (tempDir: string) => GenomaConfig;
  action?: "new" | "reset";
...
function makeSessionMemoryConfig(tempDir: string, messages?: number): GenomaConfig {
  return {
    agents: { defaults: { workspace: tempDir } },
