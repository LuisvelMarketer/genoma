...
homepage: https://docs.genoma.ai/automation/hooks#command-logger
metadata:
  {
    "genoma":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.genoma/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.genoma/logs/commands.log ~/.genoma/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/genoma`:
   ```
   /home/username/.genoma/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
