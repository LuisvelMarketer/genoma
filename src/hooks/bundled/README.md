...
This directory contains hooks that ship with Genoma. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
Hooks can be configured in `~/.genoma/genoma.json`:
...
2. Restart gateway: `pkill -9 -f 'genoma.*gateway' && pnpm genoma gateway`
3. Enable the hook: `genoma hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.genoma.ai/automation/hooks
