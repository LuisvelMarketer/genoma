...
describe("resolveTelegramAccount", () => {
  ...
  it("formats debug logs with inspect-style output when debug env is enabled", () => {
    withEnv({ TELEGRAM_BOT_TOKEN: "", GENOMA_DEBUG_TELEGRAM_ACCOUNTS: "1" }, () => {
      const cfg: GenomaConfig = {
        channels: {
          telegram: { accounts: { work: { botToken: "tok-work" } } },
        },
...
describe("resolveTelegramAccount groups inheritance (#30673)", () => {
  const createMultiAccountGroupsConfig = (): GenomaConfig => ({
    channels: {
      telegram: {
  ...
  const createDefaultAccountGroupsConfig = (includeDevAccount: boolean): GenomaConfig => ({
    channels: {
      telegram: {
