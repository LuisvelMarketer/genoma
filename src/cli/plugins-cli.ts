...
function applySlotSelectionForPlugin(
  config: GenomaConfig,
  pluginId: string,
): { config: GenomaConfig; warnings: string[] } {
...
async function runPluginInstallCommand(params: {
  ...
  if (fs.existsSync(resolved)) {
    if (opts.link) {
      ...
      let next: GenomaConfig = enablePluginInConfig(
        {
          ...cfg,
          plugins: {
            ...cfg.plugins,
...
export function registerPluginsCli(program: Command) {
  const plugins = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/plugins", "docs.genoma.ai/cli/plugins")}\n`,
    );
  ...
  plugins
    ...
    .action(async (id: string) => {
      ...
      let next: GenomaConfig = enableResult.config;
  ...
  plugins
    ...
    .action(() => {
      ...
      const docs = formatDocsLink("/plugin", "docs.genoma.ai/plugin");
      lines.push("");
