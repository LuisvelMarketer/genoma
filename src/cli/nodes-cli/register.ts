    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["genoma nodes status", "List known nodes with live status."],
          ["genoma nodes pairing pending", "Show pending node pairing requests."],
          ['genoma nodes run --node <id> --raw "uname -a"', "Run a shell command on a node."],
          ["genoma nodes camera snap --node <id>", "Capture a photo from a node camera."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/nodes", "docs.genoma.ai/cli/nodes")}\n`,
    );
