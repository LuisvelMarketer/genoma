...
import type { GenomaConfig, ConfigFileSnapshot } from "../config/types.genoma.js";
...
vi.mock("../infra/genoma-root.js", () => ({
  resolveGenomaPackageRoot: vi.fn(),
}));
...
const { resolveGenomaPackageRoot } = await import("../infra/genoma-root.js");
...
describe("update-cli", () => {
  const fixtureRoot = "/tmp/genoma-update-tests";
  let fixtureCount = 0;
  ...
  const baseConfig = {} as GenomaConfig;
  const baseSnapshot: ConfigFileSnapshot = {
    path: "/tmp/genoma-config.json",
    exists: true,
    raw: "{}",
    parsed: {},
    resolved: baseConfig,
  ...
  const mockPackageInstallStatus = (root: string) => {
    vi.mocked(resolveGenomaPackageRoot).mockResolvedValue(root);
  ...
  const setupNonInteractiveDowngrade = async () => {
    const tempDir = createCaseDir("genoma-update");
    setTty(false);
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    vi.mocked(resolveGenomaPackageRoot).mockResolvedValue(process.cwd());
    ...
    readPackageName.mockResolvedValue("genoma");
    ...
    prepareRestartScript.mockResolvedValue("/tmp/genoma-restart-test.sh");
    runRestartScript.mockResolvedValue(undefined);
    inspectPortUsage.mockResolvedValue({
      port: 18789,
      status: "busy",
      listeners: [{ pid: 4242, command: "genoma-gateway" }],
      hints: [],
    });
  ...
  it("updateStatusCommand prints table output", async () => {
    ...
    expect(logs.join("\n")).toContain("Genoma update status");
  });
    ...
    },
    {
      ...
      prepare: async () => {
        vi.mocked(readConfigFileSnapshot).mockResolvedValue({
          ...baseSnapshot,
          config: { update: { channel: "beta" } } as GenomaConfig,
        });
      },
  ...
  it("honors --tag override", async () => {
    ...
    vi.mocked(resolveGenomaPackageRoot).mockResolvedValue(tempDir);
    vi.mocked(runGatewayUpdate).mockResolvedValue(
      makeOkUpdateResult({
        mode: "npm",
      }),
  ...
  it("updateCommand refreshes service env from updated install root when available", async () => {
    const root = createCaseDir("genoma-updated-root");
  ...
  it("updateCommand continues after doctor sub-step and clears update flag", async () => {
    ...
    try {
      await withEnvAsync({ GENOMA_UPDATE_IN_PROGRESS: undefined }, async () => {
        vi.mocked(runGatewayUpdate).mockResolvedValue(makeOkUpdateResult());
        vi.mocked(runDaemonRestart).mockResolvedValue(true);
        vi.mocked(doctorCommand).mockResolvedValue(undefined);
        vi.mocked(defaultRuntime.log).mockClear();
        ...
        expect(process.env.GENOMA_UPDATE_IN_PROGRESS).toBeUndefined();
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = createCaseDir("genoma-update-wizard");
    await withEnvAsync({ GENOMA_GIT_DIR: tempDir }, async () => {
      setTty(true);
