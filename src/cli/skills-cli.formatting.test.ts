...
describe("skills-cli (e2e)", () => {
  ...
  beforeAll(() => {
    ...
    tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "genoma-skills-test-"));
    tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "genoma-bundled-skills-test-"));
    process.env.GENOMA_BUNDLED_SKILLS_DIR = tempBundledDir;
  });
