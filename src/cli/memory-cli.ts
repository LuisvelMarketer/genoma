    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["genoma memory status", "Show index and provider status."],
          ["genoma memory index --force", "Force a full reindex."],
          ['genoma memory search --query "deployment notes"', "Search indexed memory entries."],
          ["genoma memory status --json", "Output machine-readable JSON."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/memory", "docs.genoma.ai/cli/memory")}\n`,
    );
