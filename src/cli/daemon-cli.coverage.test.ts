...
const buildGatewayInstallPlan = vi.fn(
  async (params: { port: number; token?: string; env?: NodeJS.ProcessEnv }) => ({
    ...
    environment: {
      GENOMA_GATEWAY_PORT: String(params.port),
      ...(params.token ? { GENOMA_GATEWAY_TOKEN: params.token } : {}),
    },
  }),
);
...
describe("daemon-cli coverage", () => {
  ...
  beforeEach(() => {
    ...
    process.env.GENOMA_STATE_DIR = "/tmp/genoma-cli-state";
    process.env.GENOMA_CONFIG_PATH = "/tmp/genoma-cli-state/genoma.json";
  ...
  it("derives probe URL from service args + env (json)", async () => {
    ...
    serviceReadCommand.mockResolvedValueOnce({
      ...
      environment: {
        GENOMA_PROFILE: "dev",
        GENOMA_STATE_DIR: "/tmp/genoma-daemon-state",
        GENOMA_CONFIG_PATH: "/tmp/genoma-daemon-state/genoma.json",
        GENOMA_GATEWAY_PORT: "19001",
      },
      sourcePath: "/tmp/ai.genoma.gateway.plist",
    });
