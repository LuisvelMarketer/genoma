...
function renderNodeServiceStartHints(): string[] {
  const base = [formatCliCommand("genoma node install"), formatCliCommand("genoma node start")];
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("genoma node install --force")}`);
    }
    return;
  }
...
export async function runNodeDaemonStatus(opts: NodeDaemonStatusOptions = {}) {
  ...
  const hintEnv = {
    ...baseEnv,
    GENOMA_LOG_PREFIX: baseEnv.GENOMA_LOG_PREFIX ?? "node",
