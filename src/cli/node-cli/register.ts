    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          [
            "genoma node run --host 127.0.0.1 --port 18789",
            "Run the node host in the foreground.",
          ],
          ["genoma node status", "Check node host service status."],
          ["genoma node install", "Install the node host service."],
          ["genoma node restart", "Restart the installed node host service."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/node", "docs.genoma.ai/cli/node")}\n`,
    );
