...
describe("npm-resolution helpers", () => {
  it("keeps original spec when pin is disabled", () => {
    const result = resolvePinnedNpmSpec({
      rawSpec: "@genoma/plugin-alpha@latest",
      pin: false,
      resolvedSpec: "@genoma/plugin-alpha@1.2.3",
    });
    expect(result).toEqual({
      recordSpec: "@genoma/plugin-alpha@latest",
    });
  });
  ...
  it("returns pinned spec notice when resolved spec is available", () => {
    ...
    expect(result).toEqual({
      recordSpec: "@genoma/plugin-alpha@1.2.3",
      pinNotice: "Pinned npm install record to @genoma/plugin-alpha@1.2.3.",
    });
  });
  ...
  it("maps npm resolution metadata to install fields", () => {
    expect(
      ...
      resolvedName: "@genoma/plugin-alpha",
  ...
  it("builds common npm install record fields", () => {
    expect(
      buildNpmInstallRecordFields({
        spec: "@genoma/plugin-alpha@1.2.3",
        installPath: "/tmp/genoma/extensions/alpha",
        version: "1.2.3",
        resolution: {
          name: "@genoma/plugin-alpha",
          version: "1.2.3",
  ...
  it("resolves pinned install record and emits pin notice", () => {
    ...
    expect(logs).toEqual(["Pinned npm install record to @genoma/plugin-alpha@1.2.3."]);
  ...
  it("resolves pinned install record for CLI and formats warning output", () => {
    ...
    const record = resolvePinnedNpmInstallRecordForCli(
      "@genoma/plugin-alpha@latest",
      true,
      "/tmp/genoma/extensions/alpha",
      "1.2.3",
      undefined,
      (message) => logs.push(message),
    ...
    expect(record).toEqual({
      source: "npm",
      spec: "@genoma/plugin-alpha@latest",
      installPath: "/tmp/genoma/extensions/alpha",
      version: "1.2.3",
