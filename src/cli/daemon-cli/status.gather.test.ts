...
const serviceReadCommand = vi.fn(async (_env?: NodeJS.ProcessEnv) => ({
  ...
  environment: {
    GENOMA_STATE_DIR: "/tmp/genoma-daemon",
    GENOMA_CONFIG_PATH: "/tmp/genoma-daemon/genoma.json",
  },
}));
...
const resolveStateDir = vi.fn(
  (env: NodeJS.ProcessEnv) => env.GENOMA_STATE_DIR ?? "/tmp/genoma-cli",
);
const resolveConfigPath = vi.fn((env: NodeJS.ProcessEnv, stateDir: string) => {
  return env.GENOMA_CONFIG_PATH ?? `${stateDir}/genoma.json`;
});

vi.mock("../../config/config.js", () => ({
  createConfigIO: ({ configPath }: { configPath: string }) => {
    const isDaemon = configPath.includes("/genoma-daemon/");
    return {
...
describe("gatherDaemonStatus", () => {
  ...
  beforeEach(() => {
    ...
    process.env.GENOMA_STATE_DIR = "/tmp/genoma-cli";
    process.env.GENOMA_CONFIG_PATH = "/tmp/genoma-cli/genoma.json";
