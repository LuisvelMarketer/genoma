...
beforeEach(() => {
  ...
  originalHideBanner = process.env.GENOMA_HIDE_BANNER;
  delete process.env.NODE_NO_WARNINGS;
  delete process.env.GENOMA_HIDE_BANNER;
});
...
afterEach(() => {
  ...
  if (originalHideBanner === undefined) {
    ...
    process.env.GENOMA_HIDE_BANNER = originalHideBanner;
  }
});
...
describe("registerPreActionHooks", () => {
  ...
  function buildProgram() {
    const program = new Command().name("genoma");
  ...
  it("handles debug mode and plugin-required command preaction", async () => {
    await runPreAction({
      parseArgv: ["status"],
      processArgv: ["node", "genoma", "status", "--debug"],
    });
    ...
    expect(process.title).toBe("genoma-status");

    vi.clearAllMocks();
    await runPreAction({
      parseArgv: ["message", "send"],
      processArgv: ["node", "genoma", "message", "send"],
    });
  ...
  it("skips help/version preaction and respects banner opt-out", async () => {
    await runPreAction({
      parseArgv: ["status"],
      processArgv: ["node", "genoma", "--version"],
    });
    ...
    vi.clearAllMocks();
    process.env.GENOMA_HIDE_BANNER = "1";

    await runPreAction({
      parseArgv: ["status"],
      processArgv: ["node", "genoma", "status"],
    });
  ...
  it("applies --json stdout suppression only for explicit JSON output commands", async () => {
    await runPreAction({
      ...
      processArgv: ["node", "genoma", "update", "status", "--json"],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "genoma", "config", "set", "gateway.auth.mode", "{bad", "--json"],
    });
