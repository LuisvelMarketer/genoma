...
${formatHelpExamples([
  ['genoma agent --to +15555550123 --message "status update"', "Start a new session."],
  ['genoma agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'genoma agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'genoma agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['genoma agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'genoma agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.genoma.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.genoma.ai/cli/agents")}\n`,
    );
...
${formatHelpExamples([
  ...
  [
    "genoma agents set-identity --identity-file ~/.genoma/workspace/IDENTITY.md --agent main",
