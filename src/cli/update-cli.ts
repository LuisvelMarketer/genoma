...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update Genoma and inspect update channel status")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["genoma update", "Update a source checkout (git)"],
        ["genoma update --channel beta", "Switch to beta channel (git + npm)"],
        ["genoma update --channel dev", "Switch to dev channel (git + npm)"],
        ["genoma update --tag beta", "One-off update to a dist-tag or version"],
        ["genoma update --dry-run", "Preview actions without changing anything"],
        ["genoma update --no-restart", "Update without restarting the service"],
        ["genoma update --json", "Output result as JSON"],
        ["genoma update --yes", "Non-interactive (accept downgrade prompts)"],
        ["genoma update wizard", "Interactive update wizard"],
        ["genoma --update", "Shorthand for genoma update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run genoma update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.genoma.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["genoma update status", "Show channel + version status."],
          ["genoma update status --json", "JSON output."],
          ["genoma update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.genoma.ai/cli/update")}`,
    )
    .action(async (opts, command) => {
      try {
