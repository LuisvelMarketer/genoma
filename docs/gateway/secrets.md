...
Genoma supports additive secret references so credentials do not need to be stored as plaintext in config files.
...
When onboarding runs in interactive mode and you choose secret reference storage, Genoma performs a fast preflight check before saving:
...
- Set `allowSymlinkCommand: true` to allow symlink command paths (for example Homebrew shims). Genoma validates the resolved target path.
  ...
  secrets: {
    providers: {
      onepassword_openai: {
        ...
        args: ["read", "op://Personal/Genoma QA API Key/password"],
        passEnv: ["HOME"],
        jsonOnly: false,
      },
    },
  ...
  secrets: {
    providers: {
      vault_openai: {
        ...
        allowSymlinkCommand: true, // required for Homebrew symlinked binaries
        trustedDirs: ["/opt/homebrew"],
        args: ["kv", "get", "-field=OPENAI_API_KEY", "secret/genoma"],
...
When reload-time activation fails after a healthy state, Genoma enters degraded secrets state.
...
```bash
genoma secrets audit --check
genoma secrets configure
genoma secrets audit --check
```
...
- plaintext values at rest (`genoma.json`, `auth-profiles.json`, `.env`)
- unresolved refs
...
- lets you select secret-bearing fields in `genoma.json`
...
- `genoma secrets configure --providers-only`
- `genoma secrets configure --skip-provider-setup`
...
```bash
genoma secrets apply --from /tmp/genoma-secrets-plan.json
genoma secrets apply --from /tmp/genoma-secrets-plan.json --dry-run
```
...
Genoma intentionally does **not** write rollback backups that contain pre-migration plaintext secret values.
...
For static credentials, Genoma runtime no longer depends on plaintext `auth.json`.
