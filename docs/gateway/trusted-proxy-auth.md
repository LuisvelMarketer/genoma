...
read_when:
  - Running Genoma behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of Genoma
...
- You run Genoma behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. Genoma checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. Genoma extracts the user identity from the configured header
...
- Genoma can stay on loopback HTTP behind the proxy.
...
If Genoma itself serves HTTPS directly (no TLS-terminating proxy), set:
...
```yaml
routes:
  - from: https://genoma.example.com
    to: http://genoma-gateway:18789
    policy:
      - allow:
          or:
            - email:
...
`genoma security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
3. Update Genoma config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `genoma security audit` and review findings
