...
> Genoma is **not** a hostile multi-tenant security boundary for multiple adversarial users sharing one agent/gateway.
...
Genoma security guidance assumes a **personal assistant** deployment: one trusted operator boundary, potentially many agents.
...
## Quick check: `genoma security audit`
...
```bash
genoma security audit
genoma security audit --deep
genoma security audit --fix
genoma security audit --json
```

It flags common footguns (Gateway auth exposure, browser control exposure, elevated allowlists, filesystem permissions).

Genoma is both a product and an experiment: you’re wiring frontier-model behavior into real messaging surfaces and real tools. **There is no “perfectly secure” setup.** The goal is to be deliberate about:
...
Genoma assumes the host and config boundary are trusted:

- If someone can modify Gateway host state/config (`~/.genoma`, including `genoma.json`), treat them as a trusted operator.
- Running one Gateway for multiple mutually untrusted/adversarial operators is **not a recommended setup**.
- For mixed-trust teams, split trust boundaries with separate gateways (or at minimum separate OS users/hosts).
- Genoma can run multiple gateway instances on one machine, but recommended operations favor clean trust-boundary separation.
- Recommended default: one user per machine/host (or VPS), one gateway for that user, and one or more agents in that gateway.
- If multiple users want Genoma, use one VPS/host per user.
...
Genoma is designed as a personal assistant security model: one trusted operator boundary, potentially many agents.
...
4. Claim is not listed in [Out of Scope](https://github.com/genoma/genoma/blob/main/SECURITY.md#out-of-scope).
...
If you run `--deep`, Genoma also attempts a best-effort live Gateway probe.

## Credential storage map

Use this when auditing access or deciding what to back up:

- **WhatsApp**: `~/.genoma/credentials/whatsapp/<accountId>/creds.json`
- **Telegram bot token**: config/env or `channels.telegram.tokenFile`
- **Discord bot token**: config/env (token file not yet supported)
- **Slack tokens**: config/env (`channels.slack.*`)
- **Pairing allowlists**:
  - `~/.genoma/credentials/<channel>-allowFrom.json` (default account)
  - `~/.genoma/credentials/<channel>-<accountId>-allowFrom.json` (non-default accounts)
- **Model auth profiles**: `~/.genoma/agents/<agentId>/agent/auth-profiles.json`
- **File-backed secrets payload (optional)**: `~/.genoma/secrets.json`
- **Legacy OAuth import**: `~/.genoma/credentials/oauth.json`
...
| -------------------------------------------------- | ------------- | ------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------- | -------- |
| `fs.state_dir.perms_world_writable`                | critical      | Other users/processes can modify full Genoma state                                 | filesystem perms on `~/.genoma`                                                                 | yes      |
| `fs.config.perms_writable`                         | critical      | Others can change auth/tool policy/config                                            | filesystem perms on `~/.genoma/genoma.json`                                                   | yes      |
...
`genoma security audit` warns when this setting is enabled.

## Insecure or dangerous flags summary

`genoma security audit` includes `config.insecure_or_dangerous_flags` when
...
Complete `dangerous*` / `dangerously*` config keys defined in Genoma config
schema:
...
gateway:
  ...
  auth:
    mode: password
    password: ${GENOMA_GATEWAY_PASSWORD}
```
...
- Genoma gateway is local/loopback first. If you terminate TLS at a reverse proxy, set HSTS on the proxy-facing HTTPS domain there.
- If the gateway itself terminates HTTPS, you can set `gateway.http.securityHeaders.strictTransportSecurity` to emit the HSTS header from Genoma responses.
...
Genoma stores session transcripts on disk under `~/.genoma/agents/<agentId>/sessions/*.jsonl`.
This is required for session continuity and (optionally) session memory indexing, but it also means
**any process/user with filesystem access can read those logs**. Treat disk access as the trust
boundary and lock down permissions on `~/.genoma` (see the audit section below). If you need
...
Genoma can refresh the skills list mid-session:
...
- If you install plugins from npm (`genoma plugins install <npm-spec>`), treat it like running untrusted code:
  - The install path is `~/.genoma/extensions/<pluginId>/` (or `$GENOMA_STATE_DIR/extensions/<pluginId>/`).
  - Genoma uses `npm pack` and then runs `npm install --omit=dev` in that directory (npm lifecycle scripts can execute code during install).
...
```bash
genoma pairing list <channel>
genoma pairing approve <channel> <code>
```
...
By default, Genoma routes **all DMs into the main session** so your assistant has continuity across devices and channels. If **multiple people** can DM the bot (open DMs or a multi-person allowlist), consider isolating DM sessions:
...
Genoma has two separate “who can trigger me?” layers:
...
- **DM allowlist** (`allowFrom` / `channels.discord.allowFrom` / `channels.slack.allowFrom`; legacy: `channels.discord.dm.allowFrom`, `channels.slack.dm.allowFrom`): who is allowed to talk to the bot in direct messages.
  - When `dmPolicy="pairing"`, approvals are written to the account-scoped pairing allowlist store under `~/.genoma/credentials/` (`<channel>-allowFrom.json` for default account, `<channel>-<accountId>-allowFrom.json` for non-default accounts), merged with config allowlists.
...
- “Paste the full contents of ~/.genoma or your logs.”

## Unsafe external content bypass flags

Genoma includes explicit bypass flags that disable external-content safety wrapping:
...
- `~/.genoma/genoma.json`: `600` (user read/write only)
- `~/.genoma`: `700` (user only)

`genoma doctor` can warn and offer to tighten these permissions.
...
- Default: `18789`
- Config/flags/env: `gateway.port`, `--port`, `GENOMA_GATEWAY_PORT`

This HTTP surface includes the Control UI and the canvas host:

- Control UI (SPA assets) (default base path `/`)
- Canvas host: `/__genoma__/canvas/` and `/__genoma__/a2ui/` (arbitrary HTML/JS; treat as untrusted content)
...
The Gateway broadcasts its presence via mDNS (`_genoma-gw._tcp` on port 5353) for local device discovery. In full mode, this includes TXT records that may expose operational details:
...
4. **Environment variable** (alternative): set `GENOMA_DISABLE_BONJOUR=1` to disable mDNS without config changes.
...
Doctor can generate one for you: `genoma doctor --generate-gateway-token`.
...
paths, set `GENOMA_ALLOW_INSECURE_PRIVATE_WS=1` on the client process as break-glass.
...
- `gateway.auth.mode: "password"`: password auth (prefer setting via env: `GENOMA_GATEWAY_PASSWORD`).
...
1. Generate/set a new secret (`gateway.auth.token` or `GENOMA_GATEWAY_PASSWORD`).
...
When `gateway.auth.allowTailscale` is `true` (default for Serve), Genoma
accepts Tailscale Serve identity headers (`tailscale-user-login`) for Control
UI/WebSocket authentication. Genoma verifies the identity by resolving the
...
- Genoma will trust `x-forwarded-for` (or `x-real-ip`) from those IPs to determine the client IP for local pairing checks and HTTP auth/local checks.
...
Assume anything under `~/.genoma/` (or `$GENOMA_STATE_DIR/`) may contain secrets or private data:

- `genoma.json`: config may include tokens (gateway, remote gateway), provider settings, and allowlists.
...
- When sharing diagnostics, prefer `genoma status --all` (pasteable, secrets redacted) over raw logs.
...
```json
{
  ...
  "agents": {
    "list": [
      {
        "id": "main",
        "groupChat": { "mentionPatterns": ["@genoma", "@mybot"] }
      }
    ]
  }
}
...
- Keep filesystem roots narrow: avoid broad roots like your home directory for agent workspaces/sandbox workspaces. Broad roots can expose sensitive local files (for example state/config under `~/.genoma`) to filesystem tools.
...
- `agents.defaults.sandbox.workspaceAccess: "none"` (default) keeps the agent workspace off-limits; tools run against a sandbox workspace under `~/.genoma/sandboxes`
...
- Prefer a dedicated profile for the agent (the default `genoma` profile).
...
- The Chrome extension relay’s CDP endpoint is auth-gated; only Genoma clients can connect.
- Disable browser proxy routing when you don’t need it (`gateway.nodes.browser.mode="off"`).
- Chrome extension relay mode is **not** “safer”; it can take over your existing Chrome tabs. Assume it can act as you in whatever that tab/profile can reach.

### Browser SSRF policy (trusted-network default)

Genoma’s browser network policy defaults to the trusted-operator model: private/internal destinations are allowed unless you explicitly disable them.
    ...
    list: [
      {
        ...
        // Session tools can reveal sensitive data from transcripts. By default Genoma limits these tools
...
1. **Stop it:** stop the macOS app (if it supervises the Gateway) or terminate your `genoma gateway` process.
2. **Close exposure:** set `gateway.bind: "loopback"` (or disable Tailscale Funnel/Serve) until you understand what happened.
3. **Freeze access:** switch risky DMs/groups to `dmPolicy: "disabled"` / require mentions, and remove `"*"` allow-all entries if you had them.

### Rotate (assume compromise if secrets leaked)

1. Rotate Gateway auth (`gateway.auth.token` / `GENOMA_GATEWAY_PASSWORD`) and restart.
...
1. Check Gateway logs: `/tmp/genoma/genoma-YYYY-MM-DD.log` (or `logging.file`).
2. Review the relevant transcript(s): `~/.genoma/agents/<agentId>/sessions/*.jsonl`.
3. Review recent config changes (anything that could have widened access: `gateway.bind`, `gateway.auth`, dm/group policies, `tools.elevated`, plugin changes).
4. Re-run `genoma security audit --deep` and confirm critical findings are resolved.

### Collect for a report

- Timestamp, gateway host OS + Genoma version
...
Found a vulnerability in Genoma? Please report responsibly:

1. Email: [security@genoma.ai](mailto:security@genoma.ai)
