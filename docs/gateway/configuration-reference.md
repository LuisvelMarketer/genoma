...
description: "Complete field-by-field reference for ~/.genoma/genoma.json"
summary: "Complete reference for every Genoma config key, defaults, and channel settings"
read_when:
...
Every field available in `~/.genoma/genoma.json`. For a task-oriented overview, see [Configuration](/gateway/configuration).

Config format is **JSON5** (comments + trailing commas allowed). All fields are optional — Genoma uses safe defaults when omitted.
...
- Legacy single-account Baileys auth dir is migrated by `genoma doctor` into `whatsapp/default`.
...
```json5
{
  channels: {
    discord: {
      ...
      dm: { enabled: true, groupEnabled: false, groupChannels: ["genoma-dm"] },
      guilds: {
        "123456789012345678": {
...
- Genoma additionally attempts voice receive recovery by leaving/rejoining a voice session after repeated decrypt failures.
...
Mattermost ships as a plugin: `genoma plugins install @genoma/mattermost`.
...
Genoma spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
- If you add a non-default account via `genoma channels add` (or channel onboarding) while still on a single-account top-level channel config, Genoma moves account-scoped top-level single-account values into `channels.<channel>.accounts.default` first so the original account keeps working.
- Existing channel-only bindings (no `accountId`) keep matching the default account; account-scoped bindings remain optional.
- `genoma doctor --fix` also repairs mixed shapes by moving account-scoped top-level single-account values into `accounts.default` when named accounts exist but `default` is missing.
...
```json5
{
  ...
  agents: {
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["reisponde", "@genoma"] },
      },
    ],
  },
}
...
- `config: true` enables `/config` (reads/writes `genoma.json`).
...
Optional repository root shown in the system prompt's Runtime line. If unset, Genoma auto-detects by walking upward from the workspace.
...
- `model.primary`: format `provider/model` (e.g. `anthropic/claude-opus-4-6`). If you omit the provider, Genoma assumes `anthropic` (deprecated).
    ...
    defaults: {
      sandbox: {
        ...
        browser: {
          ...
          network: "genoma-sandbox-browser",
          cdpPort: 9222,
...
- `none`: per-scope sandbox workspace under `~/.genoma/sandboxes`
...
noVNC observer access uses VNC auth by default and Genoma emits a short-lived token URL that serves a local bootstrap page; noVNC password is passed via URL fragment (instead of URL query).

- `allowHostControl: false` (default) blocks sandboxed sessions from targeting the host browser.
- `network` defaults to `genoma-sandbox-browser` (dedicated bridge network). Set to `bridge` only when you explicitly want global bridge connectivity.
...
- Launch defaults are defined in `scripts/sandbox-browser-entrypoint.sh` and tuned for container hosts:
  ...
  - `--disable-3d-apis`, `--disable-software-rasterizer`, and `--disable-gpu` are
    ...
    `GENOMA_BROWSER_DISABLE_GRAPHICS_FLAGS=0` if WebGL/3D usage requires it.
  - `GENOMA_BROWSER_DISABLE_EXTENSIONS=0` re-enables extensions if your workflow
    depends on them.
  - `--renderer-process-limit=2` can be changed with
    `GENOMA_BROWSER_RENDERER_PROCESS_LIMIT=<N>`; set `0` to use Chromium's
    ...
    list: [
      {
        ...
        agentDir: "~/.genoma/agents/main/agent",
        ...
        groupChat: { mentionPatterns: ["@genoma"] },
        sandbox: { mode: "off" },
...
- **`parentForkMaxTokens`**: max parent-session `totalTokens` allowed when creating a forked thread session (default `100000`).
  - If parent `totalTokens` is above this value, Genoma starts a fresh thread session instead of inheriting parent transcript history.
...
| `group:nodes`      | `nodes`                                                                                  |
| `group:genoma`   | All built-in tools (excludes provider plugins)                                           |
...
- Files are materialized into the child workspace at `.genoma/attachments/<uuid>/` with a `.manifest.json`.
...
Genoma uses the pi-coding-agent model catalog. Add custom providers via `models.providers` in config or `~/.genoma/agents/<agentId>/agent/models.json`.
...
- Override agent config root with `GENOMA_AGENT_DIR` (or `PI_CODING_AGENT_DIR`).
...
Set `OPENCODE_API_KEY` (or `OPENCODE_ZEN_API_KEY`). Shortcut: `genoma onboard --auth-choice opencode-zen`.
...
Set `ZAI_API_KEY`. `z.ai/*` and `z-ai/*` are accepted aliases. Shortcut: `genoma onboard --auth-choice zai-api-key`.
...
For the China endpoint: `baseUrl: "https://api.moonshot.cn/v1"` or `genoma onboard --auth-choice moonshot-api-key-cn`.
...
Anthropic-compatible, built-in provider. Shortcut: `genoma onboard --auth-choice kimi-code-api-key`.
...
Base URL should omit `/v1` (Anthropic client appends it). Shortcut: `genoma onboard --auth-choice synthetic-api-key`.
...
Set `MINIMAX_API_KEY`. Shortcut: `genoma onboard --auth-choice minimax-api`.
...
- Loaded from `~/.genoma/extensions`, `<workspace>/.genoma/extensions`, plus `plugins.load.paths`.
...
- `plugins.installs`: CLI-managed install metadata used by `genoma plugins update`.
...
```json5
{
  gateway: {
    ...
    auth: {
      ...
      token: "your-token",
      // password: "your-password", // or GENOMA_GATEWAY_PASSWORD
...
- `port`: single multiplexed port for WS + HTTP. Precedence: `--port` > `GENOMA_GATEWAY_PORT` > `gateway.port` > `18789`.
...
- `GENOMA_ALLOW_INSECURE_PRIVATE_WS=1`: client-side break-glass override that allows plaintext `ws://` to trusted private-network IPs; default remains loopback-only for plaintext.
...
Convenience flags: `--dev` (uses `~/.genoma-dev` + port `19001`), `--profile <name>` (uses `~/.genoma-<name>`).
...
```json5
{
  hooks: {
    ...
    presets: ["gmail"],
    transformsDir: "~/.genoma/hooks/transforms",
    mappings: [
      {
        match: { path: "gmail" },
        action: "agent",
...
Auth: `Authorization: Bearer <token>` or `x-genoma-token: <token>`.
...
- Gateway auto-starts `gog gmail watch serve` on boot when configured. Set `GENOMA_SKIP_GMAIL_WATCHER=1` to disable.
...
```json5
{
  canvasHost: {
    ...
    liveReload: true,
    // enabled: false, // or GENOMA_SKIP_CANVAS_HOST=1
  },
}
```

- Serves agent-editable HTML/CSS/JS and A2UI over HTTP under the Gateway port:
  - `http://<gateway-host>:<gateway.port>/__genoma__/canvas/`
  - `http://<gateway-host>:<gateway.port>/__genoma__/a2ui/`
...
- Also serves A2UI at `/__genoma__/a2ui/`.
...
- Hostname defaults to `genoma`. Override with `GENOMA_MDNS_HOSTNAME`.
...
Writes a unicast DNS-SD zone under `~/.genoma/dns/`. For cross-network discovery, pair with a DNS server (CoreDNS recommended) + Tailscale split DNS.

Setup: `genoma dns setup --apply`.
...
- `.env` files: CWD `.env` + `~/.genoma/.env` (neither overrides existing vars).
...
{
  gateway: {
    auth: { token: "${GENOMA_GATEWAY_TOKEN}" },
  },
}
```
...
```json5
{
  secrets: {
    providers: {
      ...
      filemain: {
        source: "file",
        path: "~/.genoma/secrets.json",
        mode: "json",
        timeoutMs: 5000,
      },
      vault: {
        source: "exec",
        command: "/usr/local/bin/genoma-vault-resolver",
...
- Legacy OAuth imports from `~/.genoma/credentials/oauth.json`.
...
- Default log file: `/tmp/genoma/genoma-YYYY-MM-DD.log`.
...
Current builds no longer include the TCP bridge. Nodes connect over the Gateway WebSocket. `bridge.*` keys are no longer part of the config schema (validation fails until removed; `genoma doctor --fix` can strip unknown keys).
