...
read_when:
  - Generating or reviewing `genoma secrets apply` plan files
...
This page defines the strict contract enforced by `genoma secrets apply`.

If a target does not match these rules, apply fails before mutating config.

## Plan file shape

`genoma secrets apply --from <plan.json>` expects a `targets` array of plan targets:
...
If apply fails with an invalid target path message, regenerate the plan with `genoma secrets configure` or fix the target path to one of the allowed shapes above.
