...
- During heartbeat runs, Genoma treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `genoma sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, Genoma skips the heartbeat run entirely (no model call).
