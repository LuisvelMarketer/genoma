...
read_when:
  ...
  - Wiring external systems into Genoma
title: "Webhooks"
---
...
- `hooks.transformsDir` + `transform.module` loads a JS/TS module for custom logic.
  - `hooks.transformsDir` (if set) must stay within the transforms root under your Genoma config directory (typically `~/.genoma/hooks/transforms`).
...
- `genoma webhooks gmail setup` writes `hooks.gmail` config for `genoma webhooks gmail run`.
