...
read_when:
  - You want to place an outbound voice call from Genoma
...
Voice calls for Genoma via a plugin. Supports outbound notifications and
...
- Use `genoma voicecall ...` or the `voice_call` tool
...
```bash
genoma plugins install ./extensions/voice-call
