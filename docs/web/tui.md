...
```bash
genoma tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `genoma tui --deliver`
...
- Local shell commands receive `GENOMA_SHELL=tui-local` in their environment.
...
- Check the Gateway logs: `genoma logs --follow`.
- Confirm the agent can run: `genoma status` and `genoma models status`.
...
- No agents in picker: check `genoma agents list` and your routing config.
