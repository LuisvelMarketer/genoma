...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/genoma`)
...
If the page fails to load, start the Gateway first: `genoma gateway`.
...
you revoke it with `genoma devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.genoma/genoma.json` (`config.get`, `config.set`)
...
- Stop:
  ...
  - Type `/stop` (or standalone abort phrases like `stop`, `stop action`, `stop run`, `stop genoma`, `please stop`) to abort out-of-band
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. Genoma
...
Genoma **blocks** Control UI connections without device identity.
