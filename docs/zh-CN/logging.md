...
你可以在 `~/.genoma/genoma.json` 中覆盖此设置：
...
所有日志配置都在 `~/.genoma/genoma.json` 的 `logging` 下。
...
- Genoma 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
- 你也可以使用 `genoma plugins enable diagnostics-otel` 启用插件。
...
- `genoma.tokens`（计数器，属性：`genoma.token`、`genoma.channel`、`genoma.provider`、`genoma.model`）
- `genoma.cost.usd`（计数器，属性：`genoma.channel`、`genoma.provider`、`genoma.model`）
- `genoma.run.duration_ms`（直方图，属性：`genoma.channel`、`genoma.provider`、`genoma.model`）
- `genoma.context.tokens`（直方图，属性：`genoma.context`、`genoma.channel`、`genoma.provider`、`genoma.model`）

消息流：

- `genoma.webhook.received`（计数器，属性：`genoma.channel`、`genoma.webhook`）
- `genoma.webhook.error`（计数器，属性：`genoma.channel`、`genoma.webhook`）
- `genoma.webhook.duration_ms`（直方图，属性：`genoma.channel`、`genoma.webhook`）
- `genoma.message.queued`（计数器，属性：`genoma.channel`、`genoma.source`）
- `genoma.message.processed`（计数器，属性：`genoma.channel`、`genoma.outcome`）
- `genoma.message.duration_ms`（直方图，属性：`genoma.channel`、`genoma.outcome`）

队列 + 会话：

- `genoma.queue.lane.enqueue`（计数器，属性：`genoma.lane`）
- `genoma.queue.lane.dequeue`（计数器，属性：`genoma.lane`）
- `genoma.queue.depth`（直方图，属性：`genoma.lane` 或 `genoma.channel=heartbeat`）
- `genoma.queue.wait_ms`（直方图，属性：`genoma.lane`）
- `genoma.session.state`（计数器，属性：`genoma.state`、`genoma.reason`）
- `genoma.session.stuck`（计数器，属性：`genoma.state`）
- `genoma.session.stuck_age_ms`（直方图，属性：`genoma.state`）
- `genoma.run.attempt`（计数器，属性：`genoma.attempt`）
...
- `genoma.webhook.error`
  - `genoma.channel`、`genoma.webhook`、`genoma.chatId`、`genoma.error`
- `genoma.message.processed`
  - `genoma.channel`、`genoma.outcome`、`genoma.chatId`、`genoma.messageId`、`genoma.sessionKey`、`genoma.sessionId`、`genoma.reason`
- `genoma.session.stuck`
  - `genoma.state`、`genoma.ageMs`、`genoma.queueDepth`、`genoma.sessionKey`、`genoma.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `genoma doctor`。
