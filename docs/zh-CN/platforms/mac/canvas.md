...
- `~/Library/Application Support/Genoma/canvas/<session>/...`

Canvas 面板通过**自定义 URL 方案**提供这些文件：

- `genoma-canvas://<session>/<path>`

示例：

- `genoma-canvas://main/` → `<canvasRoot>/main/index.html`
- `genoma-canvas://main/assets/app.css` → `<canvasRoot>/main/assets/app.css`
- `genoma-canvas://main/widgets/todo/` → `<canvasRoot>/main/widgets/todo/index.html`
...
```bash
genoma nodes canvas present --node <id>
genoma nodes canvas navigate --node <id> --url "/"
genoma nodes canvas eval --node <id> --js "document.title"
genoma nodes canvas snapshot --node <id>
```
...
```
http://<gateway-host>:18793/__genoma__/a2ui/
```
...
genoma nodes canvas a2ui push --jsonl /tmp/a2ui-v0.8.jsonl --node <id>
```

快速测试：

```bash
genoma nodes canvas a2ui push --node <id> --text "Hello from A2UI"
```
...
```js
window.location.href = "genoma://agent?message=Review%20this%20design";
```
