...
read_when:
  - You need detailed behavior for genoma onboard
...
This page is the full reference for `genoma onboard`.
...
<Steps>
  <Step title="Existing config detection">
    - If `~/.genoma/genoma.json` exists, choose Keep, Modify, or Reset.
    - Re-running the wizard does not wipe anything unless you explicitly choose Reset (or pass `--reset`).
    - CLI `--reset` defaults to `config+creds+sessions`; use `--reset-scope full` to also remove workspace.
    - If config is invalid or contains legacy keys, the wizard stops and asks you to run `genoma doctor` before continuing.
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via
      `genoma pairing approve <channel> <code>` or use allowlists.
  </Step>
  ...
  <Step title="Health check">
    - Starts gateway (if needed) and runs `genoma health`.
    - `genoma status --deep` adds gateway health probes to status output.
  </Step>
  <Step title="Skills">
...
- OAuth credentials: `~/.genoma/credentials/oauth.json`
- Auth profiles (API keys + OAuth): `~/.genoma/agents/<agentId>/agent/auth-profiles.json`
...
`~/.genoma/credentials/oauth.json` (or `$GENOMA_STATE_DIR/credentials/oauth.json`)
to the gateway host.
</Note>
...
- Command reference: [`genoma onboard`](/cli/onboard)
