...
The onboarding wizard is the **recommended** way to set up Genoma on macOS,
...
`genoma dashboard` and chat in the browser. Docs: [Dashboard](/web/dashboard).
</Info>
...
(`web_fetch` works without a key). Easiest path: `genoma configure --section web`
...
2. **Workspace** — Location for agent files (default `~/.genoma/workspace`). Seeds bootstrap files.
...
If the config is invalid or contains legacy keys, the wizard asks you to run `genoma doctor` first.
</Note>
...
Use `genoma agents add <name>` to create a separate agent with its own workspace,
...
- Default workspaces follow `~/.genoma/workspace-<agentId>`.
...
- CLI command reference: [`genoma onboard`](/cli/onboard)
