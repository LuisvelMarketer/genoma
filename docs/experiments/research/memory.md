...
read_when:
  - Designing workspace memory (~/.genoma/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep Genoma integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.genoma/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.genoma/workspace` as canonical human-readable memory.
...
```
~/.genoma/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in Genoma**, but keep a separable core library.

### Why integrate into Genoma?

- Genoma already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `genoma memory recall "…" --k 25 --since 30d`
  - `genoma memory reflect --since 7d`
...
Pragmatic take for `~/.genoma/workspace`:
