...
This plan defines how Genoma should support ACP coding agents in thread-capable channels (Discord first) with production-level lifecycle and recovery.
...
- Genoma core owns ACP control plane concerns
...
Genoma should not reimplement ACP transport internals in core.
Genoma should not rely on a pure plugin-only interception path for routing.

## North-star architecture (holy grail)

Treat ACP as a first-class control plane in Genoma, with pluggable runtime adapters.
...
- move ACP control-plane state to a dedicated SQLite store (WAL mode) under Genoma state dir
...
- ACP event stream is normalized to Genoma reply chunks
...
For the first production backend (`extensions/acpx`), Genoma and acpx are
...
- encoded payload fields:
  - `name` (acpx named session; uses Genoma `sessionKey`)
...
- Genoma consumes ndjson events from `acpx --format json --json-strict`
