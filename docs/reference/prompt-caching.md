...
- With Anthropic API-key auth profiles, Genoma seeds `cacheRetention: "short"` for Anthropic model refs when unset.
...
For `openrouter/anthropic/*` model refs, Genoma injects Anthropic `cache_control` on system/developer prompt blocks to improve prompt-cache reuse.
...
Genoma exposes dedicated cache-trace diagnostics for embedded agent runs.
...
diagnostics:
  cacheTrace:
    enabled: true
    filePath: "~/.genoma/logs/cache-trace.jsonl" # optional
...
- `filePath`: `$GENOMA_STATE_DIR/logs/cache-trace.jsonl`
...
- `GENOMA_CACHE_TRACE=1` enables cache tracing.
- `GENOMA_CACHE_TRACE_FILE=/path/to/cache-trace.jsonl` overrides output path.
- `GENOMA_CACHE_TRACE_MESSAGES=0|1` toggles full message payload capture.
- `GENOMA_CACHE_TRACE_PROMPT=0|1` toggles prompt text capture.
- `GENOMA_CACHE_TRACE_SYSTEM=0|1` toggles system prompt capture.
