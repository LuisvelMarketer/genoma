---
summary: "How Genoma builds prompt context and reports token usage + costs"
read_when:
...
Genoma tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

Genoma assembles its own system prompt on every run. It includes:
...
For images, Genoma downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from Genoma session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` + `/usage` are supported.
- **CLI:** `genoma status --usage` and `genoma channels list` show
...
`cacheWrite`. If pricing is missing, Genoma shows tokens only. OAuth tokens
never show dollar cost.

## Cache TTL and pruning impact

Provider prompt caching only applies within the cache TTL window. Genoma can
...
Anthropic's 1M context window is currently beta-gated. Genoma can inject the
...
Genoma skips the `context-1m-*` beta header because Anthropic currently
