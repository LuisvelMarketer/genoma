...
Genoma integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- Genoma owns lifecycle when `channels.signal.autoStart=true`.
...
- Genoma spawns `imsg rpc` as a child process (legacy iMessage integration).
