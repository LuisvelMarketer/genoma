...
Genoma features that can generate provider usage or paid API calls.
...
- `genoma status --usage` and `genoma channels list` show provider **usage windows**
...
Genoma can pick up credentials from:
...
`genoma models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
