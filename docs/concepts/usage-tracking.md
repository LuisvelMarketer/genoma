...
- `/usage cost` in chats: local cost summary aggregated from Genoma session logs.
- CLI: `genoma status --usage` prints a full per-provider breakdown.
- CLI: `genoma channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
