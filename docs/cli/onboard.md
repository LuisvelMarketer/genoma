---
summary: "CLI reference for `genoma onboard` (interactive onboarding wizard)"
read_when:
...
```bash
genoma onboard
genoma onboard --flow quickstart
genoma onboard --flow manual
genoma onboard --mode remote --remote-url wss://gateway-host:18789
```

For plaintext private-network `ws://` targets (trusted networks only), set
`GENOMA_ALLOW_INSECURE_PRIVATE_WS=1` in the onboarding process environment.
...
- Fastest first chat: `genoma dashboard` (Control UI, no channel setup).
