---
summary: "CLI reference for `genoma secrets` (reload, audit, configure, apply)"
read_when:
...
Use `genoma secrets` to migrate credentials from plaintext to SecretRefs and keep the active secrets runtime healthy.
...
```bash
genoma secrets audit
genoma secrets audit --check
genoma secrets audit --json
```
...
```bash
genoma secrets configure
genoma secrets configure --plan-out /tmp/genoma-secrets-plan.json
genoma secrets configure --apply --yes
genoma secrets configure --providers-only
genoma secrets configure --skip-provider-setup
genoma secrets configure --json
```
...
- `configure` targets secret-bearing fields in `genoma.json`.
...
genoma secrets apply --from /tmp/genoma-secrets-plan.json
genoma secrets apply --from /tmp/genoma-secrets-plan.json --dry-run
genoma secrets apply --from /tmp/genoma-secrets-plan.json --json
```
...
- `genoma.json` (SecretRef targets + provider upserts/deletes)
- `auth-profiles.json` (provider-target scrubbing)
- legacy `auth.json` residues
- `~/.genoma/.env` known secret keys whose values were migrated
