---
summary: "Genoma on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up Genoma on a Raspberry Pi
  - Running Genoma on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# Genoma on Raspberry Pi

## Goal

Run a persistent, always-on Genoma Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
# Update config
genoma config set gateway.bind tailnet
...
```bash
grep -q 'NODE_COMPILE_CACHE=/var/tmp/genoma-compile-cache' ~/.bashrc || cat >> ~/.bashrc <<'EOF'
...
- `GENOMA_NO_RESPAWN=1` avoids extra startup cost from CLI self-respawn.
- First run warms the cache; later runs benefit most.

### systemd startup tuning (optional)

If this Pi is mostly running Genoma, add a service drop-in to reduce restart
jitter and keep startup env stable:

```bash
sudo systemctl edit genoma
```

```ini
[Service]
Environment=GENOMA_NO_RESPAWN=1
Environment=NODE_COMPILE_CACHE=/var/tmp/genoma-compile-cache
Restart=always
RestartSec=2
TimeoutStartSec=90
```
...
If possible, keep Genoma state/cache on SSD-backed storage to avoid SD-card
...
Most Genoma features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled genoma

# Enable if not
sudo systemctl enable genoma

# Start on boot
sudo systemctl start genoma
```
...
```bash
# Check logs
journalctl -u genoma --no-pager -n 100

# Common fix: rebuild
cd ~/genoma  # if using hackable install
npm run build
sudo systemctl restart genoma
```
