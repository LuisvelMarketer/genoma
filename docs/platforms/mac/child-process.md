...
service via the external `genoma` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `ai.genoma.gateway`
  (or `ai.genoma.<profile>` when using `--profile`/`GENOMA_PROFILE`; legacy `com.genoma.*` is supported).
