...
- Same path: run the app from a fixed location (for Genoma, `dist/Genoma.app`).
...
```bash
sudo tccutil reset Accessibility ai.genoma.mac
sudo tccutil reset ScreenCapture ai.genoma.mac
...
Workaround: move files into the Genoma workspace (`~/.genoma/workspace`) if you want to avoid per-folder grants.
