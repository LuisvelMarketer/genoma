...
Genoma supports **camera capture** for agent workflows:
...
```bash
genoma nodes camera snap --node <id>               # default: both front + back (2 MEDIA lines)
genoma nodes camera snap --node <id> --facing front
genoma nodes camera clip --node <id> --duration 3000
genoma nodes camera clip --node <id> --no-audio
```
...
- **Settings → General → Allow Camera** (`genoma.cameraEnabled`)
  - Default: **off**
  - When off: camera requests return “Camera disabled by user”.

### CLI helper (node invoke)

Use the main `genoma` CLI to invoke camera commands on the macOS node.

Examples:

```bash
genoma nodes camera list --node <id>            # list camera ids
genoma nodes camera snap --node <id>            # prints MEDIA:<path>
genoma nodes camera snap --node <id> --max-width 1280
genoma nodes camera snap --node <id> --delay-ms 2000
genoma nodes camera snap --node <id> --device-id <id>
genoma nodes camera clip --node <id> --duration 10s          # prints MEDIA:<path>
genoma nodes camera clip --node <id> --duration-ms 3000      # prints MEDIA:<path> (legacy flag)
genoma nodes camera clip --node <id> --device-id <id>
genoma nodes camera clip --node <id> --no-audio
```

Notes:

- `genoma nodes camera snap` defaults to `maxWidth=1600` unless overridden.
...
```bash
genoma nodes screen record --node <id> --duration 10s --fps 15   # prints MEDIA:<path>
```
