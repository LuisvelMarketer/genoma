...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in Genoma
...
Running Genoma via Node with `tsx` fails at startup with:

```
[genoma] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `genoma status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch genoma.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node genoma.mjs status` works on Node 25.
