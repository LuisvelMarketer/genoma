---
summary: "Use OpenAI via API keys or Codex subscription in Genoma"
read_when:
  - You want to use OpenAI models in Genoma
...
```bash
genoma onboard --auth-choice openai-api-key
# or non-interactive
genoma onboard --openai-api-key "$OPENAI_API_KEY"
```
...
genoma onboard --auth-choice openai-codex

# Or run OAuth directly
genoma models auth login --provider openai-codex
```
...
Genoma uses `pi-ai` for model streaming. For both `openai/*` and
...
For `openai/*` (Responses API), Genoma also enables WebSocket warm-up by
...
OpenAI docs describe warm-up as optional. Genoma enables it by default for
...
`baseUrl` on `api.openai.com`), Genoma now auto-enables OpenAI server-side
