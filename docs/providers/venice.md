---
summary: "Use Venice AI privacy-focused models in Genoma"
read_when:
  - You want privacy-focused inference in Genoma
...
```bash
genoma onboard --auth-choice venice-api-key
```
...
```bash
genoma agent --model venice/llama-3.3-70b --message "Hello, are you working?"
```

## Model Selection

After setup, Genoma shows all available Venice models. Pick based on your needs:
...
```bash
genoma models set venice/claude-opus-45
genoma models set venice/llama-3.3-70b
```

List all available models:

```bash
genoma models list | grep venice
```

## Configure via `genoma configure`

1. Run `genoma configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
Genoma automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
genoma agent --model venice/llama-3.3-70b --message "Quick health check"

# Use Claude via Venice (anonymized)
genoma agent --model venice/claude-opus-45 --message "Summarize this task"

# Use uncensored model
genoma agent --model venice/venice-uncensored --message "Draft options"

# Use vision model with image
genoma agent --model venice/qwen3-vl-235b-a22b --message "Review attached image"

# Use coding model
genoma agent --model venice/qwen3-coder-480b-a35b-instruct --message "Refactor this function"
```
...
The Venice model catalog updates dynamically. Run `genoma models list` to see currently available models. Some models may be temporarily offline.
