---
summary: "Use MiniMax M2.1 in Genoma"
read_when:
  - You want MiniMax models in Genoma
...
```bash
genoma plugins enable minimax-portal-auth  # skip if already loaded.
genoma gateway restart  # restart if gateway is already running
genoma onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/genoma/genoma/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `genoma.json`:
...
- Use `genoma models list` and `genoma models set minimax/MiniMax-M2.1` to switch.
...
- Running `genoma configure` and selecting **MiniMax M2.1**, or
