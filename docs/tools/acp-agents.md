...
[Agent Client Protocol (ACP)](https://agentclientprotocol.com/) sessions let Genoma run external coding harnesses (for example Pi, Claude Code, Codex, OpenCode, and Gemini CLI) through an ACP backend plugin.

If you ask Genoma in plain language to "run this in Codex" or "start Claude Code in a thread", Genoma should route that request to the ACP runtime (not the native sub-agent runtime).
...
Use ACP when you want an external harness runtime. Use sub-agents when you want Genoma-native delegated runs.

| Area          | ACP session                           | Sub-agent run                      |
| ------------- | ------------------------------------- | ---------------------------------- |
| Runtime       | ACP backend plugin (for example acpx) | Genoma native sub-agent runtime  |
...
- Genoma binds a thread to a target ACP session.
- Follow-up messages in that thread route to the bound ACP session.
- ACP output is delivered back to the same thread.
- Unfocus/close/archive/idle-timeout or max-age expiry removes the binding.

Thread binding support is adapter-specific. If the active channel adapter does not support thread bindings, Genoma returns a clear unsupported/unavailable message.
...
- If `agentId` is omitted, Genoma uses `acp.defaultAgent` when configured.
...
- `mode` (optional): `run` (one-shot) or `session` (persistent).
  - default is `run`
  - if `thread: true` and mode omitted, Genoma may default to persistent behavior per runtime path
...
If no target resolves, Genoma returns a clear error (`Unable to resolve session target: ...`).
...
Some controls depend on backend capabilities. If a backend does not support a control, Genoma returns a clear unsupported-control error.
...
When Genoma uses the acpx backend, prefer these values for `agentId` unless your acpx config defines custom agent aliases.

Direct acpx CLI usage can also target arbitrary adapters via `--agent <command>`, but that raw escape hatch is an acpx CLI feature (not the normal Genoma `agentId` path).
...
```bash
genoma plugins install @genoma/acpx
genoma config set plugins.entries.acpx.enabled true
```

Local workspace install during development:

```bash
genoma plugins install ./extensions/acpx
```
...
By default, `@genoma/acpx` uses the plugin-local pinned binary:
...
- Relative paths resolve from Genoma workspace directory.
...
- Genoma startup remains non-blocking while the backend health check runs.
...
```bash
genoma config set plugins.entries.acpx.config.permissionMode approve-all
genoma config set plugins.entries.acpx.config.nonInteractivePermissions fail
```

Restart the gateway after changing these values.

> **Important:** Genoma currently defaults to `permissionMode=approve-reads` and `nonInteractivePermissions=fail`. In non-interactive ACP sessions, any write or exec that triggers a permission prompt can fail with `AcpRuntimeError: Permission prompt unavailable in non-interactive mode`.
>
