...
- **gateway host** → `genoma` process on the gateway machine
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.genoma/exec-approvals.sock",
...
- allowlist entries live in host-local `~/.genoma/exec-approvals.json` under `agents.<id>.allowlist` (or via Control UI / `genoma approvals allowlist ...`).
- `genoma security audit` warns with `tools.exec.safe_bins_interpreter_unprofiled` when interpreter/runtime bins appear in `safeBins` without explicit profiles.
- `genoma doctor --fix` can scaffold missing custom `safeBinProfiles.<bin>` entries as `{}` (review and tighten afterward). Interpreter/runtime bins are not auto-scaffolded.
...
`~/.genoma/exec-approvals.json` directly.

CLI: `genoma approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
