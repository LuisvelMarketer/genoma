...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `genoma sandbox explain`.
...
Legacy `agent.*` configs are migrated by `genoma doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=genoma-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${GENOMA_STATE_DIR:-$HOME/.genoma}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
