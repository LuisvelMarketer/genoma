---
summary: "Chrome extension: let Genoma drive your existing Chrome tab"
read_when:
...
The Genoma Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate genoma-managed Chrome profile.
...
Genoma then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the Genoma release (npm package) as static files. There is no separate “build” step.

After upgrading Genoma:

- Re-run `genoma browser extension install` to refresh the installed files under your Genoma state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (set gateway token once)

Genoma ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.

Before first attach, open extension Options and set:

- `Port` (default `18792`)
- `Gateway token` (must match `gateway.auth.token` / `GENOMA_GATEWAY_TOKEN`)
...
- Open the tab you want Genoma to control.
...
- `ON`: attached; Genoma can drive that tab.
...
Debugging: `genoma sandbox explain`
...
`genoma browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `genoma browser extension install` first to copy the extension to a stable location under your Genoma state directory.
...
- **This is not isolated** like the dedicated genoma-managed profile.
