...
read_when:
  - Setting up Zalo Personal for Genoma
...
Status: experimental. This integration automates a **personal Zalo account** via native `zca-js` inside Genoma.
...
- Install via CLI: `genoma plugins install @genoma/zalouser`
- Or from a source checkout: `genoma plugins install ./extensions/zalouser`
...
2. Login (QR, on the Gateway machine):
   - `genoma channels login --channel zalouser`
...
```bash
genoma directory self --channel zalouser
genoma directory peers list --channel zalouser --query "name"
genoma directory groups list --channel zalouser --query "work"
```
...
- `genoma pairing list zalouser`
- `genoma pairing approve zalouser <code>`
...
- On startup, Genoma resolves group/user names in allowlists to IDs and logs the mapping; unresolved entries are kept as typed.
...
Accounts map to `zalouser` profiles in Genoma state. Example:
...
- Genoma sends a typing event before dispatching a reply (best-effort).
...
- For inbound messages that include event metadata, Genoma sends delivered + seen acknowledgements (best-effort).
...
- Re-login: `genoma channels logout --channel zalouser && genoma channels login --channel zalouser`
...
- The channel now runs fully in Genoma without external CLI binaries.
