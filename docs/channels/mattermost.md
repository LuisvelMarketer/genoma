---
summary: "Mattermost bot setup and Genoma config"
read_when:
...
```bash
genoma plugins install @genoma/mattermost
```

Local checkout (when running from a git repo):

```bash
genoma plugins install ./extensions/mattermost
```
...
4. Configure Genoma and start the gateway.
...
- Approve via:
  - `genoma pairing list mattermost`
  - `genoma pairing approve mattermost <CODE>`
...
Use these target formats with `genoma message send` or cron/webhooks:
