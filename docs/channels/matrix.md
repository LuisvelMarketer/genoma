...
Matrix is an open, decentralized messaging protocol. Genoma connects as a Matrix **user**
...
```bash
genoma plugins install @genoma/matrix
```

Local checkout (when running from a git repo):

```bash
genoma plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `genoma plugins install @genoma/matrix`
   - From a local checkout: `genoma plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: Genoma calls the same
     login endpoint, stores the access token in `~/.genoma/credentials/matrix/credentials.json`,
...
- On first connection, Genoma requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  Genoma logs a warning.
...
`~/.genoma/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `genoma pairing list matrix`
  - `genoma pairing approve matrix <CODE>`
...
- On startup, Genoma resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
