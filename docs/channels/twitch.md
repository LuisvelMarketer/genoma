...
read_when:
  - Setting up Twitch chat integration for Genoma
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. Genoma connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
genoma plugins install @genoma/twitch
```

Local checkout (when running from a git repo):

```bash
genoma plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `GENOMA_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "genoma", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use GENOMA_TWITCH_ACCESS_TOKEN env var)
...
```bash
GENOMA_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
