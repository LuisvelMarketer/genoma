...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects Genoma to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.
...
```bash
genoma plugins install @genoma/feishu
```

Local checkout (when running from a git repo):

```bash
genoma plugins install ./extensions/feishu
```
...
If you just installed Genoma, run the wizard:
...
2. Configuring app credentials in Genoma
...
1. You already ran `genoma channels add` for Feishu
2. The gateway is running (`genoma gateway status`)
...
```bash
genoma pairing approve feishu <CODE>
```
...
2. Run `genoma logs --follow` and look for `chat_id`
...
2. Run `genoma logs --follow` and look for `open_id`
...
| Command                    | Description                   |
| -------------------------- | ----------------------------- |
| `genoma gateway status`  | Show gateway status           |
| `genoma gateway install` | Install/start gateway service |
| `genoma gateway stop`    | Stop gateway service          |
| `genoma gateway restart` | Restart gateway service       |
| `genoma logs --follow`   | Tail gateway logs             |
...
4. Check logs: `genoma logs --follow`
...
5. Ensure the gateway is running: `genoma gateway status`
6. Check logs: `genoma logs --follow`
  ...
  agents: {
    list: [
      { id: "main" },
      {
        ...
        agentDir: "/home/user/.genoma/agents/clawd-fan/agent",
      },
      {
        id: "clawd-xi",
        workspace: "/home/user/clawd-xi",
        agentDir: "/home/user/.genoma/agents/clawd-xi/agent",
      },
    ],
  },
  bindings: [
