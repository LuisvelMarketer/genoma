...
read_when:
  - You want Genoma to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables Genoma to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- The onboarding wizard (`genoma onboard`) and `genoma channels add` list optional channel plugins.
...
```bash
genoma plugins install @genoma/nostr
```

Use a local checkout (dev workflows):

```bash
genoma plugins install --link <path-to-genoma>/extensions/nostr
```
  ...
  "channels": {
    "nostr": {
      ...
      "profile": {
        ...
        "nip05": "genoma@example.com",
        "lud16": "genoma@example.com"
      }
    }
  }
}
