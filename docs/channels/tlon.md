...
Tlon is a decentralized messenger built on Urbit. Genoma connects to your Urbit ship and can
...
```bash
genoma plugins install @genoma/tlon
```

Local checkout (when running from a git repo):

```bash
genoma plugins install ./extensions/tlon
```
...
By default, Genoma blocks private/internal hostnames and IP ranges for this plugin (SSRF hardening).
...
Use these with `genoma message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, Genoma replies in-thread.
