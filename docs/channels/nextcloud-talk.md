...
```bash
genoma plugins install @genoma/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
genoma plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "Genoma" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `genoma pairing list nextcloud-talk`
  - `genoma pairing approve nextcloud-talk <CODE>`
