...
- Publish as `genoma/plugin-sdk` (or export from core under `genoma/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `GenomaPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: GenomaConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: GenomaConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: GenomaConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: GenomaConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: GenomaConfig): string;
  };
};
```
...
- Introduce `genoma/plugin-sdk`.
- Add `api.runtime` to `GenomaPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `genomaRuntime: ">=2026.2.0"`).
