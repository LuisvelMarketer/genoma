...
process.env.GENOMA_PLUGIN_MANIFEST_CACHE_MS ??= "60000";
...
import type { GenomaConfig } from "../src/config/config.js";
import type { OutboundSendDeps } from "../src/infra/outbound/deliver.js";
import { withIsolatedTestHome } from "./test-env.js";

// Set HOME/state isolation before importing any runtime Genoma modules.
...
const createStubPlugin = (params: {
  ...
  config: {
    listAccountIds: (cfg: GenomaConfig) => {
    ...
    resolveAccount: (cfg: GenomaConfig, accountId?: string | null) => {
      const channels = cfg.channels as Record<string, unknown> | undefined;
      const entry = channels?.[params.id];
      if (!entry || typeof entry !== "object") {
        return {};
    ...
    isConfigured: async (_account, cfg: GenomaConfig) => {
      const channels = cfg.channels as Record<string, unknown> | undefined;
