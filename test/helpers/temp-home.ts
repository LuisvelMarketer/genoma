...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked GENOMA_HOME.
  delete process.env.GENOMA_HOME;
  process.env.GENOMA_STATE_DIR = path.join(base, ".genoma");
...
export async function withTempHome<T>(
  ...
  const prefix = opts.prefix ?? "genoma-test-home-";
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".genoma", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
