...
export async function spawnGatewayInstance(name: string): Promise<GatewayInstance> {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `genoma-e2e-${name}-`));
  const configDir = path.join(homeDir, ".genoma");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          GENOMA_SKIP_PROVIDERS: "1",
          GENOMA_SKIP_GMAIL_WATCHER: "1",
          GENOMA_SKIP_CRON: "1",
          GENOMA_SKIP_BROWSER_CONTROL_SERVER: "1",
          GENOMA_SKIP_CANVAS_HOST: "1",
          GENOMA_TEST_MINIMAL_GATEWAY: "1",
          VITEST: "1",
        },
