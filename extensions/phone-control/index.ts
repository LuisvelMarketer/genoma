...
import type { GenomaPluginApi, GenomaPluginService } from "genoma/plugin-sdk";
...
function normalizeDenyList(cfg: GenomaPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: GenomaPluginApi["config"]): string[] {
...
export default function register(api: GenomaPluginApi) {
  ...
  const timerService: GenomaPluginService = {
