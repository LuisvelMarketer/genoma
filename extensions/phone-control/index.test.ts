...
describe("phone-control plugin", () => {
  it("arms sms.send as part of the writes group", async () => {
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "genoma-phone-control-test-"));
    try {
      ...
      let command: GenomaPluginCommandDefinition | undefined;
