...
vi.mock("genoma/plugin-sdk", async () => {
  const actual = await vi.importActual<typeof import("genoma/plugin-sdk")>("genoma/plugin-sdk");
  return {
    ...actual,
...
import type { GenomaPluginServiceContext } from "genoma/plugin-sdk";
import { emitDiagnosticEvent } from "genoma/plugin-sdk";
import { createDiagnosticsOtelService } from "./service.js";

const OTEL_TEST_STATE_DIR = "/tmp/genoma-diagnostics-otel-test";
...
function createTraceOnlyContext(endpoint: string): GenomaPluginServiceContext {
...
describe("diagnostics-otel service", () => {
  ...
  test("records message-flow metrics and spans", async () => {
    ...
    expect(telemetryState.counters.get("genoma.webhook.received")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("genoma.webhook.duration_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.counters.get("genoma.message.queued")?.add).toHaveBeenCalled();
    expect(telemetryState.counters.get("genoma.message.processed")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("genoma.message.duration_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.histograms.get("genoma.queue.wait_ms")?.record).toHaveBeenCalled();
    expect(telemetryState.counters.get("genoma.session.stuck")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("genoma.session.stuck_age_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.counters.get("genoma.run.attempt")?.add).toHaveBeenCalled();

    const spanNames = telemetryState.tracer.startSpan.mock.calls.map((call) => call[0]);
    expect(spanNames).toContain("genoma.webhook.processed");
    expect(spanNames).toContain("genoma.message.processed");
    expect(spanNames).toContain("genoma.session.stuck");
  ...
  test("redacts sensitive data from log attributes before export", async () => {
    ...
    const tokenAttr = emitCall?.attributes?.["genoma.token"];
  ...
  test("redacts sensitive reason in session.state metric attributes", async () => {
    ...
    const sessionCounter = telemetryState.counters.get("genoma.session.state");
    expect(sessionCounter?.add).toHaveBeenCalledWith(
      1,
      expect.objectContaining({
        "genoma.reason": expect.stringContaining("…"),
      }),
    );
    const attrs = sessionCounter?.add.mock.calls[0]?.[1] as Record<string, unknown> | undefined;
    expect(typeof attrs?.["genoma.reason"]).toBe("string");
    expect(String(attrs?.["genoma.reason"])).not.toContain(
