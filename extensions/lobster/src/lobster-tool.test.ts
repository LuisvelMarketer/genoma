...
import type { GenomaPluginApi, GenomaPluginToolContext } from "../../../src/plugins/types.js";
import {
...
function fakeApi(overrides: Partial<GenomaPluginApi> = {}): GenomaPluginApi {
  return {
    id: "lobster",
    name: "lobster",
    source: "test",
...
function fakeCtx(overrides: Partial<GenomaPluginToolContext> = {}): GenomaPluginToolContext {
  return {
    config: {},
    workspaceDir: "/tmp",
    agentDir: "/tmp",
...
describe("lobster plugin tool", () => {
  ...
  beforeAll(async () => {
    ...
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "genoma-lobster-plugin-"));
  });
  ...
  it("can be gated off in sandboxed contexts", async () => {
    const api = fakeApi();
    const factoryTool = (ctx: GenomaPluginToolContext) => {
      if (ctx.sandboxed) {
        return null;
      }
