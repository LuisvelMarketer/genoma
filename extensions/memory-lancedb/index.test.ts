...
const liveEnabled = HAS_OPENAI_KEY && process.env.GENOMA_LIVE_TEST === "1";
...
describe("memory plugin e2e", () => {
  ...
  beforeEach(async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "genoma-memory-test-"));
...
describeLive("memory plugin live tests", () => {
  ...
  beforeEach(async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "genoma-memory-live-"));
    dbPath = path.join(tmpDir, "lancedb");
  });
