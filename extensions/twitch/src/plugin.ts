/**
 * Twitch channel plugin for Genoma.
 *
...
import { buildChannelConfigSchema } from "genoma/plugin-sdk";
 ...
 * for Genoma. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: GenomaConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: GenomaConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: GenomaConfig): boolean => {
