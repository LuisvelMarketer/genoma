...
metadata: { "genoma": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == Genoma `sessions_spawn`
- **File I/O** == Genoma `read`/`write`
- **Remote fetch** == Genoma `web_fetch` (or `exec` with curl when POST is required)
