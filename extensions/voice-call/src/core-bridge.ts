...
function resolveGenomaRoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.GENOMA_ROOT?.trim();
  if (override) {
  ...
  throw new Error("Unable to resolve core root. Set GENOMA_ROOT to the package root.");
}
...
async function importCoreExtensionAPI(): Promise<{
  ...
  const distPath = path.join(resolveGenomaRoot(), "dist", "extensionAPI.js");
