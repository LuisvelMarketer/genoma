...
`genoma.invoke --each`) without adding new Genoma code per workflow.
...
This extension depends on Genoma internal modules (the embedded agent runner).
It is intended to ship as a **bundled** Genoma extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.genoma/extensions` as a standalone plugin directory.
