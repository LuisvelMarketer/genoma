import type { AnyAgentTool, GenomaPluginApi } from "genoma/plugin-sdk";
...
export function createToolFactoryHarness(cfg: GenomaPluginApi["config"]) {
  const registered: RegisteredTool[] = [];

  const api: Pick<GenomaPluginApi, "config" | "logger" | "registerTool"> = {
    config: cfg,
    logger: {
      info: () => {},
      warn: () => {},
