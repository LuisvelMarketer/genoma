...
describe("loginGeminiCliOAuth", () => {
  ...
  const ENV_KEYS = [
    "GENOMA_GEMINI_OAUTH_CLIENT_ID",
    "GENOMA_GEMINI_OAUTH_CLIENT_SECRET",
  ...
  beforeEach(() => {
    ...
    process.env.GENOMA_GEMINI_OAUTH_CLIENT_ID = "test-client-id.apps.googleusercontent.com";
    process.env.GENOMA_GEMINI_OAUTH_CLIENT_SECRET = "GOCSPX-test-client-secret";
  ...
  it("falls back across loadCodeAssist endpoints with aligned headers and metadata", async () => {
    ...
    const fetchMock = vi.fn(async (input: string | URL | Request, init?: RequestInit) => {
      ...
      if (url === USERINFO_URL) {
        return responseJson({ email: "lobster@genoma.ai" });
      }
      if (url === LOAD_PROD) {
