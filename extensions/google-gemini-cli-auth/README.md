# Google Gemini CLI Auth (Genoma plugin)
...
```bash
genoma plugins enable google-gemini-cli-auth
```
...
```bash
genoma models auth login --provider google-gemini-cli --set-default
```
...
- `GENOMA_GEMINI_OAUTH_CLIENT_ID` / `GEMINI_CLI_OAUTH_CLIENT_ID`
- `GENOMA_GEMINI_OAUTH_CLIENT_SECRET` / `GEMINI_CLI_OAUTH_CLIENT_SECRET`
