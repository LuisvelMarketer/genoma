...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["genomaPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["genoma", "pollId"]) ??
    readNestedString(value, ["genoma", "poll", "id"]) ??
    readNestedString(value, ["data", "genomaPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "genoma", "pollId"]);
  if (!pollId) {
    return null;
  }
    ...
    actions: [
      {
        ...
        msteams: {
          type: "messageBack",
          text: "genoma poll vote",
          displayText: "Vote recorded",
          value: { genomaPollId: pollId, pollId },
        },
      },
    ],
  };
