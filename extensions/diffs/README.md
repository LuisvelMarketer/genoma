...
Read-only diff viewer plugin for **Genoma** agents.
...
Set plugin-wide defaults in `~/.genoma/genoma.json`:
...
Before:
Genoma supports plugins.

After:
Genoma supports plugins and hosted diff views.
```
...
- Artifacts are ephemeral and stored in the plugin temp subfolder (`$TMPDIR/genoma-diffs`).
