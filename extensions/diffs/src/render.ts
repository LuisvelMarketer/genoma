...
function renderDiffCard(payload: DiffViewerPayload): string {
  return `<section class="oc-diff-card">
    <diffs-container class="oc-diff-host" data-genoma-diff-host>
      <template shadowrootmode="open">${payload.prerenderedHTML}</template>
    </diffs-container>
    <script type="application/json" data-genoma-diff-payload>${escapeJsonScript(payload)}</script>
  </section>`;
}
...
function buildHtmlDocument(params: {
  ...
  return `<!doctype html>
<html lang="en"${params.runtimeMode === "image" ? ' data-genoma-diffs-ready="true"' : ""}>
  <head>
    <meta charset="utf-8" />
