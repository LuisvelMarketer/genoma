...
export async function handleWhatsAppStart(host: GenomaApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: GenomaApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: GenomaApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: GenomaApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: GenomaApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: GenomaApp): string {
...
function resolveGatewayHttpAuthHeader(host: GenomaApp): string | null {
...
function buildGatewayHttpHeaders(host: GenomaApp): Record<string, string> {
...
export function handleNostrProfileCancel(host: GenomaApp) {
...
export function handleNostrProfileToggleAdvanced(host: GenomaApp) {
  const state = host.nostrProfileFormState;
  if (!state) {
    return;
  }
...
export async function handleNostrProfileSave(host: GenomaApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: GenomaApp) {
  const state = host.nostrProfileFormState;
