...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/genoma/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/genoma");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/genoma/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__GENOMA_CONTROL_UI_BASE_PATH__ = "/genoma";
    const app = mountApp("/genoma/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/genoma");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/genoma/sessions");
  });
